  ...
  */
  ...
  describe('Basic token functions', function () {
    ...
    it('sendToPool: changes balances of Stability pool and user by the correct amounts', async () => {
      await clvToken.setPoolManagerAddress(mockPool, { from: owner })
      const stabilityPoolAddress = contractAddresses.stabilityPool

      const stabilityPool_BalanceBefore = await clvToken.balanceOf(stabilityPoolAddress)
      const bob_BalanceBefore = await clvToken.balanceOf(bob)
      assert.equal(stabilityPool_BalanceBefore, 0)
      assert.equal(bob_BalanceBefore, 100)

      await clvToken.sendToPool(bob, stabilityPoolAddress, 75, { from: mockPool })

      const stabilityPool_BalanceAfter = await clvToken.balanceOf(stabilityPoolAddress)
    ...
    it('returnFromPool: changes balances of Stability pool and user by the correct amounts', async () => {
      ...
      await clvToken.setPoolManagerAddress(mockPool, { from: owner })
      const stabilityPoolAddress = contractAddresses.stabilityPool
      await clvToken.mint(stabilityPoolAddress, 100, { from: mockPool })  
      ...
      await clvToken.returnFromPool(stabilityPoolAddress, bob, 75, { from: mockPool })
