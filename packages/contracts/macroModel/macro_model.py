# -*- coding: utf-8 -*-
"""Copy of Baseline vs Alternative V2 Copy

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1iJPtkjmX2--pXUIh6dS1WpThACtEpjCd
...
def liquidate_troves(troves, index, data, running_totals):
  ...
  running_totals["supply"] -= debt_liquidated
  ...
  if index <= day:
   return_stability = initial_return*(1+shock_return)
    ...
    return_stability = min(0.5, abs(365*(data.loc[index-day:index, 'liquidation_gain'].sum()+data.loc[index-day:index, 'airdrop_gain'].sum())/(price_LUSD_previous*stability_pool_previous)))
  else:
  ...
  return[troves, return_stability, debt_liquidated, ether_liquidated, liquidation_gain, airdrop_gain, n_liquidate, running_totals]
...
def close_troves(troves, index2, price_LUSD_previous):
  ...
  dropped_troves = troves.iloc[drops]
  closed_supply = dropped_troves["Supply"].sum()
  running_totals["supply"] -= closed_supply
  ...
  return[troves, number_closetroves, running_totals]

"""Adjust Troves"""

def adjust_troves(troves, index, running_totals):
  ...
  # for i in range(0, troves.shape[0]):
    ...
    supply_old = working_trove['Supply'] 
  ...
  #A part of the troves are adjusted by adjusting debt
    ...
    if p >= ratio:
      ...
      running_totals['supply'] += (supply_new - supply_old)
  ...
  return[troves, issuance_LUSD_adjust, running_totals]

"""Open Troves"""

def open_troves(troves, index1, price_LUSD_previous, running_totals):
  ...
  for i in range(0, number_opentroves):
    ...
    running_totals['supply'] += supply_trove
  ...
  return[troves, number_opentroves, issuance_LUSD_open, running_totals]
...
def stability_update(stability_pool_previous, return_previous, index,  running_totals):
  ...
  # stability_pool = min(stability_pool, troves["Supply"].sum())
  stability_pool = min(stability_pool, running_totals["supply"])

  last_term = (1+ return_previous- natural_rate_current)**theta

  return[stability_pool]

"""LUSD Price, liquidity pool, and redemption"""

def price_stabilizer(troves, index, data, stability_pool, n_open, running_totals):
...
#Stabilizer
  ...
  if price_LUSD_current < 1 - rate_redemption:
    ...
    while redempted <= redemption_pool:
      ...
      running_totals["supply"] -= redempted
      quantity_working_trove = troves['Supply'][troves.index[0]]
    ...
    running_totals["supply"] -= residual
  ...
  troves = troves.reset_index(drop=True)
  return[price_LUSD_current, liquidity_pool, troves, issuance_LUSD_stabilizer, redemption_fee, n_redempt, redemption_pool, n_open, running_totals]
...
result_open = open_troves(troves, 0, data['Price_LUSD'][0], running_totals)
troves = result_open[0]
issuance_LUSD_open = result_open[2]
running_totals = result_open[3]
...
#trove liquidation & return of stability pool
  result_liquidation = liquidate_troves(troves, index, data, running_totals)
  ...
  running_totals = result_liquidation[7]

#close troves
  result_close = close_troves(troves, index, price_LUSD_previous)
  troves = result_close[0]
  n_close = result_close[1]
  running_totals = result_close[2]
  #if n_close<0:
  #  break

#adjust troves
  result_adjustment = adjust_troves(troves, index, running_totals)
  troves = result_adjustment[0]
  issuance_LUSD_adjust = result_adjustment[1]
  running_totals = result_adjustment[2]

#open troves
  result_open = open_troves(troves, index, price_LUSD_previous, running_totals)
  troves = result_open[0]
...
#Stability Pool
  stability_pool = stability_update(data.loc[index-1,'stability'], return_stability, index, running_totals)[0]

#Calculating Price, Liquidity Pool, and Redemption
  result_price = price_stabilizer(troves, index, data, stability_pool, n_open, running_totals)
...
#Summary
  ...
  supply_LUSD = running_totals["supply"]
