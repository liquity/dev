...
const XBRLTokenTester = artifacts.require("./XBRLTokenTester")
...
/* NOTE: Some of the borrowing tests do not test for specific XBRL fee values. They only test that the
 * fees are non-zero when they should occur, and that they decay over time.
 *
 * Specific XBRL fee values will depend on the final fee schedule used, and the final choice for
...
contract('BorrowerOperations', async accounts => {
  ...
  const testCorpus = ({ withProxy = false }) => {
    ...
    it("addColl(), active Trove: applies pending rewards and updates user's L_ETH, L_XBRLDebt snapshots", async () => {
      // --- SETUP ---

      const { collateral: aliceCollBefore, totalDebt: aliceDebtBefore } = await openTrove({ extraXBRLAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      const { collateral: bobCollBefore, totalDebt: bobDebtBefore } = await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })

      // --- TEST ---

      // price drops to 1ETH:100XBRL, reducing Carol's ICR below MCR
      ...
      const alice_XBRLDebtRewardSnapshot_Before = alice_rewardSnapshot_Before[1]
      ...
      const bob_XBRLDebtRewardSnapshot_Before = bob_rewardSnapshot_Before[1]

      assert.equal(alice_ETHrewardSnapshot_Before, 0)
      assert.equal(alice_XBRLDebtRewardSnapshot_Before, 0)
      assert.equal(bob_ETHrewardSnapshot_Before, 0)
      assert.equal(bob_XBRLDebtRewardSnapshot_Before, 0)
      ...
      const alicePendingXBRLDebtReward = await troveManager.getPendingXBRLDebtReward(alice)
      const bobPendingXBRLDebtReward = await troveManager.getPendingXBRLDebtReward(bob)
      for (reward of [alicePendingETHReward, bobPendingETHReward, alicePendingXBRLDebtReward, bobPendingXBRLDebtReward]) {
      ...
      assert.isTrue(aliceNewDebt.eq(aliceDebtBefore.add(alicePendingXBRLDebtReward)))
      assert.isTrue(bobNewColl.eq(bobCollBefore.add(bobPendingETHReward).add(bobTopUp)))
      assert.isTrue(bobNewDebt.eq(bobDebtBefore.add(bobPendingXBRLDebtReward)))

      /* Check that both Alice and Bob's snapshots of the rewards-per-unit-staked metrics should be updated
       to the latest values of L_ETH and L_XBRLDebt */
      ...
      const alice_XBRLDebtRewardSnapshot_After = alice_rewardSnapshot_After[1]
      ...
      const bob_XBRLDebtRewardSnapshot_After = bob_rewardSnapshot_After[1]

      assert.isAtMost(th.getDifference(alice_ETHrewardSnapshot_After, L_ETH), 100)
      assert.isAtMost(th.getDifference(alice_XBRLDebtRewardSnapshot_After, L_XBRLDebt), 100)
      assert.isAtMost(th.getDifference(bob_ETHrewardSnapshot_After, L_ETH), 100)
      assert.isAtMost(th.getDifference(bob_XBRLDebtRewardSnapshot_After, L_XBRLDebt), 100)
    })
    ...
    //   // --- SETUP ---
    //   // A,B,C add 15/5/5 ETH, withdraw 100/100/900 XBRL
    ...
    //   // close Carol's Trove, liquidating her 5 ether and 900XBRL.
    ...
    it("withdrawColl(): reverts when calling address does not have active trove", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
    ...
    it("withdrawColl(): reverts if system is in Recovery Mode", async () => {
      ...
      // price drops to 1ETH:150XBRL, reducing TCR below 150%
    ...
    it("withdrawColl(): applies pending rewards and updates user's L_ETH, L_XBRLDebt snapshots", async () => {
      // --- SETUP ---
      ...
      // close Carol's Trove, liquidating her 1 ether and 180XBRL.
      await troveManager.liquidate(carol, { from: owner });
      ...
      const pendingDebtReward_A = await troveManager.getPendingXBRLDebtReward(alice)
      const pendingCollReward_B = await troveManager.getPendingETHReward(bob)
      const pendingDebtReward_B = await troveManager.getPendingXBRLDebtReward(bob)
    ...
    it("withdrawXBRL(): reverts when withdrawal would leave trove with ICR < MCR", async () => {
      // alice creates a Trove and adds first collateral
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: bob } })
      ...
      const XBRLwithdrawal = 1  // withdraw 1 wei XBRL

     await assertRevert(borrowerOperations.withdrawXBRL(th._100pct, XBRLwithdrawal, alice, alice, { from: alice }), 
      "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    })

    it("withdrawXBRL(): decays a non-zero base rate", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })

      await openTrove({ extraXBRLAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraXBRLAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraXBRLAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })

      const A_XBRLBal = await xbrlToken.balanceOf(A)
      ...
      // D withdraws XBRL
      await borrowerOperations.withdrawXBRL(th._100pct, dec(1, 18), A, A, { from: D })
      ...
      // E withdraws XBRL
      await borrowerOperations.withdrawXBRL(th._100pct, dec(1, 18), A, A, { from: E })

      const baseRate_3 = await troveManager.baseRate()
      assert.isTrue(baseRate_3.lt(baseRate_2))
    })

    it("withdrawXBRL(): reverts if max fee > 100%", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraXBRLAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraXBRLAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })

      await assertRevert(borrowerOperations.withdrawXBRL(dec(2, 18), dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.withdrawXBRL('1000000000000000001', dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
    })

    it("withdrawXBRL(): reverts if max fee < 0.5% in Normal mode", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraXBRLAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraXBRLAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })

      await assertRevert(borrowerOperations.withdrawXBRL(0, dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.withdrawXBRL(1, dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.withdrawXBRL('4999999999999999', dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
    })

    it("withdrawXBRL(): reverts if fee exceeds max fee percentage", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraXBRLAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(70, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraXBRLAmount: toBN(dec(80, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraXBRLAmount: toBN(dec(180, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })

      const totalSupply = await xbrlToken.totalSupply()
      ...
      // XBRLFee:                  15000000558793542
      // absolute _fee:            15000000558793542
      // actual feePercentage:      5000000186264514
      // user's _maxFeePercentage: 49999999999999999

      const lessThan5pct = '49999999999999999'
      await assertRevert(borrowerOperations.withdrawXBRL(lessThan5pct, dec(3, 18), A, A, { from: A }), "Fee exceeded provided maximum")

      baseRate = await troveManager.baseRate() // expect 5% base rate
      assert.equal(baseRate, dec(5, 16))
      // Attempt with maxFee 1%
      await assertRevert(borrowerOperations.withdrawXBRL(dec(1, 16), dec(1, 18), A, A, { from: B }), "Fee exceeded provided maximum")

      baseRate = await troveManager.baseRate()  // expect 5% base rate
      assert.equal(baseRate, dec(5, 16))
      // Attempt with maxFee 3.754%
      await assertRevert(borrowerOperations.withdrawXBRL(dec(3754, 13), dec(1, 18), A, A, { from: C }), "Fee exceeded provided maximum")

      baseRate = await troveManager.baseRate()  // expect 5% base rate
      assert.equal(baseRate, dec(5, 16))
      // Attempt with maxFee 0.5%%
      await assertRevert(borrowerOperations.withdrawXBRL(dec(5, 15), dec(1, 18), A, A, { from: D }), "Fee exceeded provided maximum")
    })

    it("withdrawXBRL(): succeeds when fee is less than max fee percentage", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraXBRLAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(70, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraXBRLAmount: toBN(dec(80, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      ...
      const tx1 = await borrowerOperations.withdrawXBRL(moreThan5pct, dec(1, 18), A, A, { from: A })
      ...
      // Attempt with maxFee = 5%
      const tx2 = await borrowerOperations.withdrawXBRL(dec(5, 16), dec(1, 18), A, A, { from: B })
      ...
      const tx3 = await borrowerOperations.withdrawXBRL(dec(1, 17), dec(1, 18), A, A, { from: C })
      ...
      const tx4 = await borrowerOperations.withdrawXBRL(dec(37659, 13), dec(1, 18), A, A, { from: D })
      assert.isTrue(tx4.receipt.status)

      // Attempt with maxFee 100%
      const tx5 = await borrowerOperations.withdrawXBRL(dec(1, 18), dec(1, 18), A, A, { from: E })
      assert.isTrue(tx5.receipt.status)
    })

    it("withdrawXBRL(): doesn't change base rate if it is already zero", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })

      await openTrove({ extraXBRLAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraXBRLAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraXBRLAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraXBRLAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      ...
      // D withdraws XBRL
      await borrowerOperations.withdrawXBRL(th._100pct, dec(37, 18), A, A, { from: D })
      ...
      // E opens trove 
      await borrowerOperations.withdrawXBRL(th._100pct, dec(12, 18), A, A, { from: E })

      const baseRate_3 = await troveManager.baseRate()
      assert.equal(baseRate_3, '0')
    })

    it("withdrawXBRL(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      await borrowerOperations.withdrawXBRL(th._100pct, dec(1, 18), C, C, { from: C })
    ...
    it("withdrawXBRL(): borrower can't grief the baseRate and stop it decaying by issuing debt at higher frequency than the decay granularity", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraXBRLAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraXBRLAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("withdrawXBRL(): borrowing at non-zero base rate sends XBRL fee to STBL staking contract", async () => {
      // time fast-forwards 1 year, and oneYearMultisig stakes 1 STBL
      ...
      assert.equal(stblStaking_XBRLBalance_Before, '0')
      ...
      // D withdraws XBRL
      await borrowerOperations.withdrawXBRL(th._100pct, dec(37, 18), C, C, { from: D })

      // Check STBL XBRL balance after has increased
      const stblStaking_XBRLBalance_After = await xbrlToken.balanceOf(stblStaking.address)
      assert.isTrue(stblStaking_XBRLBalance_After.gt(stblStaking_XBRLBalance_Before))
    })
    ...
    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("withdrawXBRL(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct", async () => {
        ...
        const withdrawalTx = await borrowerOperations.withdrawXBRL(th._100pct, toBN(dec(37, 18)), D, D, { from: D })

        const emittedFee = toBN(th.getXBRLFeeFromXBRLBorrowingEvent(withdrawalTx))
    ...
    it("withdrawXBRL(): Borrowing at non-zero base rate increases the STBL staking contract XBRL fees-per-unit-staked", async () => {
      ...
      // D withdraws XBRL
      await borrowerOperations.withdrawXBRL(th._100pct, toBN(dec(37, 18)), D, D, { from: D })

      // Check STBL contract XBRL fees-per-unit-staked has increased
      const F_XBRL_After = await stblStaking.F_XBRL()
      assert.isTrue(F_XBRL_After.gt(F_XBRL_Before))
    })
    ...
    it("withdrawXBRL(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      ...
      const D_XBRLBalanceBefore = await xbrlToken.balanceOf(D)

      // D withdraws XBRL
      const D_XBRLRequest = toBN(dec(37, 18))
      await borrowerOperations.withdrawXBRL(th._100pct, D_XBRLRequest, D, D, { from: D })

      // Check STBL staking XBRL balance has increased
      const stblStaking_XBRLBalance_After = await xbrlToken.balanceOf(stblStaking.address)
      assert.isTrue(stblStaking_XBRLBalance_After.gt(stblStaking_XBRLBalance_Before))

      // Check D's XBRL balance now equals their initial balance plus request XBRL
      const D_XBRLBalanceAfter = await xbrlToken.balanceOf(D)
      assert.isTrue(D_XBRLBalanceAfter.eq(D_XBRLBalanceBefore.add(D_XBRLRequest)))
    })

    it("withdrawXBRL(): Borrowing at zero base rate changes XBRL fees-per-unit-staked", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraXBRLAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraXBRLAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      // A artificially receives STBL, then stakes it
      await stblToken.unprotectedMint(A, dec(100, 18))
      ...
      // D withdraws XBRL
      await borrowerOperations.withdrawXBRL(th._100pct, dec(37, 18), D, D, { from: D })

      // Check STBL XBRL balance after > 0
      const F_XBRL_After = await stblStaking.F_XBRL()
      assert.isTrue(F_XBRL_After.gt('0'))
    })

    it("withdrawXBRL(): Borrowing at zero base rate sends debt request to user", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraXBRLAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraXBRLAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      // Check D's XBRL balance now equals their requested XBRL
      const D_XBRLBalanceAfter = await xbrlToken.balanceOf(D)

      // Check D's trove debt == D's XBRL balance + liquidation reserve
      assert.isTrue(D_XBRLBalanceAfter.eq(D_XBRLBalanceBefore.add(D_XBRLRequest)))
    })

    it("withdrawXBRL(): reverts when calling address does not have active trove", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } })

      // Bob successfully withdraws XBRL
      const txBob = await borrowerOperations.withdrawXBRL(th._100pct, dec(100, 18), bob, bob, { from: bob })
      assert.isTrue(txBob.receipt.status)

      // Carol with no active trove attempts to withdraw XBRL
      try {
        const txCarol = await borrowerOperations.withdrawXBRL(th._100pct, dec(100, 18), carol, carol, { from: carol })
        assert.isFalse(txCarol.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("withdrawXBRL(): reverts when requested withdrawal amount is zero XBRL", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } })

      // Bob successfully withdraws 1e-18 XBRL
      const txBob = await borrowerOperations.withdrawXBRL(th._100pct, 1, bob, bob, { from: bob })
      assert.isTrue(txBob.receipt.status)

      // Alice attempts to withdraw 0 XBRL
      try {
        const txAlice = await borrowerOperations.withdrawXBRL(th._100pct, 0, alice, alice, { from: alice })
    ...
    it("withdrawXBRL(): reverts when system is in Recovery Mode", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
      ...
      // Withdrawal possible when recoveryMode == false
      const txAlice = await borrowerOperations.withdrawXBRL(th._100pct, dec(100, 18), alice, alice, { from: alice })
      assert.isTrue(txAlice.receipt.status)
      ...
      //Check XBRL withdrawal impossible when recoveryMode == true
      try {
        const txBob = await borrowerOperations.withdrawXBRL(th._100pct, 1, bob, bob, { from: bob })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
    ...
    it("withdrawXBRL(): reverts when withdrawal would bring the trove's ICR < MCR", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(11, 17)), extraParams: { from: bob } })

      // Bob tries to withdraw XBRL that would bring his ICR < MCR
      try {
        const txBob = await borrowerOperations.withdrawXBRL(th._100pct, 1, bob, bob, { from: bob })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
    ...
    it("withdrawXBRL(): reverts when a withdrawal would cause the TCR of the system to fall below the CCR", async () => {
      await priceFeed.setPrice(dec(100, 18))
      const price = await priceFeed.getPrice()
      ...
      // Bob attempts to withdraw 1 XBRL.
      // System TCR would be: ((3+3) * 100 ) / (200+201) = 600/401 = 149.62%, i.e. below CCR of 150%.
      try {
        const txBob = await borrowerOperations.withdrawXBRL(th._100pct, dec(1, 18), bob, bob, { from: bob })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("withdrawXBRL(): reverts if system is in Recovery Mode", async () => {
      // --- SETUP ---
      await openTrove({ ICR: toBN(dec(15, 17)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(15, 17)), extraParams: { from: bob } })
      ...
      try {
        const txData = await borrowerOperations.withdrawXBRL(th._100pct, '200', alice, alice, { from: alice })
        assert.isFalse(txData.receipt.status)
      } catch (err) {
        assert.include(err.message, 'revert')
      }
    ...
    it("withdrawXBRL(): increases the Trove's XBRL debt by the correct amount", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      await borrowerOperations.withdrawXBRL(th._100pct, await getNetBorrowingAmount(100), alice, alice, { from: alice })
    ...
    it("withdrawXBRL(): increases XBRL debt in ActivePool by correct amount", async () => {
      ...
      // check before
      const activePool_XBRL_Before = await activePool.getXBRLDebt()
      assert.isTrue(activePool_XBRL_Before.eq(aliceDebtBefore))

      await borrowerOperations.withdrawXBRL(th._100pct, await getNetBorrowingAmount(dec(10000, 18)), alice, alice, { from: alice })

      // check after
      const activePool_XBRL_After = await activePool.getXBRLDebt()
      th.assertIsApproximatelyEqual(activePool_XBRL_After, activePool_XBRL_Before.add(toBN(dec(10000, 18))))
    })

    it("withdrawXBRL(): increases user XBRLToken balance by correct amount", async () => {
      await openTrove({ extraParams: { value: toBN(dec(100, 'ether')), from: alice } })

      // check before
      const alice_XBRLTokenBalance_Before = await xbrlToken.balanceOf(alice)
      assert.isTrue(alice_XBRLTokenBalance_Before.gt(toBN('0')))

      await borrowerOperations.withdrawXBRL(th._100pct, dec(10000, 18), alice, alice, { from: alice })

      // check after
      const alice_XBRLTokenBalance_After = await xbrlToken.balanceOf(alice)
      assert.isTrue(alice_XBRLTokenBalance_After.eq(alice_XBRLTokenBalance_Before.add(toBN(dec(10000, 18)))))
    })

    // --- repayXBRL() ---
    it("repayXBRL(): reverts when repayment would leave trove with ICR < MCR", async () => {
      // alice creates a Trove and adds first collateral
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: bob } })
      ...
      const XBRLRepayment = 1  // 1 wei repayment

     await assertRevert(borrowerOperations.repayXBRL(XBRLRepayment, alice, alice, { from: alice }), 
      "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    })

    it("repayXBRL(): Succeeds when it would leave trove with net debt >= minimum net debt", async () => {
      // Make the XBRL request 2 wei above min net debt to correct for floor division, and make net debt = min net debt + 1 wei
      ...
      const repayTxA = await borrowerOperations.repayXBRL(1, A, A, { from: A })
      assert.isTrue(repayTxA.receipt.status)

      await borrowerOperations.openTrove(th._100pct, dec(20, 25), B, B, { from: B, value: dec(100, 30) })

      const repayTxB = await borrowerOperations.repayXBRL(dec(19, 25), B, B, { from: B })
      assert.isTrue(repayTxB.receipt.status)
    })

    it("repayXBRL(): reverts when it would leave trove with net debt < minimum net debt", async () => {
      // Make the XBRL request 2 wei above min net debt to correct for floor division, and make net debt = min net debt + 1 wei
      await borrowerOperations.openTrove(th._100pct, await getNetBorrowingAmount(MIN_NET_DEBT.add(toBN('2'))), A, A, { from: A, value: dec(100, 30) })

      const repayTxAPromise = borrowerOperations.repayXBRL(2, A, A, { from: A })
    ...
    it("adjustTrove(): Reverts if repaid amount is greater than current debt", async () => {
      const { totalDebt } = await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      XBRL_GAS_COMPENSATION = await borrowerOperations.XBRL_GAS_COMPENSATION()
      const repayAmount = totalDebt.sub(XBRL_GAS_COMPENSATION).add(toBN(1))
      await openTrove({ extraXBRLAmount: repayAmount, ICR: toBN(dec(150, 16)), extraParams: { from: bob } })

      await xbrlToken.transfer(alice, repayAmount, { from: bob })

      await assertRevert(borrowerOperations.adjustTrove(th._100pct, 0, repayAmount, false, alice, alice, { from: alice }))
    })

    it("repayXBRL(): reverts when calling address does not have active trove", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      // Bob successfully repays some XBRL
      const txBob = await borrowerOperations.repayXBRL(dec(10, 18), bob, bob, { from: bob })
      assert.isTrue(txBob.receipt.status)

      // Carol with no active trove attempts to repayXBRL
      try {
        const txCarol = await borrowerOperations.repayXBRL(dec(10, 18), carol, carol, { from: carol })
        assert.isFalse(txCarol.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("repayXBRL(): reverts when attempted repayment is > the debt of the trove", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      ...
      try {
        const txAlice = await borrowerOperations.repayXBRL(aliceDebt.add(toBN(dec(1, 18))), alice, alice, { from: alice })
        assert.isFalse(txAlice.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    //repayXBRL: reduces XBRL debt in Trove
    it("repayXBRL(): reduces the Trove's XBRL debt by the correct amount", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      assert.isTrue(aliceDebtBefore.gt(toBN('0')))

      await borrowerOperations.repayXBRL(aliceDebtBefore.div(toBN(10)), alice, alice, { from: alice })  // Repays 1/10 her debt
    ...
    it("repayXBRL(): decreases XBRL debt in ActivePool by correct amount", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      assert.isTrue(aliceDebtBefore.gt(toBN('0')))
      ...
      // Check before
      const activePool_XBRL_Before = await activePool.getXBRLDebt()
      assert.isTrue(activePool_XBRL_Before.gt(toBN('0')))

      await borrowerOperations.repayXBRL(aliceDebtBefore.div(toBN(10)), alice, alice, { from: alice })  // Repays 1/10 her debt

      // check after
      const activePool_XBRL_After = await activePool.getXBRLDebt()
      th.assertIsApproximatelyEqual(activePool_XBRL_After, activePool_XBRL_Before.sub(aliceDebtBefore.div(toBN(10))))
    })

    it("repayXBRL(): decreases user XBRLToken balance by correct amount", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      assert.isTrue(aliceDebtBefore.gt(toBN('0')))
      ...
      // check after
      const alice_XBRLTokenBalance_After = await xbrlToken.balanceOf(alice)
      th.assertIsApproximatelyEqual(alice_XBRLTokenBalance_After, alice_XBRLTokenBalance_Before.sub(aliceDebtBefore.div(toBN(10))))
    })

    it('repayXBRL(): can repay debt in Recovery Mode', async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      assert.isTrue(aliceDebtBefore.gt(toBN('0')))
      ...
      const tx = await borrowerOperations.repayXBRL(aliceDebtBefore.div(toBN(10)), alice, alice, { from: alice })
    ...
    it("repayXBRL(): Reverts if borrower has insufficient XBRL balance to cover his debt repayment", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      const bobBalBefore = await xbrlToken.balanceOf(B)
      assert.isTrue(bobBalBefore.gt(toBN('0')))

      // Bob transfers all but 5 of his XBRL to Carol
      await xbrlToken.transfer(C, bobBalBefore.sub((toBN(dec(5, 18)))), { from: B })

      //Confirm B's XBRL balance has decreased to 5 XBRL
      const bobBalAfter = await xbrlToken.balanceOf(B)

      assert.isTrue(bobBalAfter.eq(toBN(dec(5, 18))))
      
      // Bob tries to repay 6 XBRL
      const repayXBRLPromise_B = borrowerOperations.repayXBRL(toBN(dec(6, 18)), B, B, { from: B })

      await assertRevert(repayXBRLPromise_B, "Caller doesnt have enough XBRL to make repayment")
    })
    ...
    it("adjustTrove(): reverts when adjustment would leave trove with ICR < MCR", async () => {
     ...
     await assertRevert(borrowerOperations.adjustTrove(th._100pct, 0, XBRLRepayment, false, alice, alice, { from: alice, value: collTopUp }), 
      "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    })
    ...
    it("adjustTrove(): doesn't change base rate if it is already zero", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraXBRLAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it("adjustTrove(): borrowing at non-zero base rate sends XBRL fee to STBL staking contract", async () => {
      ...
      // D adjusts trove
      await openTrove({ extraXBRLAmount: toBN(dec(37, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("adjustTrove(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct", async () => {
        ...
        const emittedFee = toBN(th.getXBRLFeeFromXBRLBorrowingEvent(adjustmentTx))
        assert.isTrue(emittedFee.gt(toBN('0')))
    ...
    it("adjustTrove(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      ...
      // D adjusts trove
      const XBRLRequest_D = toBN(dec(40, 18))
      await borrowerOperations.adjustTrove(th._100pct, 0, XBRLRequest_D, true, D, D, { from: D })

      // Check STBL staking XBRL balance has increased
      const stblStaking_XBRLBalance_After = await xbrlToken.balanceOf(stblStaking.address)
      assert.isTrue(stblStaking_XBRLBalance_After.gt(stblStaking_XBRLBalance_Before))

      // Check D's XBRL balance has increased by their requested XBRL
      const D_XBRLBalanceAfter = await xbrlToken.balanceOf(D)
      assert.isTrue(D_XBRLBalanceAfter.eq(D_XBRLBalanceBefore.add(XBRLRequest_D)))
    })

    it("adjustTrove(): Borrowing at zero base rate changes XBRL balance of STBL staking contract", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraXBRLAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraXBRLAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      // Check staking XBRL balance before > 0
      const stblStaking_XBRLBalance_Before = await xbrlToken.balanceOf(stblStaking.address)
      assert.isTrue(stblStaking_XBRLBalance_Before.gt(toBN('0')))

      // D adjusts trove
      await borrowerOperations.adjustTrove(th._100pct, 0, dec(37, 18), true, D, D, { from: D })

      // Check staking XBRL balance after > staking balance before
      const stblStaking_XBRLBalance_After = await xbrlToken.balanceOf(stblStaking.address)
      assert.isTrue(stblStaking_XBRLBalance_After.gt(stblStaking_XBRLBalance_Before))
    })

    it("adjustTrove(): Borrowing at zero base rate changes STBL staking contract XBRL fees-per-unit-staked", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: toBN(dec(100, 'ether')) } })
      await openTrove({ extraXBRLAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraXBRLAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraXBRLAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      ...
      // Check staking XBRL balance before == 0
      const F_XBRL_Before = await stblStaking.F_XBRL()
      assert.isTrue(F_XBRL_Before.eq(toBN('0')))

      // D adjusts trove
      await borrowerOperations.adjustTrove(th._100pct, 0, dec(37, 18), true, D, D, { from: D })

      // Check staking XBRL balance increases
      const F_XBRL_After = await stblStaking.F_XBRL()
      assert.isTrue(F_XBRL_After.gt(F_XBRL_Before))
    })

    it("adjustTrove(): Borrowing at zero base rate sends total requested XBRL to the user", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: toBN(dec(100, 'ether')) } })
      await openTrove({ extraXBRLAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraXBRLAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      const D_XBRLBalBefore = await xbrlToken.balanceOf(D)
      // Check baseRate is zero
      const baseRate_1 = await troveManager.baseRate()
      assert.equal(baseRate_1, '0')
      ...
      const DUSDBalanceBefore = await xbrlToken.balanceOf(D)

      // D adjusts trove
      const XBRLRequest_D = toBN(dec(40, 18))
      await borrowerOperations.adjustTrove(th._100pct, 0, XBRLRequest_D, true, D, D, { from: D })

      // Check D's XBRL balance increased by their requested XBRL
      const XBRLBalanceAfter = await xbrlToken.balanceOf(D)
      assert.isTrue(XBRLBalanceAfter.eq(D_XBRLBalBefore.add(XBRLRequest_D)))
    })

    it("adjustTrove(): reverts when calling address has no active trove", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraXBRLAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })

      // Alice coll and debt increase(+1 ETH, +50XBRL)
    ...
    it("adjustTrove(): debt increase that would reduce the ICR reverts in Recovery Mode", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(3, 18)), extraParams: { from: alice } })
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const CCR = await troveManager.CCR()
    ...
    it("adjustTrove(): debt increase in Recovery Mode charges no fee", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraXBRLAmount: toBN(dec(200000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      ...
      // B stakes STBL
      await stblToken.unprotectedMint(bob, dec(100, 18))
      await stblStaking.stake(dec(100, 18), { from: bob })

      const stblStakingXBRLBalanceBefore = await xbrlToken.balanceOf(stblStaking.address)
      assert.isTrue(stblStakingXBRLBalanceBefore.gt(toBN('0')))
      ...
      // Check emitted fee = 0
      const emittedFee = toBN(await th.getEventArgByName(txAlice, 'XBRLBorrowingFeePaid', '_XBRLFee'))
      assert.isTrue(emittedFee.eq(toBN('0')))

      assert.isTrue(await th.checkRecoveryMode(contracts))

      // Check no fee was sent to staking contract
      const stblStakingXBRLBalanceAfter = await xbrlToken.balanceOf(stblStaking.address)
      assert.equal(stblStakingXBRLBalanceAfter.toString(), stblStakingXBRLBalanceBefore.toString())
    })
    ...
    it("adjustTrove(): reverts when XBRL repaid is > debt of the trove", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      const bobFee = toBN(await th.getEventArgByIndex(bobOpenTx, 'XBRLBorrowingFeePaid', 1))
      assert.isTrue(bobFee.gt(toBN('0')))

      // Alice transfers XBRL to bob to compensate borrowing fees
      await xbrlToken.transfer(bob, bobFee, { from: alice })

      const remainingDebt = (await troveManager.getTroveDebt(bob)).sub(XBRL_GAS_COMPENSATION)
    ...
    it("adjustTrove(): reverts when change would cause the ICR of the trove to fall below the MCR", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(100, 18)), extraParams: { from: whale } })

      await priceFeed.setPrice(dec(100, 18))

      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(11, 17)), extraParams: { from: alice } })
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(11, 17)), extraParams: { from: bob } })

      // Bob attempts to increase debt by 100 XBRL and 1 ether, i.e. a change that constitutes a 100% ratio of coll:debt.
    ...
    it("adjustTrove(): With 0 coll change, doesnt change borrower's coll or ActivePool coll", async () => {
      ...
      // Alice adjusts trove. No coll change, and a debt increase (+50XBRL)
    ...
    it("adjustTrove(): With 0 debt change, doesnt change borrower's debt or ActivePool debt", async () => {
      ...
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      const activePoolDebtBefore = await activePool.getXBRLDebt()
      ...
      const aliceDebtAfter = await getTroveEntireDebt(alice)
      const activePoolDebtAfter = await activePool.getXBRLDebt()
    ...
    it("adjustTrove(): updates borrower's debt and coll with an increase in both", async () => {
      ...
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      ...
      // Alice adjusts trove. Coll and debt increase(+1 ETH, +50XBRL)
    ...
    it("adjustTrove(): updates borrower's debt and coll with a decrease in both", async () => {
      ...
      // Alice adjusts trove coll and debt decrease (-0.5 ETH, -50XBRL)
    ...
    it("adjustTrove(): updates borrower's  debt and coll with coll increase, debt decrease", async () => {
      ...
      // Alice adjusts trove - coll increase and debt decrease (+0.5 ETH, -50XBRL)
    ...
    it("adjustTrove(): updates borrower's debt and coll with coll decrease, debt increase", async () => {
      ...
      // Alice adjusts trove - coll decrease and debt increase (0.1 ETH, 10XBRL)
    ...
    it("adjustTrove(): updates borrower's stake and totalStakes with a coll increase", async () => {
      ...
      // Alice adjusts trove - coll and debt increase (+1 ETH, +50 XBRL)
      await borrowerOperations.adjustTrove(th._100pct, 0, dec(50, 18), true, alice, alice, { from: alice, value: dec(1, 'ether') })
    ...
    it("adjustTrove(): changes XBRLToken balance by the requested decrease", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // check after
      const alice_XBRLTokenBalance_After = await xbrlToken.balanceOf(alice)
      assert.isTrue(alice_XBRLTokenBalance_After.eq(alice_XBRLTokenBalance_Before.sub(toBN(dec(10, 18)))))
    })

    it("adjustTrove(): changes XBRLToken balance by the requested increase", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // check after
      const alice_XBRLTokenBalance_After = await xbrlToken.balanceOf(alice)
      assert.isTrue(alice_XBRLTokenBalance_After.eq(alice_XBRLTokenBalance_Before.add(toBN(dec(100, 18)))))
    })
    ...
    it("adjustTrove(): Changes the XBRL debt in ActivePool by requested decrease", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })

      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: alice } })

      const activePool_XBRLDebt_Before = await activePool.getXBRLDebt()
      assert.isTrue(activePool_XBRLDebt_Before.gt(toBN('0')))
      ...
      const activePool_XBRLDebt_After = await activePool.getXBRLDebt()
      assert.isTrue(activePool_XBRLDebt_After.eq(activePool_XBRLDebt_Before.sub(toBN(dec(30, 18)))))
    })

    it("adjustTrove(): Changes the XBRL debt in ActivePool by requested increase", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      ...
      th.assertIsApproximatelyEqual(activePool_XBRLDebt_After, activePool_XBRLDebt_Before.add(toBN(dec(100, 18))))
    })
    ...
    it("adjustTrove(): Reverts if borrower has insufficient XBRL balance to cover his debt repayment", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: B } })
      const bobDebt = await getTroveEntireDebt(B)

      // Bob transfers some XBRL to carol
      await xbrlToken.transfer(C, dec(10, 18), { from: B })

      //Confirm B's XBRL balance is less than 50 XBRL
      const B_XBRLBal = await xbrlToken.balanceOf(B)
      assert.isTrue(B_XBRLBal.lt(bobDebt))

      const repayXBRLPromise_B = borrowerOperations.adjustTrove(th._100pct, 0, bobDebt, false, B, B, { from: B })

      // B attempts to repay all his debt
      await assertRevert(repayXBRLPromise_B, "revert")
    })
    ...
    if (!withProxy) { // no need to test this with proxies
      it("Internal _adjustTrove(): reverts when op is a withdrawal and _borrower param is not the msg.sender", async () => {
        await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
        await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: bob } })
    ...
    it("closeTrove(): reverts when it would lower the TCR below CCR", async () => {
      ...
      await openTrove({ ICR: toBN(dec(120, 16)), extraXBRLAmount: toBN(dec(300, 18)), extraParams:{ from: bob } })

      const price = await priceFeed.getPrice()
      
      // to compensate borrowing fees
      await xbrlToken.transfer(alice, dec(300, 18), { from: bob })
    ...
    it("closeTrove(): reverts when system is in Recovery Mode", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(100000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })

      // Alice transfers her XBRL to Bob and Carol so they can cover fees
      const aliceBal = await xbrlToken.balanceOf(alice)
      await xbrlToken.transfer(bob, aliceBal.div(toBN(2)), { from: alice })
      await xbrlToken.transfer(carol, aliceBal.div(toBN(2)), { from: alice })
    ...
    it("closeTrove(): reverts when trove is the only one in the system", async () => {
      ...
      await xbrlToken.unprotectedMint(alice, dec(100000, 18))

      // Check she has more XBRL than her trove debt
      const aliceBal = await xbrlToken.balanceOf(alice)
      const aliceDebt = await getTroveEntireDebt(alice)
    ...
    it("closeTrove(): reduces a Trove's collateral to zero", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })

      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })

      const aliceCollBefore = await getTroveEntireColl(alice)
      const dennisXBRL = await xbrlToken.balanceOf(dennis)
      assert.isTrue(aliceCollBefore.gt(toBN('0')))
      assert.isTrue(dennisXBRL.gt(toBN('0')))

      // To compensate borrowing fees
      await xbrlToken.transfer(alice, dennisXBRL.div(toBN(2)), { from: dennis })
    ...
    it("closeTrove(): zero's the troves reward snapshots", async () => {
      ...
      const L_XBRLDebt_A_Snapshot = (await troveManager.rewardSnapshots(alice))[1]
      assert.isTrue(L_ETH_A_Snapshot.gt(toBN('0')))
      assert.isTrue(L_XBRLDebt_A_Snapshot.gt(toBN('0')))
      ...
      const L_XBRLDebt_Snapshot_A_AfterLiquidation = (await troveManager.rewardSnapshots(alice))[1]

      assert.isTrue(L_ETH_Snapshot_A_AfterLiquidation.gt(toBN('0')))
      assert.isTrue(L_XBRLDebt_Snapshot_A_AfterLiquidation.gt(toBN('0')))

      // to compensate borrowing fees
      await xbrlToken.transfer(alice, await xbrlToken.balanceOf(dennis), { from: dennis })
      ...
      const L_XBRLDebt_Snapshot_A_afterAliceCloses = (await troveManager.rewardSnapshots(alice))[1]

      assert.equal(L_ETH_Snapshot_A_afterAliceCloses, '0')
      assert.equal(L_XBRLDebt_Snapshot_A_afterAliceCloses, '0')
    })
    ...
    it("closeTrove(): reduces ActivePool debt by correct amount", async () => {
      ...
      // Check before
      const activePool_Debt_before = await activePool.getXBRLDebt()
      ...
      // Check after
      const activePool_Debt_After = (await activePool.getXBRLDebt()).toString()
    ...
    it("closeTrove(): subtracts the debt of the closed Trove from the Borrower's XBRLToken balance", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      const alice_XBRLBalance_Before = await xbrlToken.balanceOf(alice)
      assert.isTrue(alice_XBRLBalance_Before.gt(toBN('0')))

      // close trove
      await borrowerOperations.closeTrove({ from: alice })

      // check alice XBRL balance after
      const alice_XBRLBalance_After = await xbrlToken.balanceOf(alice)
      th.assertIsApproximatelyEqual(alice_XBRLBalance_After, alice_XBRLBalance_Before.sub(aliceDebt.sub(XBRL_GAS_COMPENSATION)))
    })

    it("closeTrove(): applies pending rewards", async () => {
      // --- SETUP ---
      await openTrove({ extraXBRLAmount: toBN(dec(1000000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      const whaleDebt = await getTroveEntireDebt(whale)
      const whaleColl = await getTroveEntireColl(whale)

      await openTrove({ extraXBRLAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
      ...
      await xbrlToken.transfer(alice, dec(10000, 18), { from: whale })
      await xbrlToken.transfer(bob, dec(10000, 18), { from: whale })
      ...
      const defaultPool_XBRLDebt = await defaultPool.getXBRLDebt()

      // Carol's liquidated coll (1 ETH) and drawn debt should have entered the Default Pool
      assert.isAtMost(th.getDifference(defaultPool_ETH, liquidatedColl_C), 100)
      assert.isAtMost(th.getDifference(defaultPool_XBRLDebt, liquidatedDebt_C), 100)
      ...
      const defaultPool_XBRLDebt_afterAliceCloses = await defaultPool.getXBRLDebt()

      assert.isAtMost(th.getDifference(defaultPool_ETH_afterAliceCloses,
        defaultPool_ETH.sub(pendingCollReward_A)), 1000)
      assert.isAtMost(th.getDifference(defaultPool_XBRLDebt_afterAliceCloses,
        defaultPool_XBRLDebt.sub(pendingDebtReward_A)), 1000)
      ...
      const defaultPool_XBRLDebt_afterBobCloses = await defaultPool.getXBRLDebt()

      assert.isAtMost(th.getDifference(defaultPool_ETH_afterBobCloses, 0), 100000)
      assert.isAtMost(th.getDifference(defaultPool_XBRLDebt_afterBobCloses, 0), 100000)
    })

    it("closeTrove(): reverts if borrower has insufficient XBRL balance to repay his entire debt", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })

      //Confirm Bob's XBRL balance is less than his trove debt
      const B_XBRLBal = await xbrlToken.balanceOf(B)
      const B_troveDebt = await getTroveEntireDebt(B)

      assert.isTrue(B_XBRLBal.lt(B_troveDebt))

      const closeTrovePromise_B = borrowerOperations.closeTrove({ from: B })

      // Check closing trove reverts
      await assertRevert(closeTrovePromise_B, "BorrowerOps: Caller doesnt have enough XBRL to make repayment")
    })
    ...
    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("openTrove(): emits a TroveUpdated event with the correct collateral and debt", async () => {
        const txA = (await openTrove({ extraXBRLAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })).tx
        const txB = (await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })).tx
        const txC = (await openTrove({ extraXBRLAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })).tx
        ...
        const txD = (await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })).tx
        const txE = (await openTrove({ extraXBRLAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })).tx
    ...
    it("openTrove(): decays a non-zero base rate", async () => {
      ...
      // E opens trove 
      await openTrove({ extraXBRLAmount: toBN(dec(12, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
    ...
    it("openTrove(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
      ...
      await openTrove({ extraXBRLAmount: toBN(dec(1, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      ...
      await openTrove({ extraXBRLAmount: toBN(dec(1, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
    ...
    it("openTrove(): borrowing at non-zero base rate sends XBRL fee to STBL staking contract", async () => {
      // time fast-forwards 1 year, and oneYearMultisig stakes 1 STBL
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await stblToken.approve(stblStaking.address, dec(1, 18), { from: oneYearMultisig })
      await stblStaking.stake(dec(1, 18), { from: oneYearMultisig })
    ...
    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("openTrove(): borrowing at non-zero base records the (drawn debt + fee  + liq. reserve) on the Trove struct", async () => {
        ...
        const D_XBRLRequest = toBN(dec(20000, 18))

        // D withdraws XBRL
        const openTroveTx = await borrowerOperations.openTrove(th._100pct, D_XBRLRequest, ZERO_ADDRESS, ZERO_ADDRESS, { from: D, value: dec(200, 'ether') })

        const emittedFee = toBN(th.getXBRLFeeFromXBRLBorrowingEvent(openTroveTx))
        assert.isTrue(toBN(emittedFee).gt(toBN('0')))

        const newDebt = (await troveManager.Troves(D))[0]

        // Check debt on Trove struct equals drawn debt plus emitted fee
        th.assertIsApproximatelyEqual(newDebt, D_XBRLRequest.add(emittedFee).add(XBRL_GAS_COMPENSATION), 100000)
      })
    }

    it("openTrove(): Borrowing at non-zero base rate increases the STBL staking contract XBRL fees-per-unit-staked", async () => {
      // time fast-forwards 1 year, and oneYearMultisig stakes 1 STBL
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await stblToken.approve(stblStaking.address, dec(1, 18), { from: oneYearMultisig })
      await stblStaking.stake(dec(1, 18), { from: oneYearMultisig })
    ...
    it("openTrove(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      ...
      // D opens trove 
      const XBRLRequest_D = toBN(dec(40000, 18))
      await borrowerOperations.openTrove(th._100pct, XBRLRequest_D, D, D, { from: D, value: dec(500, 'ether') })
      ...
      // Check D's XBRL balance now equals their requested XBRL
      const XBRLBalance_D = await xbrlToken.balanceOf(D)
      assert.isTrue(XBRLRequest_D.eq(XBRLBalance_D))
    })

    it("openTrove(): Borrowing at zero base rate changes the STBL staking contract XBRL fees-per-unit-staked", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      // Check XBRL reward per STBL staked == 0
      const F_XBRL_Before = await stblStaking.F_XBRL()
      assert.equal(F_XBRL_Before, '0')
      ...
      // Check XBRL reward per STBL staked > 0
      const F_XBRL_After = await stblStaking.F_XBRL()
      assert.isTrue(F_XBRL_After.gt(toBN('0')))
    })
    ...
    it("openTrove(): Borrowing at zero base rate charges minimum fee", async () => {
      ...
      const XBRLRequest = toBN(dec(10000, 18))
      const txC = await borrowerOperations.openTrove(th._100pct, XBRLRequest, ZERO_ADDRESS, ZERO_ADDRESS, { value: dec(100, 'ether'), from: C })
      const _XBRLFee = toBN(th.getEventArgByName(txC, "XBRLBorrowingFeePaid", "_XBRLFee"))

      const expectedFee = BORROWING_FEE_FLOOR.mul(toBN(XBRLRequest)).div(toBN(dec(1, 18)))
      assert.isTrue(_XBRLFee.eq(expectedFee))
    })

    it("openTrove(): reverts when system is in Recovery Mode and ICR < CCR", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
      await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      assert.isFalse(await th.checkRecoveryMode(contracts))
      ...
      try {
        const txBob = await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(149, 16)), extraParams: { from: alice } })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("openTrove(): reverts when trove ICR < MCR", async () => {
      ...
      try {
        const txBob = (await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(109, 16)), extraParams: { from: bob } })).tx
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
      ...
      try {
        const txBob = await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(109, 16)), extraParams: { from: bob } })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("openTrove(): reverts when opening the trove would cause the TCR of the system to fall below the CCR", async () => {
      ...
      await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(15, 17)), extraParams: { from: alice } })
      ...
      try {
        const txBob = await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(149, 16)), extraParams: { from: bob } })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("openTrove(): reverts if trove is already active", async () => {
      ...
      await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(15, 17)), extraParams: { from: alice } })
      await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(15, 17)), extraParams: { from: bob } })

      try {
        const txB_1 = await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(3, 18)), extraParams: { from: bob } })
    ...
    it("openTrove(): Can open a trove with ICR >= CCR when system is in Recovery Mode", async () => {
      ...
      // price drops to 1ETH:100XBRL, reducing TCR below 150%
      await priceFeed.setPrice('100000000000000000000');
      const price = await priceFeed.getPrice()
      ...
      const txCarol = (await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(15, 17)), extraParams: { from: carol } })).tx
    ...
    it("openTrove(): creates a new Trove and assigns the correct collateral and debt amount", async () => {
      ...
      const XBRLRequest = MIN_NET_DEBT
      borrowerOperations.openTrove(th._100pct, MIN_NET_DEBT, carol, carol, { from: alice, value: dec(100, 'ether') })

      // Get the expected debt based on the XBRL request (adding fee and liq. reserve on top)
      const expectedDebt = XBRLRequest
        .add(await troveManager.getBorrowingFee(XBRLRequest))
        .add(XBRL_GAS_COMPENSATION)
    ...
    it("openTrove(): records up-to-date initial snapshots of L_ETH and L_XBRLDebt", async () => {
      // --- SETUP ---
      ...
      /* with total stakes = 10 ether, after liquidation, L_ETH should equal 1/10 ether per-ether-staked,
       and L_XBRL should equal 18 XBRL per-ether-staked. */

      const L_ETH = await troveManager.L_ETH()
      const L_XBRL = await troveManager.L_XBRLDebt()

      assert.isTrue(L_ETH.gt(toBN('0')))
      assert.isTrue(L_XBRL.gt(toBN('0')))

      // Bob opens trove
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })

      // Check Bob's snapshots of L_ETH and L_XBRL equal the respective current values
      const bob_rewardSnapshot = await troveManager.rewardSnapshots(bob)
      const bob_ETHrewardSnapshot = bob_rewardSnapshot[0]
      const bob_XBRLDebtRewardSnapshot = bob_rewardSnapshot[1]

      assert.isAtMost(th.getDifference(bob_ETHrewardSnapshot, L_ETH), 1000)
      assert.isAtMost(th.getDifference(bob_XBRLDebtRewardSnapshot, L_XBRL), 1000)
    })

    it("openTrove(): allows a user to open a Trove, then close it, then re-open it", async () => {
      // Open Troves
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
      await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
    ...
    it("openTrove(): increases the Trove's XBRL debt by the correct amount", async () => {
      // check before
      const alice_Trove_Before = await troveManager.Troves(alice)
      const debt_Before = alice_Trove_Before[0]
      assert.equal(debt_Before, 0)

      await borrowerOperations.openTrove(th._100pct, await getOpenTroveXBRLAmount(dec(10000, 18)), alice, alice, { from: alice, value: dec(100, 'ether') })
    ...
    it("openTrove(): increases XBRL debt in ActivePool by the debt of the trove", async () => {
      const activePool_XBRLDebt_Before = await activePool.getXBRLDebt()
      assert.equal(activePool_XBRLDebt_Before, 0)
      ...
      const activePool_XBRLDebt_After = await activePool.getXBRLDebt()
      assert.isTrue(activePool_XBRLDebt_After.eq(aliceDebt))
    })

    it("openTrove(): increases user XBRLToken balance by correct amount", async () => {
      // check before
      const alice_XBRLTokenBalance_Before = await xbrlToken.balanceOf(alice)
      assert.equal(alice_XBRLTokenBalance_Before, 0)

      await borrowerOperations.openTrove(th._100pct, dec(10000, 18), alice, alice, { from: alice, value: dec(100, 'ether') })

      // check after
      const alice_XBRLTokenBalance_After = await xbrlToken.balanceOf(alice)
      assert.equal(alice_XBRLTokenBalance_After, dec(10000, 18))
    })
    ...
    it("getCompositeDebt(): returns debt + gas comp", async () => {
      ...
      assert.equal(res1, XBRL_GAS_COMPENSATION.toString())

      const res2 = await borrowerOperations.getCompositeDebt(dec(90, 18))
      th.assertIsApproximatelyEqual(res2, XBRL_GAS_COMPENSATION.add(toBN(dec(90, 18))))

      const res3 = await borrowerOperations.getCompositeDebt(dec(24423422357345049, 12))
      th.assertIsApproximatelyEqual(res3, XBRL_GAS_COMPENSATION.add(toBN(dec(24423422357345049, 12))))
    })
    ...
    describe("getNewTCRFromTroveChange() returns the correct TCR", async () => {
      ...
      it("collChange = 0, debtChange = 0", async () => {
        ...
        await borrowerOperations.openTrove(th._100pct, troveXBRLAmount, alice, alice, { from: alice, value: troveColl })
        await borrowerOperations.openTrove(th._100pct, troveXBRLAmount, bob, bob, { from: bob, value: troveColl })
    ...
    if (!withProxy) {
      it('closeTrove(): fails if owner cannot receive ETH', async () => {
        ...
        // Alice sends XBRL to NonPayable so its XBRL balance covers its debt
        await xbrlToken.transfer(nonPayable.address, dec(10000, 18), {from: alice})
...
/* TODO:
 ...
 changes with addColl, withdrawColl, withdrawXBRL, repayXBRL, etc. Can split them up and put them with
