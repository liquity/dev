...
const ONEUSDTokenTester = artifacts.require("./ONEUSDTokenTester")
...
/* NOTE: Some of the borrowing tests do not test for specific 1USD fee values. They only test that the
 * fees are non-zero when they should occur, and that they decay over time.
 *
 * Specific 1USD fee values will depend on the final fee schedule used, and the final choice for
...
contract('BorrowerOperations', async accounts => {
  ...
  const testCorpus = ({ withProxy = false }) => {
    ...
    it("addColl(): Increases the activePool ONE and raw ether balance by correct amount", async () => {
      const { collateral: aliceColl } = await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })

      const activePool_ONE_Before = await activePool.getONE()
      const activePool_RawEther_Before = toBN(await web3.eth.getBalance(activePool.address))

      assert.isTrue(activePool_ONE_Before.eq(aliceColl))
      ...
      const activePool_ONE_After = await activePool.getONE()
      const activePool_RawEther_After = toBN(await web3.eth.getBalance(activePool.address))
      assert.isTrue(activePool_ONE_After.eq(aliceColl.add(toBN(dec(1, 'ether')))))
    ...
    it("addColl(), active Trove: applies pending rewards and updates user's L_ONE, L_ONEUSDDebt snapshots", async () => {
      // --- SETUP ---

      const { collateral: aliceCollBefore, totalDebt: aliceDebtBefore } = await openTrove({ extra1USDAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      const { collateral: bobCollBefore, totalDebt: bobDebtBefore } = await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ extra1USDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })

      // --- TEST ---

      // price drops to 1ONE:100 1USD, reducing Carol's ICR below MCR
      ...
      const L_ONE = await troveManager.L_ONE()
      ...
      const alice_ONErewardSnapshot_Before = alice_rewardSnapshot_Before[0]
      const alice_1USDDebtRewardSnapshot_Before = alice_rewardSnapshot_Before[1]

      const bob_rewardSnapshot_Before = await troveManager.rewardSnapshots(bob)
      const bob_ONErewardSnapshot_Before = bob_rewardSnapshot_Before[0]
      const bob_1USDDebtRewardSnapshot_Before = bob_rewardSnapshot_Before[1]

      assert.equal(alice_ONErewardSnapshot_Before, 0)
      assert.equal(alice_1USDDebtRewardSnapshot_Before, 0)
      assert.equal(bob_ONErewardSnapshot_Before, 0)
      assert.equal(bob_1USDDebtRewardSnapshot_Before, 0)

      const alicePendingONEReward = await troveManager.getPendingONEReward(alice)
      const bobPendingONEReward = await troveManager.getPendingONEReward(bob)
      const alicePending1USDDebtReward = await troveManager.getPending1USDDebtReward(alice)
      const bobPending1USDDebtReward = await troveManager.getPending1USDDebtReward(bob)
      for (reward of [alicePendingONEReward, bobPendingONEReward, alicePending1USDDebtReward, bobPending1USDDebtReward]) {
      ...
      assert.isTrue(aliceNewColl.eq(aliceCollBefore.add(alicePendingONEReward).add(aliceTopUp)))
      assert.isTrue(aliceNewDebt.eq(aliceDebtBefore.add(alicePending1USDDebtReward)))
      assert.isTrue(bobNewColl.eq(bobCollBefore.add(bobPendingONEReward).add(bobTopUp)))
      assert.isTrue(bobNewDebt.eq(bobDebtBefore.add(bobPending1USDDebtReward)))

      /* Check that both Alice and Bob's snapshots of the rewards-per-unit-staked metrics should be updated
       to the latest values of L_ONE and L_1USDDebt */
      const alice_rewardSnapshot_After = await troveManager.rewardSnapshots(alice)
      const alice_ONErewardSnapshot_After = alice_rewardSnapshot_After[0]
      const alice_1USDDebtRewardSnapshot_After = alice_rewardSnapshot_After[1]

      const bob_rewardSnapshot_After = await troveManager.rewardSnapshots(bob)
      const bob_ONErewardSnapshot_After = bob_rewardSnapshot_After[0]
      const bob_1USDDebtRewardSnapshot_After = bob_rewardSnapshot_After[1]

      assert.isAtMost(th.getDifference(alice_ONErewardSnapshot_After, L_ONE), 100)
      assert.isAtMost(th.getDifference(alice_1USDDebtRewardSnapshot_After, L_1USDDebt), 100)
      assert.isAtMost(th.getDifference(bob_ONErewardSnapshot_After, L_ONE), 100)
      assert.isAtMost(th.getDifference(bob_1USDDebtRewardSnapshot_After, L_1USDDebt), 100)
    })
    ...
    //   // --- SETUP ---
    //   // A,B,C add 15/5/5 ONE, withdraw 100/100/900ONEUSD
    ...
    //   // close Carol's Trove, liquidating her 5 ether and 900 1USD.
    //   await troveManager.liquidate(carol, { from: owner });

    //   // dennis tops up his trove by 1 ONE
    ...
    //   the ONEfrom her Trove has now become the totalPendingONEReward. So:

    //   totalStakes = (alice_Stake + bob_Stake + dennis_orig_stake ) = (15 + 4 + 1) =  20 ONE.
    //   totalCollateral = (alice_Collateral + bob_Collateral + dennis_orig_coll + totalPendingONEReward) = (15 + 4 + 1 + 5)  = 25 ONE.

    //   Therefore, as Dennis adds 1 ether collateral, his corrected stake should be:  s = 2 * (20 / 25 ) = 1.6 ONE*/
    ...
    it("withdrawColl(): reverts when requested ONEwithdrawal is > the trove's collateral", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
    ...
    it("withdrawColl(): reverts if system is in Recovery Mode", async () => {
      ...
      // price drops to 1ONE:150 1USD, reducing TCR below 150%
    ...
    it("withdrawColl(): reduces ActivePool ONEand raw ether by correct amount", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      const aliceCollBefore = await getTroveEntireColl(alice)
      ...
      const activePool_ONE_After = await activePool.getONE()
      const activePool_RawEther_After = toBN(await web3.eth.getBalance(activePool.address))
      assert.isTrue(activePool_ONE_After.eq(activePool_ONE_before.sub(toBN(dec(1, 'ether')))))
    ...
    it("withdrawColl(): sends the correct amount of ONEto the user", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice, value: dec(2, 'ether') } })

      const alice_ONEBalance_Before = toBN(web3.utils.toBN(await web3.eth.getBalance(alice)))
      await borrowerOperations.withdrawColl(dec(1, 'ether'), alice, alice, { from: alice, gasPrice: 0 })

      const alice_ONEBalance_After = toBN(web3.utils.toBN(await web3.eth.getBalance(alice)))
      const balanceDiff = alice_ONEBalance_After.sub(alice_ONEBalance_Before)

      assert.isTrue(balanceDiff.eq(toBN(dec(1, 'ether'))))
    })

    it("withdrawColl(): applies pending rewards and updates user's L_ONE, L_1USDDebt snapshots", async () => {
      // --- SETUP ---
      ...
      // close Carol's Trove, liquidating her 1 ether and 180 1USD.
      await troveManager.liquidate(carol, { from: owner });
      ...
      const pendingCollReward_A = await troveManager.getPendingONEReward(alice)
      const pendingDebtReward_A = await troveManager.getPending1USDDebtReward(alice)
      const pendingCollReward_B = await troveManager.getPendingONEReward(bob)
      const pendingDebtReward_B = await troveManager.getPending1USDDebtReward(bob)
    ...
    it("withdraw1USD(): reverts when withdrawal would leave trove with ICR < MCR", async () => {
      // alice creates a Trove and adds first collateral
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: bob } })
      ...
      const ONEUSDwithdrawal = 1  // withdraw 1 weiONEUSD

      await assertRevert(borrowerOperations.withdraw1USD(th._100pct, ONEUSDwithdrawal, alice, alice, { from: alice }),
        "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    })

    it("withdraw1USD(): decays a non-zero base rate", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })

      await openTrove({ extra1USDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extra1USDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extra1USDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extra1USDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })

      const A_1USDBal = await oneusdToken.balanceOf(A)
      ...
      // D withdrawsONEUSD
      await borrowerOperations.withdraw1USD(th._100pct, dec(1, 18), A, A, { from: D })
      ...
      // E withdrawsONEUSD
      await borrowerOperations.withdraw1USD(th._100pct, dec(1, 18), A, A, { from: E })

      const baseRate_3 = await troveManager.baseRate()
      assert.isTrue(baseRate_3.lt(baseRate_2))
    })

    it("withdraw1USD(): reverts if max fee > 100%", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extra1USDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extra1USDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extra1USDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })

      await assertRevert(borrowerOperations.withdraw1USD(dec(2, 18), dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.withdraw1USD('1000000000000000001', dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
    })

    it("withdraw1USD(): reverts if max fee < 0.5% in Normal mode", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extra1USDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extra1USDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extra1USDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })

      await assertRevert(borrowerOperations.withdraw1USD(0, dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.withdraw1USD(1, dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.withdraw1USD('4999999999999999', dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
    })

    it("withdraw1USD(): reverts if fee exceeds max fee percentage", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extra1USDAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extra1USDAmount: toBN(dec(70, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extra1USDAmount: toBN(dec(80, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extra1USDAmount: toBN(dec(180, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      ...
      //ONEUSDFee:                  15000000558793542
      // absolute _fee:            15000000558793542
      // actual feePercentage:      5000000186264514
      // user's _maxFeePercentage: 49999999999999999

      const lessThan5pct = '49999999999999999'
      await assertRevert(borrowerOperations.withdraw1USD(lessThan5pct, dec(3, 18), A, A, { from: A }), "Fee exceeded provided maximum")

      baseRate = await troveManager.baseRate() // expect 5% base rate
      assert.equal(baseRate, dec(5, 16))
      // Attempt with maxFee 1%
      await assertRevert(borrowerOperations.withdraw1USD(dec(1, 16), dec(1, 18), A, A, { from: B }), "Fee exceeded provided maximum")

      baseRate = await troveManager.baseRate()  // expect 5% base rate
      assert.equal(baseRate, dec(5, 16))
      // Attempt with maxFee 3.754%
      await assertRevert(borrowerOperations.withdraw1USD(dec(3754, 13), dec(1, 18), A, A, { from: C }), "Fee exceeded provided maximum")

      baseRate = await troveManager.baseRate()  // expect 5% base rate
      assert.equal(baseRate, dec(5, 16))
      // Attempt with maxFee 0.5%%
      await assertRevert(borrowerOperations.withdraw1USD(dec(5, 15), dec(1, 18), A, A, { from: D }), "Fee exceeded provided maximum")
    })

    it("withdraw1USD(): succeeds when fee is less than max fee percentage", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extra1USDAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extra1USDAmount: toBN(dec(70, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extra1USDAmount: toBN(dec(80, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      ...
      const tx1 = await borrowerOperations.withdraw1USD(moreThan5pct, dec(1, 18), A, A, { from: A })
      ...
      // Attempt with maxFee = 5%
      const tx2 = await borrowerOperations.withdraw1USD(dec(5, 16), dec(1, 18), A, A, { from: B })
      ...
      const tx3 = await borrowerOperations.withdraw1USD(dec(1, 17), dec(1, 18), A, A, { from: C })
      ...
      const tx4 = await borrowerOperations.withdraw1USD(dec(37659, 13), dec(1, 18), A, A, { from: D })
      assert.isTrue(tx4.receipt.status)

      // Attempt with maxFee 100%
      const tx5 = await borrowerOperations.withdraw1USD(dec(1, 18), dec(1, 18), A, A, { from: E })
      assert.isTrue(tx5.receipt.status)
    })

    it("withdraw1USD(): doesn't change base rate if it is already zero", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })

      await openTrove({ extra1USDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extra1USDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extra1USDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extra1USDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extra1USDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      ...
      // D withdrawsONEUSD
      await borrowerOperations.withdraw1USD(th._100pct, dec(37, 18), A, A, { from: D })
      ...
      // E opens trove 
      await borrowerOperations.withdraw1USD(th._100pct, dec(12, 18), A, A, { from: E })

      const baseRate_3 = await troveManager.baseRate()
      assert.equal(baseRate_3, '0')
    })

    it("withdraw1USD(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      await borrowerOperations.withdraw1USD(th._100pct, dec(1, 18), C, C, { from: C })
    ...
    it("withdraw1USD(): borrower can't grief the baseRate and stop it decaying by issuing debt at higher frequency than the decay granularity", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extra1USDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extra1USDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extra1USDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("withdraw1USD(): borrowing at non-zero base rate sends 1USDfee to LQTY staking contract", async () => {
      ...
      // Check LQTY 1USDbalance before == 0
      const lqtyStaking_1USDBalance_Before = await oneusdToken.balanceOf(lqtyStaking.address)
      assert.equal(lqtyStaking_1USDBalance_Before, '0')
      ...
      // D withdrawsONEUSD
      await borrowerOperations.withdraw1USD(th._100pct, dec(37, 18), C, C, { from: D })

      // Check LQTY 1USDbalance after has increased
      const lqtyStaking_1USDBalance_After = await oneusdToken.balanceOf(lqtyStaking.address)
      assert.isTrue(lqtyStaking_1USDBalance_After.gt(lqtyStaking_1USDBalance_Before))
    })

    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("withdraw1USD(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct", async () => {
        // time fast-forwards 1 year, and multisig stakes 1 LQTY
        await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
        await lqtyToken.approve(lqtyStaking.address, dec(1, 18), { from: multisig })
        await lqtyStaking.stake(dec(1, 18), { from: multisig })
        ...
        const withdrawalTx = await borrowerOperations.withdraw1USD(th._100pct, toBN(dec(37, 18)), D, D, { from: D })

        const emittedFee = toBN(th.get1USDFeeFrom1USDBorrowingEvent(withdrawalTx))
    ...
    it("withdraw1USD(): Borrowing at non-zero base rate increases the LQTY staking contract 1USDfees-per-unit-staked", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 LQTY
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await lqtyToken.approve(lqtyStaking.address, dec(1, 18), { from: multisig })
      await lqtyStaking.stake(dec(1, 18), { from: multisig })

      // Check LQTY contract 1USDfees-per-unit-staked is zero
      const F_1USD_Before = await lqtyStaking.F_1USD()
      ...
      // D withdrawsONEUSD
      await borrowerOperations.withdraw1USD(th._100pct, toBN(dec(37, 18)), D, D, { from: D })

      // Check LQTY contract 1USDfees-per-unit-staked has increased
      const F_1USD_After = await lqtyStaking.F_1USD()
      assert.isTrue(F_1USD_After.gt(F_1USD_Before))
    })

    it("withdraw1USD(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 LQTY
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await lqtyToken.approve(lqtyStaking.address, dec(1, 18), { from: multisig })
      await lqtyStaking.stake(dec(1, 18), { from: multisig })
      ...
      const D_1USDBalanceBefore = await oneusdToken.balanceOf(D)

      // D withdrawsONEUSD
      const D_1USDRequest = toBN(dec(37, 18))
      await borrowerOperations.withdraw1USD(th._100pct, D_1USDRequest, D, D, { from: D })

      // Check LQTY staking 1USDbalance has increased
      const lqtyStaking_1USDBalance_After = await oneusdToken.balanceOf(lqtyStaking.address)
      assert.isTrue(lqtyStaking_1USDBalance_After.gt(lqtyStaking_1USDBalance_Before))

      // Check D's 1USDbalance now equals their initial balance plus requestONEUSD
      const D_1USDBalanceAfter = await oneusdToken.balanceOf(D)
      assert.isTrue(D_1USDBalanceAfter.eq(D_1USDBalanceBefore.add(D_1USDRequest)))
    })

    it("withdraw1USD(): Borrowing at zero base rate changes 1USDfees-per-unit-staked", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extra1USDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extra1USDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extra1USDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      // D withdrawsONEUSD
      await borrowerOperations.withdraw1USD(th._100pct, dec(37, 18), D, D, { from: D })

      // Check LQTY 1USDbalance after > 0
      const F_1USD_After = await lqtyStaking.F_1USD()
      assert.isTrue(F_1USD_After.gt('0'))
    })

    it("withdraw1USD(): Borrowing at zero base rate sends debt request to user", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extra1USDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extra1USDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extra1USDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      // Check D's 1USDbalance now equals their requestedONEUSD
      const D_1USDBalanceAfter = await oneusdToken.balanceOf(D)

      // Check D's trove debt == D's 1USDbalance + liquidation reserve
      assert.isTrue(D_1USDBalanceAfter.eq(D_1USDBalanceBefore.add(D_1USDRequest)))
    })

    it("withdraw1USD(): reverts when calling address does not have active trove", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } })

      // Bob successfully withdrawsONEUSD
      const txBob = await borrowerOperations.withdraw1USD(th._100pct, dec(100, 18), bob, bob, { from: bob })
      assert.isTrue(txBob.receipt.status)

      // Carol with no active trove attempts to withdrawONEUSD
      try {
        const txCarol = await borrowerOperations.withdraw1USD(th._100pct, dec(100, 18), carol, carol, { from: carol })
        assert.isFalse(txCarol.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("withdraw1USD(): reverts when requested withdrawal amount is zeroONEUSD", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } })

      // Bob successfully withdraws 1e-18ONEUSD
      const txBob = await borrowerOperations.withdraw1USD(th._100pct, 1, bob, bob, { from: bob })
      assert.isTrue(txBob.receipt.status)

      // Alice attempts to withdraw 0ONEUSD
      try {
        const txAlice = await borrowerOperations.withdraw1USD(th._100pct, 0, alice, alice, { from: alice })
    ...
    it("withdraw1USD(): reverts when system is in Recovery Mode", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
      ...
      // Withdrawal possible when recoveryMode == false
      const txAlice = await borrowerOperations.withdraw1USD(th._100pct, dec(100, 18), alice, alice, { from: alice })
      assert.isTrue(txAlice.receipt.status)
      ...
      //Check 1USDwithdrawal impossible when recoveryMode == true
      try {
        const txBob = await borrowerOperations.withdraw1USD(th._100pct, 1, bob, bob, { from: bob })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
    ...
    it("withdraw1USD(): reverts when withdrawal would bring the trove's ICR < MCR", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(11, 17)), extraParams: { from: bob } })

      // Bob tries to withdraw 1USDthat would bring his ICR < MCR
      try {
        const txBob = await borrowerOperations.withdraw1USD(th._100pct, 1, bob, bob, { from: bob })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
    ...
    it("withdraw1USD(): reverts when a withdrawal would cause the TCR of the system to fall below the CCR", async () => {
      await priceFeed.setPrice(dec(100, 18))
      const price = await priceFeed.getPrice()
      ...
      // Bob attempts to withdraw 1ONEUSD.
      // System TCR would be: ((3+3) * 100 ) / (200+201) = 600/401 = 149.62%, i.e. below CCR of 150%.
      try {
        const txBob = await borrowerOperations.withdraw1USD(th._100pct, dec(1, 18), bob, bob, { from: bob })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("withdraw1USD(): reverts if system is in Recovery Mode", async () => {
      // --- SETUP ---
      await openTrove({ ICR: toBN(dec(15, 17)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(15, 17)), extraParams: { from: bob } })
      ...
      try {
        const txData = await borrowerOperations.withdraw1USD(th._100pct, '200', alice, alice, { from: alice })
        assert.isFalse(txData.receipt.status)
      } catch (err) {
        assert.include(err.message, 'revert')
      }
    ...
    it("withdraw1USD(): increases the Trove's 1USDdebt by the correct amount", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      await borrowerOperations.withdraw1USD(th._100pct, await getNetBorrowingAmount(100), alice, alice, { from: alice })
    ...
    it("withdraw1USD(): increases 1USDdebt in ActivePool by correct amount", async () => {
      ...
      // check before
      const activePool_1USD_Before = await activePool.get1USDDebt()
      assert.isTrue(activePool_1USD_Before.eq(aliceDebtBefore))

      await borrowerOperations.withdraw1USD(th._100pct, await getNetBorrowingAmount(dec(10000, 18)), alice, alice, { from: alice })

      // check after
      const activePool_1USD_After = await activePool.get1USDDebt()
      th.assertIsApproximatelyEqual(activePool_1USD_After, activePool_1USD_Before.add(toBN(dec(10000, 18))))
    })

    it("withdraw1USD(): increases userONEUSDToken balance by correct amount", async () => {
      await openTrove({ extraParams: { value: toBN(dec(100, 'ether')), from: alice } })

      // check before
      const alice_1USDTokenBalance_Before = await oneusdToken.balanceOf(alice)
      assert.isTrue(alice_1USDTokenBalance_Before.gt(toBN('0')))

      await borrowerOperations.withdraw1USD(th._100pct, dec(10000, 18), alice, alice, { from: alice })

      // check after
      const alice_1USDTokenBalance_After = await oneusdToken.balanceOf(alice)
      assert.isTrue(alice_1USDTokenBalance_After.eq(alice_1USDTokenBalance_Before.add(toBN(dec(10000, 18)))))
    })

    // --- repay1USD() ---
    it("repay1USD(): reverts when repayment would leave trove with ICR < MCR", async () => {
      // alice creates a Trove and adds first collateral
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: bob } })
      ...
      const ONEUSDRepayment = 1  // 1 wei repayment

      await assertRevert(borrowerOperations.repay1USD(ONEUSDRepayment, alice, alice, { from: alice }),
        "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    })

    it("repay1USD(): Succeeds when it would leave trove with net debt >= minimum net debt", async () => {
      // Make the 1USDrequest 2 wei above min net debt to correct for floor division, and make net debt = min net debt + 1 wei
      ...
      const repayTxA = await borrowerOperations.repay1USD(1, A, A, { from: A })
      assert.isTrue(repayTxA.receipt.status)

      await borrowerOperations.openTrove(th._100pct, dec(20, 25), B, B, { from: B, value: dec(100, 30) })

      const repayTxB = await borrowerOperations.repay1USD(dec(19, 25), B, B, { from: B })
      assert.isTrue(repayTxB.receipt.status)
    })

    it("repay1USD(): reverts when it would leave trove with net debt < minimum net debt", async () => {
      // Make the 1USDrequest 2 wei above min net debt to correct for floor division, and make net debt = min net debt + 1 wei
      await borrowerOperations.openTrove(th._100pct, await getNetBorrowingAmount(MIN_NET_DEBT.add(toBN('2'))), A, A, { from: A, value: dec(100, 30) })

      const repayTxAPromise = borrowerOperations.repay1USD(2, A, A, { from: A })
    ...
    it("adjustTrove(): Reverts if repaid amount is greater than current debt", async () => {
      const { totalDebt } = await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      ONEUSD_GAS_COMPENSATION = await borrowerOperations.ONEUSD_GAS_COMPENSATION()
      const repayAmount = totalDebt.sub(ONEUSD_GAS_COMPENSATION).add(toBN(1))
      await openTrove({ extra1USDAmount: repayAmount, ICR: toBN(dec(150, 16)), extraParams: { from: bob } })

      await oneusdToken.transfer(alice, repayAmount, { from: bob })

      await assertRevert(borrowerOperations.adjustTrove(th._100pct, 0, repayAmount, false, alice, alice, { from: alice }),
        "SafeMath: subtraction overflow")
    })

    it("repay1USD(): reverts when calling address does not have active trove", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      // Bob successfully repays someONEUSD
      const txBob = await borrowerOperations.repay1USD(dec(10, 18), bob, bob, { from: bob })
      assert.isTrue(txBob.receipt.status)

      // Carol with no active trove attempts to repay1USD
      try {
        const txCarol = await borrowerOperations.repay1USD(dec(10, 18), carol, carol, { from: carol })
        assert.isFalse(txCarol.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("repay1USD(): reverts when attempted repayment is > the debt of the trove", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      ...
      try {
        const txAlice = await borrowerOperations.repay1USD(aliceDebt.add(toBN(dec(1, 18))), alice, alice, { from: alice })
        assert.isFalse(txAlice.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    //repay1USD: reduces 1USDdebt in Trove
    it("repay1USD(): reduces the Trove's 1USDdebt by the correct amount", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      assert.isTrue(aliceDebtBefore.gt(toBN('0')))

      await borrowerOperations.repay1USD(aliceDebtBefore.div(toBN(10)), alice, alice, { from: alice })  // Repays 1/10 her debt
    ...
    it("repay1USD(): decreases 1USDdebt in ActivePool by correct amount", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      assert.isTrue(aliceDebtBefore.gt(toBN('0')))
      ...
      // Check before
      const activePool_1USD_Before = await activePool.get1USDDebt()
      assert.isTrue(activePool_1USD_Before.gt(toBN('0')))

      await borrowerOperations.repay1USD(aliceDebtBefore.div(toBN(10)), alice, alice, { from: alice })  // Repays 1/10 her debt

      // check after
      const activePool_1USD_After = await activePool.get1USDDebt()
      th.assertIsApproximatelyEqual(activePool_1USD_After, activePool_1USD_Before.sub(aliceDebtBefore.div(toBN(10))))
    })

    it("repay1USD(): decreases userONEUSDToken balance by correct amount", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      assert.isTrue(aliceDebtBefore.gt(toBN('0')))
      ...
      // check after
      const alice_1USDTokenBalance_After = await oneusdToken.balanceOf(alice)
      th.assertIsApproximatelyEqual(alice_1USDTokenBalance_After, alice_1USDTokenBalance_Before.sub(aliceDebtBefore.div(toBN(10))))
    })

    it('repay1USD(): can repay debt in Recovery Mode', async () => {
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      assert.isTrue(aliceDebtBefore.gt(toBN('0')))
      ...
      const tx = await borrowerOperations.repay1USD(aliceDebtBefore.div(toBN(10)), alice, alice, { from: alice })
    ...
    it("repay1USD(): Reverts if borrower has insufficient 1USDbalance to cover his debt repayment", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      const bobBalBefore = await oneusdToken.balanceOf(B)
      assert.isTrue(bobBalBefore.gt(toBN('0')))

      // Bob transfers all but 5 of his 1USDto Carol
      await oneusdToken.transfer(C, bobBalBefore.sub((toBN(dec(5, 18)))), { from: B })

      //Confirm B's 1USDbalance has decreased to 5ONEUSD
      const bobBalAfter = await oneusdToken.balanceOf(B)

      assert.isTrue(bobBalAfter.eq(toBN(dec(5, 18))))

      // Bob tries to repay 6ONEUSD
      const repay1USDPromise_B = borrowerOperations.repay1USD(toBN(dec(6, 18)), B, B, { from: B })

      await assertRevert(repay1USDPromise_B, "Caller doesnt have enough 1USDto make repayment")
    })
    ...
    it("adjustTrove(): reverts when adjustment would leave trove with ICR < MCR", async () => {
      ...
      await assertRevert(borrowerOperations.adjustTrove(th._100pct, 0, ONEUSDRepayment, false, alice, alice, { from: alice, value: collTopUp }),
        "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    })
    ...
    it("adjustTrove(): doesn't change base rate if it is already zero", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extra1USDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it("adjustTrove(): borrowing at non-zero base rate sends 1USDfee to LQTY staking contract", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 LQTY
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await lqtyToken.approve(lqtyStaking.address, dec(1, 18), { from: multisig })
      await lqtyStaking.stake(dec(1, 18), { from: multisig })
      ...
      // D adjusts trove
      await openTrove({ extra1USDAmount: toBN(dec(37, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("adjustTrove(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct", async () => {
        ...
        const emittedFee = toBN(th.get1USDFeeFrom1USDBorrowingEvent(adjustmentTx))
        assert.isTrue(emittedFee.gt(toBN('0')))
    ...
    it("adjustTrove(): Borrowing at non-zero base rate increases the LQTY staking contract 1USDfees-per-unit-staked", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 LQTY
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await lqtyToken.approve(lqtyStaking.address, dec(1, 18), { from: multisig })
      await lqtyStaking.stake(dec(1, 18), { from: multisig })
    ...
    it("adjustTrove(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      ...
      // D adjusts trove
      const ONEUSDRequest_D = toBN(dec(40, 18))
      await borrowerOperations.adjustTrove(th._100pct, 0, ONEUSDRequest_D, true, D, D, { from: D })

      // Check LQTY staking 1USDbalance has increased
      const lqtyStaking_1USDBalance_After = await oneusdToken.balanceOf(lqtyStaking.address)
      assert.isTrue(lqtyStaking_1USDBalance_After.gt(lqtyStaking_1USDBalance_Before))

      // Check D's 1USDbalance has increased by their requestedONEUSD
      const D_1USDBalanceAfter = await oneusdToken.balanceOf(D)
      assert.isTrue(D_1USDBalanceAfter.eq(D_1USDBalanceBefore.add(ONEUSDRequest_D)))
    })

    it("adjustTrove(): Borrowing at zero base rate changes 1USDbalance of LQTY staking contract", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extra1USDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extra1USDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extra1USDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      // Check staking 1USDbalance before > 0
      const lqtyStaking_1USDBalance_Before = await oneusdToken.balanceOf(lqtyStaking.address)
      assert.isTrue(lqtyStaking_1USDBalance_Before.gt(toBN('0')))

      // D adjusts trove
      await borrowerOperations.adjustTrove(th._100pct, 0, dec(37, 18), true, D, D, { from: D })

      // Check staking 1USDbalance after > staking balance before
      const lqtyStaking_1USDBalance_After = await oneusdToken.balanceOf(lqtyStaking.address)
      assert.isTrue(lqtyStaking_1USDBalance_After.gt(lqtyStaking_1USDBalance_Before))
    })

    it("adjustTrove(): Borrowing at zero base rate changes LQTY staking contract 1USDfees-per-unit-staked", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: toBN(dec(100, 'ether')) } })
      await openTrove({ extra1USDAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      ...
      // Check staking 1USDbalance before == 0
      const F_1USD_Before = await lqtyStaking.F_1USD()
      assert.isTrue(F_1USD_Before.eq(toBN('0')))

      // D adjusts trove
      await borrowerOperations.adjustTrove(th._100pct, 0, dec(37, 18), true, D, D, { from: D })

      // Check staking 1USDbalance increases
      const F_1USD_After = await lqtyStaking.F_1USD()
      assert.isTrue(F_1USD_After.gt(F_1USD_Before))
    })

    it("adjustTrove(): Borrowing at zero base rate sends total requested 1USDto the user", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: toBN(dec(100, 'ether')) } })
      await openTrove({ extra1USDAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extra1USDAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extra1USDAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      const D_1USDBalBefore = await oneusdToken.balanceOf(D)
      // Check baseRate is zero
      const baseRate_1 = await troveManager.baseRate()
      assert.equal(baseRate_1, '0')
      ...
      const DUSDBalanceBefore = await oneusdToken.balanceOf(D)

      // D adjusts trove
      const ONEUSDRequest_D = toBN(dec(40, 18))
      await borrowerOperations.adjustTrove(th._100pct, 0, ONEUSDRequest_D, true, D, D, { from: D })

      // Check D's 1USDbalance increased by their requestedONEUSD
      const ONEUSDBalanceAfter = await oneusdToken.balanceOf(D)
      assert.isTrue(ONEUSDBalanceAfter.eq(D_1USDBalBefore.add(ONEUSDRequest_D)))
    })
    ...
    it("adjustTrove(): reverts when calling address has no active trove", async () => {
      ...
      // Alice coll and debt increase(+1 ONE, +50 1USD)
    ...
    it("adjustTrove(): reverts in Recovery Mode when the adjustment would reduce the TCR", async () => {
      ...
      await priceFeed.setPrice(dec(120, 18)) // trigger drop in ONEprice
    ...
    it("adjustTrove(): debt increase that would reduce the ICR reverts in Recovery Mode", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(3, 18)), extraParams: { from: alice } })
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const CCR = await troveManager.CCR()

      assert.isFalse(await th.checkRecoveryMode(contracts))

      await priceFeed.setPrice(dec(105, 18)) // trigger drop in ONEprice
      const price = await priceFeed.getPrice()
    ...
    it("adjustTrove(): A trove with ICR < CCR in Recovery Mode can adjust their trove to ICR > CCR", async () => {
      ...
      await priceFeed.setPrice(dec(100, 18)) // trigger drop in ONEprice
      const price = await priceFeed.getPrice()
    ...
    it("adjustTrove(): debt increase in Recovery Mode charges no fee", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extra1USDAmount: toBN(dec(200000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      ...
      const lqtyStaking1USDBalanceBefore = await oneusdToken.balanceOf(lqtyStaking.address)
      assert.isTrue(lqtyStaking1USDBalanceBefore.gt(toBN('0')))
      ...
      // Check emitted fee = 0
      const emittedFee = toBN(await th.getEventArgByName(txAlice, 'ONEUSDBorrowingFeePaid', '_1USDFee'))
      assert.isTrue(emittedFee.eq(toBN('0')))

      assert.isTrue(await th.checkRecoveryMode(contracts))

      // Check no fee was sent to staking contract
      const lqtyStaking1USDBalanceAfter = await oneusdToken.balanceOf(lqtyStaking.address)
      assert.equal(lqtyStaking1USDBalanceAfter.toString(), lqtyStaking1USDBalanceBefore.toString())
    })
    ...
    it("adjustTrove(): reverts when 1USDrepaid is > debt of the trove", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      const bobFee = toBN(await th.getEventArgByIndex(bobOpenTx, 'ONEUSDBorrowingFeePaid', 1))
      assert.isTrue(bobFee.gt(toBN('0')))

      // Alice transfers 1USDto bob to compensate borrowing fees
      await oneusdToken.transfer(bob, bobFee, { from: alice })

      const remainingDebt = (await troveManager.getTroveDebt(bob)).sub(ONEUSD_GAS_COMPENSATION)
    ...
    it("adjustTrove(): reverts when attempted ONEwithdrawal is >= the trove's collateral", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
      ...
      // Carol attempts an adjustment that would withdraw 1 wei more than her ONE
      try {
    ...
    it("adjustTrove(): reverts when change would cause the ICR of the trove to fall below the MCR", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(100, 18)), extraParams: { from: whale } })

      await priceFeed.setPrice(dec(100, 18))

      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(11, 17)), extraParams: { from: alice } })
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(11, 17)), extraParams: { from: bob } })

      // Bob attempts to increase debt by 100 1USDand 1 ether, i.e. a change that constitutes a 100% ratio of coll:debt.
    ...
    it("adjustTrove(): With 0 coll change, doesnt change borrower's coll or ActivePool coll", async () => {
      ...
      const aliceCollBefore = await getTroveEntireColl(alice)
      const activePoolCollBefore = await activePool.getONE()

      assert.isTrue(aliceCollBefore.gt(toBN('0')))
      assert.isTrue(aliceCollBefore.eq(activePoolCollBefore))

      // Alice adjusts trove. No coll change, and a debt increase (+50 1USD)
      await borrowerOperations.adjustTrove(th._100pct, 0, dec(50, 18), true, alice, alice, { from: alice, value: 0 })

      const aliceCollAfter = await getTroveEntireColl(alice)
      const activePoolCollAfter = await activePool.getONE()
    ...
    it("adjustTrove(): With 0 debt change, doesnt change borrower's debt or ActivePool debt", async () => {
      ...
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      const activePoolDebtBefore = await activePool.get1USDDebt()
      ...
      const aliceDebtAfter = await getTroveEntireDebt(alice)
      const activePoolDebtAfter = await activePool.get1USDDebt()
    ...
    it("adjustTrove(): updates borrower's debt and coll with an increase in both", async () => {
      ...
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      ...
      // Alice adjusts trove. Coll and debt increase(+1 ONE, +50 1USD)
    ...
    it("adjustTrove(): updates borrower's debt and coll with a decrease in both", async () => {
      ...
      // Alice adjusts trove coll and debt decrease (-0.5 ONE, -50 1USD)
    ...
    it("adjustTrove(): updates borrower's  debt and coll with coll increase, debt decrease", async () => {
      ...
      // Alice adjusts trove - coll increase and debt decrease (+0.5 ONE, -50 1USD)
    ...
    it("adjustTrove(): updates borrower's debt and coll with coll decrease, debt increase", async () => {
      ...
      // Alice adjusts trove - coll decrease and debt increase (0.1 ONE, 10 1USD)
    ...
    it("adjustTrove(): updates borrower's stake and totalStakes with a coll increase", async () => {
      ...
      // Alice adjusts trove - coll and debt increase (+1 ONE, +50ONEUSD)
      await borrowerOperations.adjustTrove(th._100pct, 0, dec(50, 18), true, alice, alice, { from: alice, value: dec(1, 'ether') })
    ...
    it("adjustTrove(): changesONEUSDToken balance by the requested decrease", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // check after
      const alice_1USDTokenBalance_After = await oneusdToken.balanceOf(alice)
      assert.isTrue(alice_1USDTokenBalance_After.eq(alice_1USDTokenBalance_Before.sub(toBN(dec(10, 18)))))
    })

    it("adjustTrove(): changesONEUSDToken balance by the requested increase", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // check after
      const alice_1USDTokenBalance_After = await oneusdToken.balanceOf(alice)
      assert.isTrue(alice_1USDTokenBalance_After.eq(alice_1USDTokenBalance_Before.add(toBN(dec(100, 18)))))
    })

    it("adjustTrove(): Changes the activePool ONEand raw ether balance by the requested decrease", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      const activePool_ONE_Before = await activePool.getONE()
      const activePool_RawEther_Before = toBN(await web3.eth.getBalance(activePool.address))
      assert.isTrue(activePool_ONE_Before.gt(toBN('0')))
      ...
      const activePool_ONE_After = await activePool.getONE()
      const activePool_RawEther_After = toBN(await web3.eth.getBalance(activePool.address))
      assert.isTrue(activePool_ONE_After.eq(activePool_ONE_Before.sub(toBN(dec(1, 17)))))
      assert.isTrue(activePool_RawEther_After.eq(activePool_ONE_Before.sub(toBN(dec(1, 17)))))
    })

    it("adjustTrove(): Changes the activePool ONEand raw ether balance by the amount of ONEsent", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      const activePool_ONE_After = await activePool.getONE()
      const activePool_RawEther_After = toBN(await web3.eth.getBalance(activePool.address))
      assert.isTrue(activePool_ONE_After.eq(activePool_ONE_Before.add(toBN(dec(1, 18)))))
      assert.isTrue(activePool_RawEther_After.eq(activePool_ONE_Before.add(toBN(dec(1, 18)))))
    })

    it("adjustTrove(): Changes the 1USDdebt in ActivePool by requested decrease", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })

      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: alice } })

      const activePool_1USDDebt_Before = await activePool.get1USDDebt()
      assert.isTrue(activePool_1USDDebt_Before.gt(toBN('0')))
      ...
      const activePool_1USDDebt_After = await activePool.get1USDDebt()
      assert.isTrue(activePool_1USDDebt_After.eq(activePool_1USDDebt_Before.sub(toBN(dec(30, 18)))))
    })

    it("adjustTrove(): Changes the 1USDdebt in ActivePool by requested increase", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      ...
      th.assertIsApproximatelyEqual(activePool_1USDDebt_After, activePool_1USDDebt_Before.add(toBN(dec(100, 18))))
    })
    ...
    it("adjustTrove(): Reverts if borrower has insufficient 1USDbalance to cover his debt repayment", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: B } })
      const bobDebt = await getTroveEntireDebt(B)

      // Bob transfers some 1USDto carol
      await oneusdToken.transfer(C, dec(10, 18), { from: B })

      //Confirm B's 1USDbalance is less than 50ONEUSD
      const B_1USDBal = await oneusdToken.balanceOf(B)
      assert.isTrue(B_1USDBal.lt(bobDebt))

      const repay1USDPromise_B = borrowerOperations.adjustTrove(th._100pct, 0, bobDebt, false, B, B, { from: B })

      // B attempts to repay all his debt
      await assertRevert(repay1USDPromise_B, "revert")
    })
    ...
    if (!withProxy) { // no need to test this with proxies
      it("Internal _adjustTrove(): reverts when op is a withdrawal and _borrower param is not the msg.sender", async () => {
        await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
        await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: bob } })
    ...
    it("closeTrove(): reverts when it would lower the TCR below CCR", async () => {
      ...
      await openTrove({ ICR: toBN(dec(120, 16)), extra1USDAmount: toBN(dec(300, 18)), extraParams: { from: bob } })

      const price = await priceFeed.getPrice()

      // to compensate borrowing fees
      await oneusdToken.transfer(alice, dec(300, 18), { from: bob })
    ...
    it("closeTrove(): reverts when system is in Recovery Mode", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(100000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })

      // Alice transfers her 1USDto Bob and Carol so they can cover fees
      const aliceBal = await oneusdToken.balanceOf(alice)
      await oneusdToken.transfer(bob, aliceBal.div(toBN(2)), { from: alice })
      await oneusdToken.transfer(carol, aliceBal.div(toBN(2)), { from: alice })
    ...
    it("closeTrove(): reverts when trove is the only one in the system", async () => {
      ...
      await oneusdToken.unprotectedMint(alice, dec(100000, 18))

      // Check she has more 1USDthan her trove debt
      const aliceBal = await oneusdToken.balanceOf(alice)
      const aliceDebt = await getTroveEntireDebt(alice)
    ...
    it("closeTrove(): reduces a Trove's collateral to zero", async () => {
      ...
      const aliceCollBefore = await getTroveEntireColl(alice)
      const dennis1USD = await oneusdToken.balanceOf(dennis)
      assert.isTrue(aliceCollBefore.gt(toBN('0')))
      assert.isTrue(dennis1USD.gt(toBN('0')))

      // To compensate borrowing fees
      await oneusdToken.transfer(alice, dennis1USD.div(toBN(2)), { from: dennis })
    ...
    it("closeTrove(): zero's the troves reward snapshots", async () => {
      ...
      const L_ONE_A_Snapshot = (await troveManager.rewardSnapshots(alice))[0]
      const L_1USDDebt_A_Snapshot = (await troveManager.rewardSnapshots(alice))[1]
      assert.isTrue(L_ONE_A_Snapshot.gt(toBN('0')))
      assert.isTrue(L_1USDDebt_A_Snapshot.gt(toBN('0')))
      ...
      const L_ONE_Snapshot_A_AfterLiquidation = (await troveManager.rewardSnapshots(alice))[0]
      const L_1USDDebt_Snapshot_A_AfterLiquidation = (await troveManager.rewardSnapshots(alice))[1]

      assert.isTrue(L_ONE_Snapshot_A_AfterLiquidation.gt(toBN('0')))
      assert.isTrue(L_1USDDebt_Snapshot_A_AfterLiquidation.gt(toBN('0')))

      // to compensate borrowing fees
      await oneusdToken.transfer(alice, await oneusdToken.balanceOf(dennis), { from: dennis })
      ...
      const L_ONE_Snapshot_A_afterAliceCloses = (await troveManager.rewardSnapshots(alice))[0]
      const L_1USDDebt_Snapshot_A_afterAliceCloses = (await troveManager.rewardSnapshots(alice))[1]

      assert.equal(L_ONE_Snapshot_A_afterAliceCloses, '0')
      assert.equal(L_1USDDebt_Snapshot_A_afterAliceCloses, '0')
    })
    ...
    it("closeTrove(): reduces ActivePool ONEand raw ether by correct amount", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      // Check active Pool ONEbefore
      const activePool_ONE_before = await activePool.getONE()
      const activePool_RawEther_before = toBN(await web3.eth.getBalance(activePool.address))
      assert.isTrue(activePool_ONE_before.eq(aliceColl.add(dennisColl)))
      assert.isTrue(activePool_ONE_before.gt(toBN('0')))
      assert.isTrue(activePool_RawEther_before.eq(activePool_ONE_before))
      ...
      const activePool_ONE_After = await activePool.getONE()
      const activePool_RawEther_After = toBN(await web3.eth.getBalance(activePool.address))
      assert.isTrue(activePool_ONE_After.eq(dennisColl))
    ...
    it("closeTrove(): reduces ActivePool debt by correct amount", async () => {
      ...
      // Check after
      const activePool_Debt_After = (await activePool.get1USDDebt()).toString()
    ...
    if (!withProxy) { // TODO: wrap web3.eth.getBalance to be able to go through proxies
      it("closeTrove(): sends the correct amount of ONEto the user", async () => {
        await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })
        await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
        ...
        const alice_ONEBalance_Before = web3.utils.toBN(await web3.eth.getBalance(alice))
        ...
        const alice_ONEBalance_After = web3.utils.toBN(await web3.eth.getBalance(alice))
        const balanceDiff = alice_ONEBalance_After.sub(alice_ONEBalance_Before)
    ...
    it("closeTrove(): subtracts the debt of the closed Trove from the Borrower'sONEUSDToken balance", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      assert.isTrue(alice_1USDBalance_Before.gt(toBN('0')))

      // close trove
      await borrowerOperations.closeTrove({ from: alice })

      // check alice 1USDbalance after
      const alice_1USDBalance_After = await oneusdToken.balanceOf(alice)
      th.assertIsApproximatelyEqual(alice_1USDBalance_After, alice_1USDBalance_Before.sub(aliceDebt.sub(ONEUSD_GAS_COMPENSATION)))
    })
    ...
    it("closeTrove(): applies pending rewards", async () => {
      ...
      await openTrove({ extra1USDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
      ...
      const defaultPool_ONE = await defaultPool.getONE()
      const defaultPool_1USDDebt = await defaultPool.get1USDDebt()

      // Carol's liquidated coll (1 ONE) and drawn debt should have entered the Default Pool
      assert.isAtMost(th.getDifference(defaultPool_ONE, liquidatedColl_C), 100)
      assert.isAtMost(th.getDifference(defaultPool_1USDDebt, liquidatedDebt_C), 100)
      ...
      const defaultPool_ONE_afterAliceCloses = await defaultPool.getONE()
      const defaultPool_1USDDebt_afterAliceCloses = await defaultPool.get1USDDebt()

      assert.isAtMost(th.getDifference(defaultPool_ONE_afterAliceCloses,
        defaultPool_ONE.sub(pendingCollReward_A)), 1000)
      assert.isAtMost(th.getDifference(defaultPool_1USDDebt_afterAliceCloses,
        defaultPool_1USDDebt.sub(pendingDebtReward_A)), 1000)
      ...
      const defaultPool_ONE_afterBobCloses = await defaultPool.getONE()
      const defaultPool_1USDDebt_afterBobCloses = await defaultPool.get1USDDebt()

      assert.isAtMost(th.getDifference(defaultPool_ONE_afterBobCloses, 0), 100000)
      assert.isAtMost(th.getDifference(defaultPool_1USDDebt_afterBobCloses, 0), 100000)
    })

    it("closeTrove(): reverts if borrower has insufficient 1USDbalance to repay his entire debt", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extra1USDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })

      //Confirm Bob's 1USDbalance is less than his trove debt
      const B_1USDBal = await oneusdToken.balanceOf(B)
      const B_troveDebt = await getTroveEntireDebt(B)

      assert.isTrue(B_1USDBal.lt(B_troveDebt))

      const closeTrovePromise_B = borrowerOperations.closeTrove({ from: B })

      // Check closing trove reverts
      await assertRevert(closeTrovePromise_B, "BorrowerOps: Caller doesnt have enough 1USDto make repayment")
    })
    ...
    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("openTrove(): emits a TroveUpdated event with the correct collateral and debt", async () => {
        const txA = (await openTrove({ extra1USDAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })).tx
        const txB = (await openTrove({ extra1USDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })).tx
        const txC = (await openTrove({ extra1USDAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })).tx
        ...
        const txD = (await openTrove({ extra1USDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })).tx
        const txE = (await openTrove({ extra1USDAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })).tx
    ...
    it("openTrove(): decays a non-zero base rate", async () => {
      ...
      // E opens trove 
      await openTrove({ extra1USDAmount: toBN(dec(12, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
    ...
    it("openTrove(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
      ...
      await openTrove({ extra1USDAmount: toBN(dec(1, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      ...
      await openTrove({ extra1USDAmount: toBN(dec(1, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
    ...
    it("openTrove(): borrowing at non-zero base rate sends 1USDfee to LQTY staking contract", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 LQTY
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await lqtyToken.approve(lqtyStaking.address, dec(1, 18), { from: multisig })
      await lqtyStaking.stake(dec(1, 18), { from: multisig })
    ...
    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("openTrove(): borrowing at non-zero base records the (drawn debt + fee  + liq. reserve) on the Trove struct", async () => {
        ...
        const D_1USDRequest = toBN(dec(20000, 18))

        // D withdraws 1USD
        const openTroveTx = await borrowerOperations.openTrove(th._100pct, D_1USDRequest, ZERO_ADDRESS, ZERO_ADDRESS, { from: D, value: dec(200, 'ether') })

        const emittedFee = toBN(th.get1USDFeeFrom1USDBorrowingEvent(openTroveTx))
        assert.isTrue(toBN(emittedFee).gt(toBN('0')))

        const newDebt = (await troveManager.Troves(D))[0]

        // Check debt on Trove struct equals drawn debt plus emitted fee
        th.assertIsApproximatelyEqual(newDebt, D_1USDRequest.add(emittedFee).add(ONEUSD_GAS_COMPENSATION), 100000)
      })
    }

    it("openTrove(): Borrowing at non-zero base rate increases the LQTY staking contract 1USDfees-per-unit-staked", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 LQTY
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await lqtyToken.approve(lqtyStaking.address, dec(1, 18), { from: multisig })
      await lqtyStaking.stake(dec(1, 18), { from: multisig })
    ...
    it("openTrove(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      ...
      // D opens trove 
      const ONEUSDRequest_D = toBN(dec(40000, 18))
      await borrowerOperations.openTrove(th._100pct, ONEUSDRequest_D, D, D, { from: D, value: dec(500, 'ether') })
      ...
      // Check D's 1USDbalance now equals their requestedONEUSD
      const ONEUSDBalance_D = await oneusdToken.balanceOf(D)
      assert.isTrue(ONEUSDRequest_D.eq(ONEUSDBalance_D))
    })

    it("openTrove(): Borrowing at zero base rate changes the LQTY staking contract 1USDfees-per-unit-staked", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extra1USDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extra1USDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      // Check 1USDreward per LQTY staked == 0
      const F_1USD_Before = await lqtyStaking.F_1USD()
      assert.equal(F_1USD_Before, '0')
      ...
      // Check 1USDreward per LQTY staked > 0
      const F_1USD_After = await lqtyStaking.F_1USD()
      assert.isTrue(F_1USD_After.gt(toBN('0')))
    })
    ...
    it("openTrove(): Borrowing at zero base rate charges minimum fee", async () => {
      ...
      const ONEUSDRequest = toBN(dec(10000, 18))
      const txC = await borrowerOperations.openTrove(th._100pct, ONEUSDRequest, ZERO_ADDRESS, ZERO_ADDRESS, { value: dec(100, 'ether'), from: C })
      const _1USDFee = toBN(th.getEventArgByName(txC, "ONEUSDBorrowingFeePaid", "_1USDFee"))

      const expectedFee = BORROWING_FEE_FLOOR.mul(toBN(ONEUSDRequest)).div(toBN(dec(1, 18)))
      assert.isTrue(_1USDFee.eq(expectedFee))
    })

    it("openTrove(): reverts when system is in Recovery Mode and ICR < CCR", async () => {
      await openTrove({ extra1USDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
      await openTrove({ extra1USDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      assert.isFalse(await th.checkRecoveryMode(contracts))
      ...
      try {
        const txBob = await openTrove({ extra1USDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(149, 16)), extraParams: { from: alice } })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("openTrove(): reverts when trove ICR < MCR", async () => {
      ...
      try {
        const txBob = (await openTrove({ extra1USDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(109, 16)), extraParams: { from: bob } })).tx
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
      ...
      try {
        const txBob = await openTrove({ extra1USDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(109, 16)), extraParams: { from: bob } })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("openTrove(): reverts when opening the trove would cause the TCR of the system to fall below the CCR", async () => {
      ...
      await openTrove({ extra1USDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(15, 17)), extraParams: { from: alice } })
      ...
      try {
        const txBob = await openTrove({ extra1USDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(149, 16)), extraParams: { from: bob } })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("openTrove(): reverts if trove is already active", async () => {
      ...
      await openTrove({ extra1USDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(15, 17)), extraParams: { from: alice } })
      await openTrove({ extra1USDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(15, 17)), extraParams: { from: bob } })

      try {
        const txB_1 = await openTrove({ extra1USDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(3, 18)), extraParams: { from: bob } })
    ...
    it("openTrove(): Can open a trove with ICR >= CCR when system is in Recovery Mode", async () => {
      ...
      // price drops to 1ONE:100 1USD, reducing TCR below 150%
      await priceFeed.setPrice('100000000000000000000');
      const price = await priceFeed.getPrice()
      ...
      const txCarol = (await openTrove({ extra1USDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(15, 17)), extraParams: { from: carol } })).tx
    ...
    it("openTrove(): creates a new Trove and assigns the correct collateral and debt amount", async () => {
      ...
      const ONEUSDRequest = MIN_NET_DEBT
      borrowerOperations.openTrove(th._100pct, MIN_NET_DEBT, carol, carol, { from: alice, value: dec(100, 'ether') })

      // Get the expected debt based on the 1USDrequest (adding fee and liq. reserve on top)
      const expectedDebt = ONEUSDRequest
        .add(await troveManager.getBorrowingFee(ONEUSDRequest))
        .add(ONEUSD_GAS_COMPENSATION)
    ...
    it("openTrove(): Increases the activePool ONEand raw ether balance by correct amount", async () => {
      const activePool_ONE_Before = await activePool.getONE()
      const activePool_RawEther_Before = await web3.eth.getBalance(activePool.address)
      assert.equal(activePool_ONE_Before, 0)
      ...
      const activePool_ONE_After = await activePool.getONE()
      const activePool_RawEther_After = toBN(await web3.eth.getBalance(activePool.address))
      assert.isTrue(activePool_ONE_After.eq(aliceCollAfter))
      assert.isTrue(activePool_RawEther_After.eq(aliceCollAfter))
    })

    it("openTrove(): records up-to-date initial snapshots of L_ONE and L_1USDDebt", async () => {
      // --- SETUP ---
      ...
      /* with total stakes = 10 ether, after liquidation, L_ONE should equal 1/10 ether per-ether-staked,
       and L_1USD should equal 18 1USDper-ether-staked. */

      const L_ONE = await troveManager.L_ONE()
      const L_1USD = await troveManager.L_1USDDebt()

      assert.isTrue(L_ONE.gt(toBN('0')))
      assert.isTrue(L_1USD.gt(toBN('0')))

      // Bob opens trove
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })

      // Check Bob's snapshots of L_ONE and L_1USD equal the respective current values
      const bob_rewardSnapshot = await troveManager.rewardSnapshots(bob)
      const bob_ONErewardSnapshot = bob_rewardSnapshot[0]
      const bob_1USDDebtRewardSnapshot = bob_rewardSnapshot[1]

      assert.isAtMost(th.getDifference(bob_ONErewardSnapshot, L_ONE), 1000)
      assert.isAtMost(th.getDifference(bob_1USDDebtRewardSnapshot, L_1USD), 1000)
    })

    it("openTrove(): allows a user to open a Trove, then close it, then re-open it", async () => {
      // Open Troves
      await openTrove({ extra1USDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
      await openTrove({ extra1USDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extra1USDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
    ...
    it("openTrove(): increases the Trove's 1USDdebt by the correct amount", async () => {
      // check before
      const alice_Trove_Before = await troveManager.Troves(alice)
      const debt_Before = alice_Trove_Before[0]
      assert.equal(debt_Before, 0)

      await borrowerOperations.openTrove(th._100pct, await getOpenTrove1USDAmount(dec(10000, 18)), alice, alice, { from: alice, value: dec(100, 'ether') })
    ...
    it("openTrove(): increases 1USDdebt in ActivePool by the debt of the trove", async () => {
      const activePool_1USDDebt_Before = await activePool.get1USDDebt()
      assert.equal(activePool_1USDDebt_Before, 0)
      ...
      const activePool_1USDDebt_After = await activePool.get1USDDebt()
      assert.isTrue(activePool_1USDDebt_After.eq(aliceDebt))
    })

    it("openTrove(): increases userONEUSDToken balance by correct amount", async () => {
      // check before
      const alice_1USDTokenBalance_Before = await oneusdToken.balanceOf(alice)
      assert.equal(alice_1USDTokenBalance_Before, 0)

      await borrowerOperations.openTrove(th._100pct, dec(10000, 18), alice, alice, { from: alice, value: dec(100, 'ether') })

      // check after
      const alice_1USDTokenBalance_After = await oneusdToken.balanceOf(alice)
      assert.equal(alice_1USDTokenBalance_After, dec(10000, 18))
    })
    ...
    it("getCompositeDebt(): returns debt + gas comp", async () => {
      ...
      assert.equal(res1, ONEUSD_GAS_COMPENSATION.toString())

      const res2 = await borrowerOperations.getCompositeDebt(dec(90, 18))
      th.assertIsApproximatelyEqual(res2, ONEUSD_GAS_COMPENSATION.add(toBN(dec(90, 18))))

      const res3 = await borrowerOperations.getCompositeDebt(dec(24423422357345049, 12))
      th.assertIsApproximatelyEqual(res3, ONEUSD_GAS_COMPENSATION.add(toBN(dec(24423422357345049, 12))))
    })
    ...
    describe("getNewTCRFromTroveChange() returns the correct TCR", async () => {
      ...
      it("collChange = 0, debtChange = 0", async () => {
        ...
        const trove1USDAmount = await getOpenTrove1USDAmount(troveTotalDebt)
        await borrowerOperations.openTrove(th._100pct, trove1USDAmount, alice, alice, { from: alice, value: troveColl })
        await borrowerOperations.openTrove(th._100pct, trove1USDAmount, bob, bob, { from: bob, value: troveColl })
    ...
    if (!withProxy) {
      it('closeTrove(): fails if owner cannot receive ONE', async () => {
        ...
        // Alice sends 1USDto NonPayable so its 1USDbalance covers its debt
        await oneusdToken.transfer(nonPayable.address, dec(10000, 18), { from: alice })
        ...
        await th.assertRevert(nonPayable.forward(borrowerOperations.address, closeTroveData), 'ActivePool: sending ONEfailed')
      })
    }
  }
...
/* TODO:
 ...
 changes with addColl, withdrawColl, withdraw1USD, repay1USD, etc. Can split them up and put them with
