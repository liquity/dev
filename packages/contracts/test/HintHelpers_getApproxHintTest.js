...
contract('HintHelpers', async accounts => {
  ...
  /* Open a Trove for each account. ONEUSD debt is 200 ONEUSD each, with collateral beginning at
  1.5 ether, and rising by 0.01 ether per Trove.  Hence, the ICR of account (i + 1) is always 1% greater than the ICR of account i. 
 */

  // Open Troves in parallel, then withdraw ONEUSD in parallel
  const makeTrovesInParallel = async (accounts, n) => {
    ...
    const withdraw1USDpromises = activeAccounts.map(account => withdraw1USDfromTrove(account))
    await Promise.all(withdraw1USDpromises)
  ...
  const withdraw1USDfromTrove = async (account) => {
    await borrowerOperations.withdraw1USD(th._100pct, '100000000000000000000', account, account, { from: account })
  }

  // Sequentially add coll and withdraw 1USD, 1 account at a time
  const makeTrovesInSequence = async (accounts, n) => {
    ...
    for (const account of activeAccounts) {
      ...
      await th.openTrove(contracts, { extra1USDAmount: toBN(dec(10000, 18)), ICR: ICR_BN, extraParams: { from: account } })
