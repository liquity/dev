...
const deployStabilio = deploymentHelpers.deployStabilio
...
contract('Pool Manager: Sum-Product rounding errors', async accounts => {
  ...
  beforeEach(async () => {
    contracts = await deployStabilio()
  ...
  it.skip("Rounding errors: 100 deposits of 100XBRL into SP, then 200 liquidations of 49XBRL", async () => {
    ...
    const SP_TotalDeposits = await stabilityPool.getTotalXBRLDeposits()
    const SP_ETH = await stabilityPool.getETH()
    const compoundedDeposit = await stabilityPool.getCompoundedXBRLDeposit(depositors[0])
