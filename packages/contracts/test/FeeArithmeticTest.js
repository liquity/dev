...
contract('Fee arithmetic tests', async accounts => {
  ...
  before(async () => {
    ...
    aggregatorTester = await AggregatorTester.new()
    AggregatorTester.setAsDeployed(aggregatorTester)
  ...
  it("minutesPassedSinceLastFeeOp(): returns minutes passed for no time increase", async () => {
    await aggregatorTester.setLastFeeOpTimeToNow()
    const minutesPassed = await aggregatorTester.minutesPassedSinceLastFeeOp()
  ...
  it("decayBaseRateFromBorrowing(): returns the initial base rate for no time increase", async () => {
    await aggregatorTester.setBaseRate(dec(5, 17))
    await aggregatorTester.setLastFeeOpTimeToNow()

    const baseRateBefore = await aggregatorTester.baseRate()
    assert.equal(baseRateBefore, dec(5, 17))

    await aggregatorTester.unprotectedDecayBaseRateFromBorrowing()
    const baseRateAfter = await aggregatorTester.baseRate()
  ...
  it("decayBaseRateFromBorrowing(): returns the initial base rate for less than one minute passed ", async () => {
    ...
    // 1 second
    const baseRateBefore_1 = await aggregatorTester.baseRate()
    assert.equal(baseRateBefore_1, dec(5, 17))

    await th.fastForwardTime(1, web3.currentProvider)

    await aggregatorTester.unprotectedDecayBaseRateFromBorrowing()
    const baseRateAfter_1 = await aggregatorTester.baseRate()
    ...
    const baseRateBefore_2 = await aggregatorTester.baseRate()
    await th.fastForwardTime(17, web3.currentProvider)

    await aggregatorTester.unprotectedDecayBaseRateFromBorrowing()
    const baseRateAfter_2 = await aggregatorTester.baseRate()
    ...
    const baseRateBefore_3 = await aggregatorTester.baseRate()
    await th.fastForwardTime(29, web3.currentProvider)

    await aggregatorTester.unprotectedDecayBaseRateFromBorrowing()
    const baseRateAfter_3 = await aggregatorTester.baseRate()
    ...
    const baseRateBefore_4 = await aggregatorTester.baseRate()
    await th.fastForwardTime(50, web3.currentProvider)

    await aggregatorTester.unprotectedDecayBaseRateFromBorrowing()
    const baseRateAfter_4 = await aggregatorTester.baseRate()
  ...
  it("decayBaseRateFromBorrowing(): returns correctly decayed base rate, for various durations. Initial baseRate = 0.01", async () => {
    ...
    for (i = 0; i < decayBaseRateResults.seconds.length; i++) {
      ...
      await aggregatorTester.setBaseRate(dec(1, 16))
      const contractBaseRate = await aggregatorTester.baseRate()
      ...
      await aggregatorTester.unprotectedDecayBaseRateFromBorrowing()
      const decayedBaseRate = await aggregatorTester.baseRate()
  ...
  it("decayBaseRateFromBorrowing(): returns correctly decayed base rate, for various durations. Initial baseRate = 0.1", async () => {
    ...
    for (i = 0; i < decayBaseRateResults.seconds.length; i++) {
      ...
      await aggregatorTester.setBaseRate(dec(1, 17))
      const contractBaseRate = await aggregatorTester.baseRate()
  ...
  it("decayBaseRateFromBorrowing(): returns correctly decayed base rate, for various durations. Initial baseRate = 0.34539284", async () => {
    ...
    for (i = 0; i < decayBaseRateResults.seconds.length; i++) {
      // Set base rate to 0.1 in TroveManager
      await aggregatorTester.setBaseRate('345392840000000000')
      const contractBaseRate = await aggregatorTester.baseRate()
      await aggregatorTester.setBaseRate('345392840000000000')
  ...
  it("decayBaseRateFromBorrowing(): returns correctly decayed base rate, for various durations. Initial baseRate = 0.9976", async () => {
    ...
    for (i = 0; i < decayBaseRateResults.seconds.length; i++) {
      ...
      await aggregatorTester.setBaseRate('997600000000000000')
      await aggregatorTester.setBaseRate('997600000000000000')
