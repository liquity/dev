...
contract('During the initial lockup period', async accounts => {
  ...
  describe('LQTY transfer during first year after LQTY deployment', async accounts => {
    ...
    it("LQTY deployer can not transfer LQTY to a LC that was deployed directly", async () => {
      // LQTY deployer deploys LC_A
      ...
      // LQTY deployer attempts LQTY transfer to LC_A
      try {
        const LQTYtransferTx_A = await lqtyToken.transfer(LC_A.address, dec(1, 18), { from: liquityAG })
      ...
      // LQTY deployer attempts LQTY transfer to LC_B
      try {
        const LQTYtransferTx_B = await lqtyToken.transfer(LC_B.address, dec(1, 18), { from: liquityAG })
    ...
    it("LQTY deployer can not transfer to an EOA", async () => {
      ...
      for (const contract of Object.keys(coreContracts)) {
        ...
        await assertRevert(LQTYtransferTxPromise, "LQTYToken: caller must not be the deployer")
      }

      // Deployer attempts LQTY transfer to LQTY contracts (excluding LCs)
    ...
    it("LQTY deployer can not approve any EOA or Liquity contract to spend their LQTY", async () => {
      ...
      await assertRevert(LQTYApproveTxPromise_1, "LQTYToken: caller must not be the deployer")
      await assertRevert(LQTYApproveTxPromise_2, "LQTYToken: caller must not be the deployer")
      ...
      for (const contract of Object.keys(coreContracts)) {
        ...
        await assertRevert(LQTYApproveTxPromise, "LQTYToken: caller must not be the deployer")
      }

      // Deployer attempts to approve LQTY contracts to spend LQTY (excluding LCs)
      for (const contract of Object.keys(LQTYContracts)) {
    ...
    it("LQTY deployer can not increaseAllowance for any EOA or Liquity contract", async () => {
      ...
      await assertRevert(LQTYIncreaseAllowanceTxPromise_1, "LQTYToken: caller must not be the deployer")
      await assertRevert(LQTYIncreaseAllowanceTxPromise_2, "LQTYToken: caller must not be the deployer")

      // Deployer attempts to approve Liquity contracts to spend LQTY
      for (const contract of Object.keys(coreContracts)) {
        const LQTYIncreaseAllowanceTxPromise = lqtyToken.increaseAllowance(coreContracts[contract].address, dec(1, 18), { from: liquityAG })
        await assertRevert(LQTYIncreaseAllowanceTxPromise, "LQTYToken: caller must not be the deployer")
      }
    ...
    it("LQTY deployer can not decreaseAllowance for any EOA or Liquity contract", async () => {
      ...
      await assertRevert(LQTYDecreaseAllowanceTxPromise_1, "LQTYToken: caller must not be the deployer")
      await assertRevert(LQTYDecreaseAllowanceTxPromise_2, "LQTYToken: caller must not be the deployer")
      ...
      for (const contract of Object.keys(coreContracts)) {
        ...
        await assertRevert(LQTYDecreaseAllowanceTxPromise, "LQTYToken: caller must not be the deployer")
      }

      // Deployer attempts to decrease allowance on LQTY contracts (excluding LCs)
      for (const contract of Object.keys(LQTYContracts)) {
    ...
    it("LQTY deployer can not be the sender in a transferFrom() call", async () => {
      ...
      await assertRevert(LQTYtransferFromTxPromise_1, "LQTYToken: caller must not be the deployer")
      await assertRevert(LQTYtransferFromTxPromise_2, "LQTYToken: caller must not be the deployer")
    })
    ...
    it("LQTY deployer can not stake their LQTY in the staking contract", async () => {
      ...
      await assertRevert(LQTYStakingTxPromise_1, "LQTYToken: caller must not be the deployer")
    })

    // --- Anyone else ---

    it("Anyone (other than Liquity AG) can transfer LQTY to LCs deployed by anyone through the Factory", async () => {
    ...
    it("Anyone (other than Liquity AG) can transfer LQTY to LCs deployed by anyone directly", async () => {
      // Start D, E, F with some LQTY
      await lqtyToken.unprotectedMint(D, dec(1, 24))
      await lqtyToken.unprotectedMint(E, dec(2, 24))
      await lqtyToken.unprotectedMint(F, dec(3, 24))
  ...
  describe('Transferring LQTY to LCs', async accounts => {
    it("LQTY deployer can transfer LQTY (vesting) to lockup contracts they deployed", async () => {
      ...
      await lqtyToken.transfer(LC_T1.address, dec(1, 24), { from: liquityAG })
      await lqtyToken.transfer(LC_T2.address, dec(1, 24), { from: liquityAG })
      await lqtyToken.transfer(LC_T3.address, dec(1, 24), { from: liquityAG })
    ...
    it("LQTY deployer can transfer LQTY to lockup contracts deployed by anyone", async () => {
      ...
      // LQTY deployer transfers LQTY to LCs deployed by other accounts
      await lqtyToken.transfer(LC_A.address, dec(1, 24), { from: liquityAG })
      await lqtyToken.transfer(LC_B.address, dec(2, 24), { from: liquityAG })
      await lqtyToken.transfer(LC_C.address, dec(3, 24), { from: liquityAG })
  ...
  describe('Deploying new LCs', async accounts => {
    ...
    it("Anyone can deploy LCs with unlockTime = one year from deployment, directly and through factory", async () => {
      ...
      const LCDeploymentTx_3 = await lockupContractFactory.deployLockupContract(A, oneYearFromSystemDeployment, { from: E })
      const LCDeploymentTx_4 = await lockupContractFactory.deployLockupContract(C, twoYearsFromSystemDeployment, { from: liquityAG })
    ...
    it("Anyone can deploy LCs with unlockTime > one year from deployment, directly and through factory", async () => {
      ...
      const LC_2 = await LockupContract.new(lqtyToken.address, B, justOverOneYear, { from: liquityAG })
      const LCTxReceipt_2 = await web3.eth.getTransactionReceipt(LC_2.transactionHash)
    ...
    it("No one can deploy LCs with unlockTime < one year from deployment, directly or through factory", async () => {
      ...
      const directDeploymentTxPromise_2 = LockupContract.new(lqtyToken.address, B, '43200', { from: liquityAG })
      ...
      const factoryDploymentTxPromise_2 = lockupContractFactory.deployLockupContract(C, '43200', { from: liquityAG })
    ...
    describe('Withdrawal Attempts on LCs before unlockTime has passed ', async accounts => {
      it("LQTY Deployer can't withdraw from a funded LC they deployed for another beneficiary through the Factory before the unlockTime", async () => {
        ...
        // LQTY deployer attempts withdrawal from LC they deployed through the Factory
        try {
          const withdrawalAttempt = await LC_T1.withdrawLQTY({ from: liquityAG })
      ...
      it("LQTY Deployer can't withdraw from a funded LC that someone else deployed before the unlockTime", async () => {
        ...
        //LQTY deployer fund the newly deployed LCs
        await lqtyToken.transfer(LC_B.address, dec(2, 18), { from: liquityAG })
        ...
        // LQTY deployer attempts withdrawal from LCs
        try {
          const withdrawalAttempt_B = await LC_B.withdrawLQTY({ from: liquityAG })
