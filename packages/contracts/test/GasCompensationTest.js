...
contract('Gas compensation tests', async accounts => {
  ...
  it('_getCollGasCompensation(): returns the 0.5% of collaterall if it is < $10 in value', async () => {
    ...
    /* 
    ONE:USD price = 1000000000 (1 billion)
    coll = 0.000000005 ONE (5e9 wei): $5 in value 
    -> Expect 0.5% of collaterall as gas compensation */
  ...
  it('_getCollGasCompensation(): returns 0.5% of collaterall when 0.5% of collateral < $10 in value', async () => {
    ...
    ONE:USD price = 200
    coll = 9.999 ONE  
    0.5% of coll = 0.04995 ONE. USD value: $9.99
    -> Expect 0.5% of collaterall as gas compensation */
    ...
    /* ONE:USD price = 200
     coll = 0.055 ONE  
     0.5% of coll = 0.000275 ONE. USD value: $0.055
     -> Expect 0.5% of collaterall as gas compensation */
    ...
    /* ONE:USD price = 200
    coll = 6.09232408808723580 ONE  
    0.5% of coll = 0.004995 ONE. USD value: $6.09
    -> Expect 0.5% of collaterall as gas compensation */
  ...
  it('getCollGasCompensation(): returns 0.5% of collaterall when 0.5% of collateral = $10 in value', async () => {
    ...
    ONE:USD price = 200
    coll = 10 ONE  
    0.5% of coll = 0.5 ONE. USD value: $10
    -> Expect 0.5% of collaterall as gas compensation */
  ...
  it('getCollGasCompensation(): returns 0.5% of collaterall when 0.5% of collateral = $10 in value', async () => {
    ...
    ONE:USD price = 200 $/E
    coll = 100 ONE  
    0.5% of coll = 0.5 ONE. USD value: $100
    -> Expect $100 gas compensation, i.e. 0.5 ONE */
    ...
    ONE:USD price = 200 $/E
    coll = 10.001 ONE  
    0.5% of coll = 0.050005 ONE. USD value: $10.001
    -> Expect $100 gas compensation, i.e.  0.050005  ONE */
    ...
    ONE:USD price = 200 $/E
    coll = 37.5 ONE  
    0.5% of coll = 0.1875 ONE. USD value: $37.5
    -> Expect $37.5 gas compensation i.e.  0.1875  ONE */
    ...
    /* 
    ONE:USD price = 45323.54542 $/E
    coll = 94758.230582309850 ONE  
    0.5% of coll = 473.7911529 ONE. USD value: $21473894.84
    -> Expect $21473894.8385808 gas compensation, i.e.  473.7911529115490  ONE */
    ...
    /* 
    ONE:USD price = 1000000 $/E (1 million)
    coll = 300000000 ONE   (300 million)
    0.5% of coll = 1500000 ONE. USD value: $150000000000
    -> Expect $150000000000 gas compensation, i.e. 1500000 ONE */
  ...
  it('_getCompositeDebt(): returns (debt + 50) when collateral < $10 in value', async () => {
    ...
    debt = 10 1USD
    0.5% of coll = 0.04995 ONE. USD value: $9.99
    -> Expect composite debt = 10 + 200  = 2100 1USD*/
    ...
    /* ONE:USD price = 200
     ...
     debt = 0 1USD
     0.5% of coll = 0.000275 ONE. USD value: $0.055
     -> Expect composite debt = 0 + 200 = 200 1USD*/
    ...
    // debt = 200 1USD 
    // 0.5% of coll = 0.004995 ONE. USD value: $6.09
    // -> Expect  composite debt =  200 + 200 = 400  1USD */
  ...
  // returns $10 worth of ONE when 0.5% of coll == $10
  it('getCompositeDebt(): returns (debt + 50) collateral = $10 in value', async () => {
    ...
    debt = 123.45 1USD
    0.5% of coll = 0.5 ONE. USD value: $10
    -> Expect composite debt = (123.45 + 200) = 323.45 1USD  */
  ...
  it('getCompositeDebt(): returns (debt + 50) when 0.5% of collateral > $10 in value', async () => {
    ...
    coll = 100 ONE  
    debt = 2000 1USD
    -> Expect composite debt = (2000 + 200) = 2200 1USD  */
    ...
    coll = 37.5 ONE  
    debt = 500 1USD
    -> Expect composite debt = (500 + 200) = 700 1USD  */
    ...
    coll = 94758.230582309850 ONE  
    debt = 1 billion 1USD
    -> Expect composite debt = (1000000000 + 200) = 1000000200 1USD  */
    await priceFeed.setPrice('45323545420000000000000')
    const price_2 = await priceFeed.getPrice()
    ...
    ONE:USD price = 1000000 $/E (1 million)
    coll = 300000000 ONE   (300 million)
    debt = 54321.123456789 1USD
   -> Expect composite debt = (54321.123456789 + 200) = 54521.123456789 1USD */
    await priceFeed.setPrice(dec(1, 24))
    const price_3 = await priceFeed.getPrice()
  ...
  it('getCurrentICR(): Incorporates virtual debt, and returns the correct ICR for new troves', async () => {
    ...
    // F opens with 1 ONE, 100 1USD
    await openTrove({ ICR: toBN(dec(2, 18)), extra1USDAmount: dec(100, 18), extraParams: { from: flyn } })
    ...
    // E opens with 4405.45 ONE, 32598.35 1USD
  ...
  it('Gas compensation from pool-offset liquidations. All collateral paid as compensation', async () => {
    ...
    // A-E open troves
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extra1USDAmount: dec(100, 18), extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extra1USDAmount: dec(200, 18), extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extra1USDAmount: dec(300, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(2, 18)), extra1USDAmount: A_totalDebt, extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(2, 18)), extra1USDAmount: B_totalDebt.add(C_totalDebt), extraParams: { from: erin } })

    // D, E each provide 1USD to SP
    ...
    const ONEUSDinSP_0 = await stabilityPool.getTotal1USDDeposits()
    ...
    // Check liquidator's balance increases by 0.5% of A's coll (1 ONE)
    ...
    // Check SP 1USD has decreased due to the liquidation 
    const ONEUSDinSP_A = await stabilityPool.getTotal1USDDeposits()
    assert.isTrue(ONEUSDinSP_A.lte(ONEUSDinSP_0))

    // Check ONE in SP has received the liquidation
    const ONEinSP_A = await stabilityPool.getONE()
    assert.equal(ONEinSP_A.toString(), aliceColl.sub(_0pt5percent_aliceColl)) // 1 ONE - 0.5%
    ...
    // Check liquidator's balance increases by B's 0.5% of coll, 2 ONE
    ...
    assert.equal(compensationReceived_B, _0pt5percent_bobColl) // 0.5% of 2 ONE

    // Check SP ONEUSD has decreased due to the liquidation of B
    const ONEUSDinSP_B = await stabilityPool.getTotal1USDDeposits()
    assert.isTrue(ONEUSDinSP_B.lt(ONEUSDinSP_A))

    // Check ONE in SP has received the liquidation
    const ONEinSP_B = await stabilityPool.getONE()
    assert.equal(ONEinSP_B.toString(), aliceColl.sub(_0pt5percent_aliceColl).add(bobColl).sub(_0pt5percent_bobColl)) // (1 + 2 ONE) * 0.995
    ...
    /* 
    ONE:USD price = 3.141592653589793238
    Carol coll = 3 ONE. Value = (3 * 3.141592653589793238) = $6
    -> Expect 0.5% of collaterall to be sent to liquidator, as gas compensation */
    ...
    // Check liquidator's balance increases by C's 0.5% of coll, 3 ONE
    ...
    // Check SP 1USD has decreased due to the liquidation of C
    const ONEUSDinSP_C = await stabilityPool.getTotal1USDDeposits()
    assert.isTrue(ONEUSDinSP_C.lt(ONEUSDinSP_B))

    // Check ONE in SP has not changed due to the lquidation of C
    const ONEinSP_C = await stabilityPool.getONE()
    assert.equal(ONEinSP_C.toString(), aliceColl.sub(_0pt5percent_aliceColl).add(bobColl).sub(_0pt5percent_bobColl).add(carolColl).sub(_0pt5percent_carolColl)) // (1+2+3 ONE) * 0.995
  })
  ...
  it('gas compensation from pool-offset liquidations: 0.5% collateral < $10 in value. Compensates $10 worth of collateral, liquidates the remainder', async () => {
    ...
    // A-E open troves
    await openTrove({ ICR: toBN(dec(2, 18)), extra1USDAmount: dec(200, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(120, 16)), extra1USDAmount: dec(5000, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(60, 18)), extra1USDAmount: dec(600, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(80, 18)), extra1USDAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(80, 18)), extra1USDAmount: dec(1, 23), extraParams: { from: erin } })

    // D, E each provide 10000 1USD to SP
    ...
    const ONEUSDinSP_0 = await stabilityPool.getTotal1USDDeposits()
    const ONEinSP_0 = await stabilityPool.getONE()
    ...
    /* 
    ONE:USD price = 199.999
    Alice coll = 1 ONE. Value: $199.999
    0.5% of coll  = 0.05 ONE. Value: (0.05 * 199.999) = $9.99995
    Minimum comp = $10 = 0.05000025000125001 ONE.
    -> Expect 0.05000025000125001 ONE sent to liquidator, 
    and (1 - 0.05000025000125001) = 0.94999974999875 ONE remainder liquidated */
    ...
    // Check SP 1USD has decreased due to the liquidation of A
    const ONEUSDinSP_A = await stabilityPool.getTotal1USDDeposits()
    assert.isTrue(ONEUSDinSP_A.lt(ONEUSDinSP_0))

    // Check ONE in SP has increased by the remainder of B's coll
    const collRemainder_A = aliceColl.sub(_0pt5percent_aliceColl)
    const ONEinSP_A = await stabilityPool.getONE()

    const SPONEIncrease_A = ONEinSP_A.sub(ONEinSP_0)

    assert.isAtMost(th.getDifference(SPONEIncrease_A, collRemainder_A), 1000)
    ...
    /* 
    ONE:USD price = 15
    Bob coll = 15 ONE. Value: $165
    0.5% of coll  = 0.75 ONE. Value: (0.75 * 11) = $8.25
    Minimum comp = $10 =  0.66666...ONE.
    -> Expect 0.666666666666666666 ONE sent to liquidator, 
    and (15 - 0.666666666666666666) ONE remainder liquidated */
    ...
    // Check SP 1USD has decreased due to the liquidation of B
    const ONEUSDinSP_B = await stabilityPool.getTotal1USDDeposits()
    assert.isTrue(ONEUSDinSP_B.lt(ONEUSDinSP_A))
    ...
    const SPONEIncrease_B = ONEinSP_B.sub(ONEinSP_A)

    assert.isAtMost(th.getDifference(SPONEIncrease_B, collRemainder_B), 1000)
  })
  ...
  it('gas compensation from pool-offset liquidations: 0.5% collateral > $10 in value. Compensates 0.5% of  collateral, liquidates the remainder', async () => {
    ...
    // A-E open troves
    await openTrove({ ICR: toBN(dec(2, 18)), extra1USDAmount: dec(2000, 18), extraParams: { from: alice, } })
    await openTrove({ ICR: toBN(dec(1875, 15)), extra1USDAmount: dec(8000, 18), extraParams: { from: bob, } })
    await openTrove({ ICR: toBN(dec(2, 18)), extra1USDAmount: dec(600, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(4, 18)), extra1USDAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(4, 18)), extra1USDAmount: dec(1, 23), extraParams: { from: erin } })
    ...
    /* 
    ONE:USD price = 200
    Alice coll = 10.001 ONE. Value: $2000.2
    0.5% of coll  = 0.050005 ONE. Value: (0.050005 * 200) = $10.01
    Minimum comp = $10 = 0.05 ONE.
    -> Expect  0.050005 ONE sent to liquidator, 
    and (10.001 - 0.050005) ONE remainder liquidated */
    ...
    // Check SP ONEUSD has decreased due to the liquidation of A 
    const ONEUSDinSP_A = await stabilityPool.getTotal1USDDeposits()
    assert.isTrue(ONEUSDinSP_A.lt(ONEUSDinSP_0))

    // Check ONE in SP has increased by the remainder of A's coll
    const collRemainder_A = aliceColl.sub(_0pt5percent_aliceColl)
    const ONEinSP_A = await stabilityPool.getONE()
    ...
    /* 
   ONE:USD price = 200
   Bob coll = 37.5 ONE. Value: $7500
   0.5% of coll  = 0.1875 ONE. Value: (0.1875 * 200) = $37.5
   Minimum comp = $10 = 0.05 ONE.
   -> Expect 0.1875 ONE sent to liquidator, 
   and (37.5 - 0.1875 ONE) ONE remainder liquidated */
  ...
  it('Gas compensation from pool-offset liquidations. Liquidation event emits the correct gas compensation and total liquidated coll and debt', async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extra1USDAmount: dec(100, 18), extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extra1USDAmount: dec(200, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(2, 18)), extra1USDAmount: dec(300, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(2, 18)), extra1USDAmount: A_totalDebt, extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(2, 18)), extra1USDAmount: B_totalDebt, extraParams: { from: erin } })
  ...
  it('liquidateTroves(): full offset.  Compensates the correct amount, and liquidates the remainder', async () => {
    ...
    // A-F open troves
    await openTrove({ ICR: toBN(dec(118, 16)), extra1USDAmount: dec(2000, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(526, 16)), extra1USDAmount: dec(8000, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(488, 16)), extra1USDAmount: dec(600, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(545, 16)), extra1USDAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(10, 18)), extra1USDAmount: dec(1, 23), extraParams: { from: erin } })
    await openTrove({ ICR: toBN(dec(10, 18)), extra1USDAmount: dec(1, 23), extraParams: { from: flyn } })
    ...
    */
    ...
    // Check 1USD in SP has decreased
    const ONEUSDinSP_1 = await stabilityPool.getTotal1USDDeposits()
    assert.isTrue(ONEUSDinSP_1.lt(ONEUSDinSP_0))
    ...
    // Check ONE in stability pool now equals the expected liquidated collateral
    const ONEinSP = (await stabilityPool.getONE()).toString()
    assert.equal(expectedLiquidatedColl, ONEinSP)
  })
  ...
  it('liquidateTroves(): full redistribution. Compensates the correct amount, and liquidates the remainder', async () => {
    ...
    const ONEUSDinDefaultPool_0 = await defaultPool.get1USDDebt()
    ...
    */
    ...
    // Check 1USD in DefaultPool has decreased
    const ONEUSDinDefaultPool_1 = await defaultPool.get1USDDebt()
    assert.isTrue(ONEUSDinDefaultPool_1.gt(ONEUSDinDefaultPool_0))
    ...
    // Check ONE in defaultPool now equals the expected liquidated collateral
    const ONEinDefaultPool = (await defaultPool.getONE()).toString()
    assert.isAtMost(th.getDifference(expectedLiquidatedColl, ONEinDefaultPool), 1000)
  })
  ...
  it('liquidateTroves(): full offset. Liquidation event emits the correct gas compensation and total liquidated coll and debt', async () => {
    ...
    // A-F open troves
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(118, 16)), extra1USDAmount: dec(2000, 18), extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(526, 16)), extra1USDAmount: dec(8000, 18), extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(488, 16)), extra1USDAmount: dec(600, 18), extraParams: { from: carol } })
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(545, 16)), extra1USDAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(10, 18)), extra1USDAmount: dec(1, 23), extraParams: { from: erin } })
    await openTrove({ ICR: toBN(dec(10, 18)), extra1USDAmount: dec(1, 23), extraParams: { from: flyn } })
    ...
    */
    ...
    // Expect liquidatedDebt = 51 + 190 + 1025 + 13510 = 14646 1USD
  ...
  it('Trove ordering: same collateral, decreasing debt. Price successively increases. Troves should maintain ordering by ICR', async () => {
    ...
    let debt = 50
    // create 10 troves, constant coll, descending debt 100 to 90 1USD
    for (const account of _10_accounts) {

      const debtString = debt.toString().concat('000000000000000000')
      await openTrove({ extra1USDAmount: debtString, extraParams: { from: account, value: dec(30, 'ether') } })
      ...
      for (account of _10_accounts) {
        ...
        if (ICRList.length > 1) {
          ...
          try {
            ...
            console.log(`ONE price at which trove ordering breaks: ${price}`)
            logICRs(ICRList)
          }
        }
  ...
  it('Trove ordering: increasing collateral, constant debt. Price successively increases. Troves should maintain ordering by ICR', async () => {
    ...
    let coll = 50
    // create 20 troves, increasing collateral, constant debt = 100 1USD
    for (const account of _20_accounts) {

      const collString = coll.toString().concat('000000000000000000')
      await openTrove({ extra1USDAmount: dec(100, 18), extraParams: { from: account, value: collString } })
