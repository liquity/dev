...
contract('StabilityPool', async accounts => {
  ...
  describe("Stability Pool Mechanisms", async () => {
    ...
    // --- provideToSP() ---
    // increases recorded XBRL at Stability Pool
    it("provideToSP(): increases the Stability Pool XBRL balance", async () => {
      // --- SETUP --- Give Alice a least 200
      await openTrove({ extraXBRLAmount: toBN(200), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      // check XBRL balances after
      const stabilityPool_XBRL_After = await stabilityPool.getTotalXBRLDeposits()
      assert.equal(stabilityPool_XBRL_After, 200)
    })
    ...
    it("provideToSP(): reduces the user's XBRL balance by the correct amount", async () => {
      // --- SETUP --- Give Alice a least 200
      await openTrove({ extraXBRLAmount: toBN(200), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      // check user's XBRL balance change
      const alice_XBRLBalance_After = await xbrlToken.balanceOf(alice)
      assert.equal(alice_XBRLBalance_Before.sub(alice_XBRLBalance_After), '200')
    })

    it("provideToSP(): increases totalXBRLDeposits by correct amount", async () => {
      // --- SETUP ---

      // Whale opens Trove with 50 ETH, adds 2000 XBRL to StabilityPool
      await openTrove({ extraXBRLAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
      await stabilityPool.provideToSP(dec(2000, 18), frontEnd_1, { from: whale })

      const totalXBRLDeposits = await stabilityPool.getTotalXBRLDeposits()
      assert.equal(totalXBRLDeposits, dec(2000, 18))
    })
    ...
    it('provideToSP(): Correctly updates user snapshots of accumulated rewards per unit staked', async () => {
      ...
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })
      const whaleXBRL = await xbrlToken.balanceOf(whale)
      await stabilityPool.provideToSP(whaleXBRL, frontEnd_1, { from: whale })

      // 2 Troves opened, each withdraws minimum debt
      await openTrove({ extraXBRLAmount: 0, ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1, } })
      await openTrove({ extraXBRLAmount: 0, ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_2, } })

      // Alice makes Trove and withdraws 100 XBRL
      await openTrove({ extraXBRLAmount: toBN(dec(100, 18)), ICR: toBN(dec(5, 18)), extraParams: { from: alice, value: dec(50, 'ether') } })


      // price drops: defaulter's Troves fall below MCR, whale doesn't
      await priceFeed.setPrice(dec(105, 18));

      const SPXBRL_Before = await stabilityPool.getTotalXBRLDeposits()
      ...
      const SPXBRL_After = await stabilityPool.getTotalXBRLDeposits()
      assert.isTrue(SPXBRL_After.lt(SPXBRL_Before))
    ...
    it("provideToSP(), multiple deposits: updates user's deposit and snapshots", async () => {
      ...
      // 3 Troves opened. Two users withdraw 160 XBRL each
      await openTrove({ extraXBRLAmount: 0, ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1, value: dec(50, 'ether') } })
      await openTrove({ extraXBRLAmount: 0, ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_2, value: dec(50, 'ether') } })
      await openTrove({ extraXBRLAmount: 0, ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_3, value: dec(50, 'ether') } })

      // --- TEST ---

      // Alice makes deposit #1: 150 XBRL
      await openTrove({ extraXBRLAmount: toBN(dec(250, 18)), ICR: toBN(dec(3, 18)), extraParams: { from: alice } })
      ...
      // 2 users with Trove with 180 XBRL drawn are closed
      await troveManager.liquidate(defaulter_1, { from: owner })  // 180 XBRL closed
      await troveManager.liquidate(defaulter_2, { from: owner }) // 180 XBRL closed

      const alice_compoundedDeposit_1 = await stabilityPool.getCompoundedXBRLDeposit(alice)
      ...
      // Bob withdraws XBRL and deposits to StabilityPool
      await openTrove({ extraXBRLAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await stabilityPool.provideToSP(dec(427, 18), frontEnd_1, { from: alice })

      // Defaulter 3 Trove is closed
      await troveManager.liquidate(defaulter_3, { from: owner })

      const alice_compoundedDeposit_2 = await stabilityPool.getCompoundedXBRLDeposit(alice)
    ...
    it("provideToSP(): reverts if user tries to provide more than their XBRL balance", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })

      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice, value: dec(50, 'ether') } })
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob, value: dec(50, 'ether') } })
      const aliceXBRLbal = await xbrlToken.balanceOf(alice)
      const bobXBRLbal = await xbrlToken.balanceOf(bob)

      // Alice, attempts to deposit 1 wei more than her balance

      const aliceTxPromise = stabilityPool.provideToSP(aliceXBRLbal.add(toBN(1)), frontEnd_1, { from: alice })
      await assertRevert(aliceTxPromise, "revert")

      // Bob, attempts to deposit 235534 more than his balance

      const bobTxPromise = stabilityPool.provideToSP(bobXBRLbal.add(toBN(dec(235534, 18))), frontEnd_1, { from: bob })
      await assertRevert(bobTxPromise, "revert")
    })

    it("provideToSP(): reverts if user tries to provide 2^256-1 XBRL, which exceeds their balance", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice, value: dec(50, 'ether') } })
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob, value: dec(50, 'ether') } })
      ...
      // Alice attempts to deposit 2^256-1 XBRL
      try {
    ...
    it("provideToSP(): reverts if cannot receive ETH Gain", async () => {
      ...
      await xbrlToken.transfer(nonPayable.address, dec(250, 18), { from: whale })

      // NonPayable makes deposit #1: 150 XBRL
      ...
      // NonPayable tries to make deposit #2: 100XBRL (which also attempts to withdraw ETH gain)
    ...
    it("provideToSP(): doesn't impact other users' deposits or ETH gains", async () => {
      ...
      await openTrove({ extraXBRLAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraXBRLAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ extraXBRLAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
      ...
      // D opens a trove
      await openTrove({ extraXBRLAmount: toBN(dec(300, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })
      ...
      const alice_XBRLDeposit_Before = (await stabilityPool.getCompoundedXBRLDeposit(alice)).toString()
      const bob_XBRLDeposit_Before = (await stabilityPool.getCompoundedXBRLDeposit(bob)).toString()
      const carol_XBRLDeposit_Before = (await stabilityPool.getCompoundedXBRLDeposit(carol)).toString()
      ...
      //check non-zero XBRL and ETHGain in the Stability Pool
      const XBRLinSP = await stabilityPool.getTotalXBRLDeposits()
      const ETHinSP = await stabilityPool.getETH()
      assert.isTrue(XBRLinSP.gt(mv._zeroBN))
      assert.isTrue(ETHinSP.gt(mv._zeroBN))

      // D makes an SP deposit
      await stabilityPool.provideToSP(dec(1000, 18), frontEnd_1, { from: dennis })
      assert.equal((await stabilityPool.getCompoundedXBRLDeposit(dennis)).toString(), dec(1000, 18))

      const alice_XBRLDeposit_After = (await stabilityPool.getCompoundedXBRLDeposit(alice)).toString()
      const bob_XBRLDeposit_After = (await stabilityPool.getCompoundedXBRLDeposit(bob)).toString()
      const carol_XBRLDeposit_After = (await stabilityPool.getCompoundedXBRLDeposit(carol)).toString()
      ...
      assert.equal(alice_XBRLDeposit_Before, alice_XBRLDeposit_After)
      assert.equal(bob_XBRLDeposit_Before, bob_XBRLDeposit_After)
      assert.equal(carol_XBRLDeposit_Before, carol_XBRLDeposit_After)
    ...
    it("provideToSP(): doesn't impact system debt, collateral or TCR", async () => {
      ...
      // D opens a trove
      await openTrove({ extraXBRLAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })
      ...
      const activeDebt_Before = (await activePool.getXBRLDebt()).toString()
      const defaultedDebt_Before = (await defaultPool.getXBRLDebt()).toString()
      ...
      const activeDebt_After = (await activePool.getXBRLDebt()).toString()
      const defaultedDebt_After = (await defaultPool.getXBRLDebt()).toString()
    ...
    it("provideToSP(): doesn't impact any troves, including the caller's trove", async () => {
      ...
      // D opens a trove
      await openTrove({ extraXBRLAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })
    ...
    it("provideToSP(): doesn't protect the depositor's trove from liquidation", async () => {
      ...
      assert.equal((await stabilityPool.getCompoundedXBRLDeposit(bob)).toString(), dec(1000, 18))
    ...
    it("provideToSP(): providing 0 XBRL reverts", async () => {
      // --- SETUP ---
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })
      ...
      // A, B, C provides 100, 50, 30 XBRL to SP
      await stabilityPool.provideToSP(dec(100, 18), frontEnd_1, { from: alice })
      ...
      const XBRLinSP_Before = (await stabilityPool.getTotalXBRLDeposits()).toString()

      assert.equal(XBRLinSP_Before, dec(180, 18))

      // Bob provides 0 XBRL to the Stability Pool 
      const txPromise_B = stabilityPool.provideToSP(0, frontEnd_1, { from: bob })
      await th.assertRevert(txPromise_B)
    })

    // --- STBL functionality ---
    it("provideToSP(), new deposit: when SP > 0, triggers STBL reward event - increases the sum G", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })

      // A, B, C open troves and make Stability Pool deposits
      await openTrove({ extraXBRLAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraXBRLAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("provideToSP(), new deposit: sets the correct front end tag", async () => {
      ...
      await openTrove({ extraXBRLAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraXBRLAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it("provideToSP(), new deposit: depositor does not receive any STBL rewards", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })
      ...
      // Get A, B, C STBL balances before and confirm they're zero
      const A_STBLBalance_Before = await stblToken.balanceOf(A)
      const B_STBLBalance_Before = await stblToken.balanceOf(B)

      assert.equal(A_STBLBalance_Before, '0')
      assert.equal(B_STBLBalance_Before, '0')
      ...
      // Get A, B, C STBL balances after, and confirm they're still zero
      const A_STBLBalance_After = await stblToken.balanceOf(A)
      const B_STBLBalance_After = await stblToken.balanceOf(B)

      assert.equal(A_STBLBalance_After, '0')
      assert.equal(B_STBLBalance_After, '0')
    })

    it("provideToSP(), new deposit after past full withdrawal: depositor does not receive any STBL rewards", async () => {
      ...
      await openTrove({ extraXBRLAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraXBRLAmount: toBN(dec(4000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })

      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1 } })

      // --- SETUP --- 

      const initialDeposit_A = await xbrlToken.balanceOf(A)
      const initialDeposit_B = await xbrlToken.balanceOf(B)
      // A, B provide to SP
      ...
      // Get A, B, C STBL balances before and confirm they're non-zero
      const A_STBLBalance_Before = await stblToken.balanceOf(A)
      const B_STBLBalance_Before = await stblToken.balanceOf(B)
      assert.isTrue(A_STBLBalance_Before.gt(toBN('0')))
      assert.isTrue(B_STBLBalance_Before.gt(toBN('0')))
      ...
      // Get A, B, C STBL balances after, and confirm they have not changed
      const A_STBLBalance_After = await stblToken.balanceOf(A)
      const B_STBLBalance_After = await stblToken.balanceOf(B)

      assert.isTrue(A_STBLBalance_After.eq(A_STBLBalance_Before))
      assert.isTrue(B_STBLBalance_After.eq(B_STBLBalance_Before))
    })

    it("provideToSP(), new eligible deposit: tagged front end receives STBL rewards", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      await openTrove({ extraXBRLAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraXBRLAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraXBRLAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraXBRLAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: F } })
      ...
      // Get F1, F2, F3 STBL balances before, and confirm they're zero
      const frontEnd_1_STBLBalance_Before = await stblToken.balanceOf(frontEnd_1)
      const frontEnd_2_STBLBalance_Before = await stblToken.balanceOf(frontEnd_2)
      const frontEnd_3_STBLBalance_Before = await stblToken.balanceOf(frontEnd_3)

      assert.equal(frontEnd_1_STBLBalance_Before, '0')
      assert.equal(frontEnd_2_STBLBalance_Before, '0')
      assert.equal(frontEnd_3_STBLBalance_Before, '0')

      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)

      // console.log(`STBLSupplyCap before: ${await communityIssuance.STBLSupplyCap()}`)
      // console.log(`totalSTBLIssued before: ${await communityIssuance.totalSTBLIssued()}`)
      // console.log(`STBL balance of CI before: ${await stblToken.balanceOf(communityIssuance.address)}`)
      ...
      // console.log(`STBLSupplyCap after: ${await communityIssuance.STBLSupplyCap()}`)
      // console.log(`totalSTBLIssued after: ${await communityIssuance.totalSTBLIssued()}`)
      // console.log(`STBL balance of CI after: ${await stblToken.balanceOf(communityIssuance.address)}`)

      // Get F1, F2, F3 STBL balances after, and confirm they have increased
      const frontEnd_1_STBLBalance_After = await stblToken.balanceOf(frontEnd_1)
      const frontEnd_2_STBLBalance_After = await stblToken.balanceOf(frontEnd_2)
      const frontEnd_3_STBLBalance_After = await stblToken.balanceOf(frontEnd_3)

      assert.isTrue(frontEnd_1_STBLBalance_After.gt(frontEnd_1_STBLBalance_Before))
      assert.isTrue(frontEnd_2_STBLBalance_After.gt(frontEnd_2_STBLBalance_Before))
      assert.isTrue(frontEnd_3_STBLBalance_After.gt(frontEnd_3_STBLBalance_Before))
    })
    ...
    it("provideToSP(), new deposit: depositor does not receive ETH gains", async () => {
      ...
      // Whale transfers XBRL to A, B
      await xbrlToken.transfer(A, dec(100, 18), { from: whale })
      await xbrlToken.transfer(B, dec(200, 18), { from: whale })

      // C, D open troves
      await openTrove({ extraXBRLAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraXBRLAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it("provideToSP(), new deposit after past full withdrawal: depositor does not receive ETH gains", async () => {
      ...
      // Whale transfers XBRL to A, B
      await xbrlToken.transfer(A, dec(1000, 18), { from: whale })
      await xbrlToken.transfer(B, dec(1000, 18), { from: whale })

      // C, D open troves
      await openTrove({ extraXBRLAmount: toBN(dec(4000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraXBRLAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it("provideToSP(), topup: triggers STBL reward event - increases the sum G", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // Expect G has increased from the STBL reward event triggered by B's topup
      assert.isTrue(G_After.gt(G_Before))
    })
    ...
    it("provideToSP(), topup from different front end: doesn't change the front end tag", async () => {
      ...
      await xbrlToken.transfer(D, dec(100, 18), { from: whale })
      await xbrlToken.transfer(E, dec(200, 18), { from: whale })

      // A, B, C open troves 
      await openTrove({ extraXBRLAmount: toBN(dec(100, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraXBRLAmount: toBN(dec(200, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(300, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("provideToSP(), topup: depositor receives STBL rewards", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // Get A, B, C STBL balance before
      const A_STBLBalance_Before = await stblToken.balanceOf(A)
      const B_STBLBalance_Before = await stblToken.balanceOf(B)
      const C_STBLBalance_Before = await stblToken.balanceOf(C)
      ...
      // Get STBL balance after
      const A_STBLBalance_After = await stblToken.balanceOf(A)
      const B_STBLBalance_After = await stblToken.balanceOf(B)
      const C_STBLBalance_After = await stblToken.balanceOf(C)

      // Check STBL Balance of A, B, C has increased
      assert.isTrue(A_STBLBalance_After.gt(A_STBLBalance_Before))
      assert.isTrue(B_STBLBalance_After.gt(B_STBLBalance_Before))
      assert.isTrue(C_STBLBalance_After.gt(C_STBLBalance_Before))
    })

    it("provideToSP(), topup: tagged front end receives STBL rewards", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // Get front ends' STBL balance before
      const F1_STBLBalance_Before = await stblToken.balanceOf(frontEnd_1)
      const F2_STBLBalance_Before = await stblToken.balanceOf(frontEnd_2)
      const F3_STBLBalance_Before = await stblToken.balanceOf(frontEnd_3)
      ...
      // Get front ends' STBL balance after
      const F1_STBLBalance_After = await stblToken.balanceOf(A)
      const F2_STBLBalance_After = await stblToken.balanceOf(B)
      const F3_STBLBalance_After = await stblToken.balanceOf(C)

      // Check STBL Balance of front ends has increased
      assert.isTrue(F1_STBLBalance_After.gt(F1_STBLBalance_Before))
      assert.isTrue(F2_STBLBalance_After.gt(F2_STBLBalance_Before))
      assert.isTrue(F3_STBLBalance_After.gt(F3_STBLBalance_Before))
    })
    ...
    it("provideToSP(), topup: tagged front end's stake increases", async () => {
      ...
      await openTrove({ extraXBRLAmount: toBN(dec(200, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(300, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraXBRLAmount: toBN(dec(100, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraXBRLAmount: toBN(dec(200, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraXBRLAmount: toBN(dec(300, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: F } })
    ...
    it("provideToSP(), topup: tagged front end's snapshots update", async () => {
      ...
      // A, B, C, open troves 
      await openTrove({ extraXBRLAmount: toBN(dec(200, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraXBRLAmount: toBN(dec(400, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(600, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      await stabilityPool.provideToSP(await xbrlToken.balanceOf(D), ZERO_ADDRESS, { from: D })
      ...
      // between topups, because some block.timestamp time passes (and STBL is issued) between ops
      const G1 = await stabilityPool.epochToScaleToG(currentScale, currentEpoch)
      await stabilityPool.provideToSP(deposit_A, frontEnd_1, { from: A })
    ...
    it("provideToSP(): reverts when amount is zero", async () => {
      ...
      // Whale transfers XBRL to C, D
      await xbrlToken.transfer(C, dec(100, 18), { from: whale })
      await xbrlToken.transfer(D, dec(100, 18), { from: whale })
    ...
    it("provideToSP(): reverts if user is a registered front end", async () => {
      // C, D, E, F open troves 
      await openTrove({ extraXBRLAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraXBRLAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraXBRLAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraXBRLAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: F } })
    ...
    it("withdrawFromSP(): reverts when user has no active deposit", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(100, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraXBRLAmount: toBN(dec(100, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
    ...
    it("withdrawFromSP(): partial retrieval - retrieves correct XBRL amount and the entire ETH Gain, and updates deposit", async () => {
      // --- SETUP ---
      // Whale deposits 185000 XBRL in StabilityPool
      await openTrove({ extraXBRLAmount: toBN(dec(1, 24)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // Alice makes deposit #1: 15000 XBRL
      await openTrove({ extraXBRLAmount: toBN(dec(15000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      await stabilityPool.provideToSP(dec(15000, 18), frontEnd_1, { from: alice })

      // price drops: defaulters' Troves fall below MCR, alice and whale Trove remain active
      await priceFeed.setPrice(dec(105, 18));

      // 2 users with Trove with 170 XBRL drawn are closed
      const liquidationTX_1 = await troveManager.liquidate(defaulter_1, { from: owner })  // 170 XBRL closed
      const liquidationTX_2 = await troveManager.liquidate(defaulter_2, { from: owner }) // 170 XBRL closed

      const [liquidatedDebt_1] = await th.getEmittedLiquidationValues(liquidationTX_1)
      const [liquidatedDebt_2] = await th.getEmittedLiquidationValues(liquidationTX_2)

      // Alice XBRLLoss is ((15000/200000) * liquidatedDebt), for each liquidation
      const expectedXBRLLoss_A = (liquidatedDebt_1.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18))))
        .add(liquidatedDebt_2.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18))))

      const expectedCompoundedXBRLDeposit_A = toBN(dec(15000, 18)).sub(expectedXBRLLoss_A)
      const compoundedXBRLDeposit_A = await stabilityPool.getCompoundedXBRLDeposit(alice)

      assert.isAtMost(th.getDifference(expectedCompoundedXBRLDeposit_A, compoundedXBRLDeposit_A), 100000)

      // Alice retrieves part of her entitled XBRL: 9000 XBRL
      await stabilityPool.withdrawFromSP(dec(9000, 18), { from: alice })

      const expectedNewDeposit_A = (compoundedXBRLDeposit_A.sub(toBN(dec(9000, 18))))
    ...
    it("withdrawFromSP(): partial retrieval - leaves the correct amount of XBRL in the Stability Pool", async () => {
      // --- SETUP ---
      // Whale deposits 185000 XBRL in StabilityPool
      await openTrove({ extraXBRLAmount: toBN(dec(1, 24)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await stabilityPool.provideToSP(dec(185000, 18), frontEnd_1, { from: whale })
      ...
      const SP_XBRL_Before = await stabilityPool.getTotalXBRLDeposits()
      assert.equal(SP_XBRL_Before, dec(200000, 18))
      ...
      Expect XBRL in SP = (200000 - liquidatedDebt_1 - liquidatedDebt_2 - 9000) */
      const expectedSPXBRL = toBN(dec(200000, 18))
        .sub(toBN(liquidatedDebt_1))
        .sub(toBN(liquidatedDebt_2))
        .sub(toBN(dec(9000, 18)))

      const SP_XBRL_After = (await stabilityPool.getTotalXBRLDeposits()).toString()

      th.assertIsApproximatelyEqual(SP_XBRL_After, expectedSPXBRL)
    })

    it("withdrawFromSP(): full retrieval - leaves the correct amount of XBRL in the Stability Pool", async () => {
      // --- SETUP ---
      // Whale deposits 185000 XBRL in StabilityPool
      ...
      const XBRLinSPBefore = await stabilityPool.getTotalXBRLDeposits()

      // Alice retrieves all of her entitled XBRL:
      await stabilityPool.withdrawFromSP(dec(15000, 18), { from: alice })

      const expectedXBRLinSPAfter = XBRLinSPBefore.sub(compoundedXBRLDeposit_A)

      const XBRLinSPAfter = await stabilityPool.getTotalXBRLDeposits()
      assert.isAtMost(th.getDifference(expectedXBRLinSPAfter, XBRLinSPAfter), 100000)
    })
    ...
    it("withdrawFromSP(): it correctly updates the user's XBRL and ETH snapshots of entitled reward per unit staked", async () => {
      // --- SETUP ---
      // Whale deposits 185000 XBRL in StabilityPool
      await openTrove({ extraXBRLAmount: toBN(dec(1000000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await stabilityPool.provideToSP(dec(185000, 18), frontEnd_1, { from: whale })
    ...
    it("withdrawFromSP(): decreases StabilityPool ETH", async () => {
      ...
      const liquidationTx_1 = await troveManager.liquidate(defaulter_1, { from: owner })  // 180 XBRL closed
    ...
    it("withdrawFromSP(): All depositors are able to withdraw from the SP to their account", async () => {
      ...
      for (account of depositors) {
        await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: account } })
      ...
      const totalDeposits = (await stabilityPool.getTotalXBRLDeposits()).toString()

      assert.isAtMost(th.getDifference(totalDeposits, '0'), 100000)
    })

    it("withdrawFromSP(): increases depositor's XBRL token balance by the expected amount", async () => {
      // Whale opens trove 
      await openTrove({ extraXBRLAmount: toBN(dec(100000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      const aliceBalBefore = await xbrlToken.balanceOf(alice)
      const bobBalBefore = await xbrlToken.balanceOf(bob)

      /* From an offset of 10000 XBRL, each depositor receives
      XBRLLoss = 1666.6666666666666666 XBRL

      and thus with a deposit of 10000 XBRL, each should withdraw 8333.3333333333333333 XBRL (in practice, slightly less due to rounding error)
      */

      // Price bounces back to $200 per ETH
      await priceFeed.setPrice(dec(200, 18))

      // Bob issues a further 5000 XBRL from his trove 
      await borrowerOperations.withdrawXBRL(th._100pct, dec(5000, 18), bob, bob, { from: bob })

      // Expect Alice's XBRL balance increase be very close to 8333.3333333333333333 XBRL
      await stabilityPool.withdrawFromSP(dec(10000, 18), { from: alice })
      const aliceBalance = (await xbrlToken.balanceOf(alice))

      assert.isAtMost(th.getDifference(aliceBalance.sub(aliceBalBefore), '8333333333333333333333'), 100000)

      // expect Bob's XBRL balance increase to be very close to  13333.33333333333333333 XBRL
      await stabilityPool.withdrawFromSP(dec(10000, 18), { from: bob })
      const bobBalance = (await xbrlToken.balanceOf(bob))
    ...
    it("withdrawFromSP(): doesn't impact other users Stability deposits or ETH gains", async () => {
      ...
      await openTrove({ extraXBRLAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
    ...
    it("withdrawFromSP(): succeeds when amount is 0 and system has an undercollateralized trove", async () => {
      ...
      const A_STBLBalBefore = await stblToken.balanceOf(A)

      // Check Alice has gains to withdraw
      const A_pendingETHGain = await stabilityPool.getDepositorETHGain(A)
      const A_pendingSTBLGain = await stabilityPool.getDepositorSTBLGain(A)
      assert.isTrue(A_pendingETHGain.gt(toBN('0')))
      assert.isTrue(A_pendingSTBLGain.gt(toBN('0')))
      ...
      const A_STBLBalAfter = await stblToken.balanceOf(A)
      const A_STBLBalDiff = A_STBLBalAfter.sub(A_STBLBalBefore)

      // Check A's ETH and STBL balances have increased correctly
      assert.isTrue(A_ETHBalAfter.sub(A_expectedBalance).eq(A_pendingETHGain))
      assert.isAtMost(th.getDifference(A_STBLBalDiff, A_pendingSTBLGain), 1000)
    })

    it("withdrawFromSP(): withdrawing 0 XBRL doesn't alter the caller's deposit or the total XBRL in the Stability Pool", async () => {
      // --- SETUP ---
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // Bob withdraws 0 XBRL from the Stability Pool 
      await stabilityPool.withdrawFromSP(0, { from: bob })

      // check Bob's deposit and total XBRL in Stability Pool has not changed
      const bob_Deposit_After = (await stabilityPool.getCompoundedXBRLDeposit(bob)).toString()
      const XBRLinSP_After = (await stabilityPool.getTotalXBRLDeposits()).toString()

      assert.equal(bob_Deposit_Before, bob_Deposit_After)
      assert.equal(XBRLinSP_Before, XBRLinSP_After)
    })
    ...
    it("withdrawFromSP(): withdrawing 0 ETH Gain does not alter the caller's ETH balance, their trove collateral, or the ETH  in the Stability Pool", async () => {
      ...
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1 } })
    ...
    it("withdrawFromSP(): Request to withdraw > caller's deposit only withdraws the caller's compounded deposit", async () => {
      ...
      const alice_XBRL_Balance_Before = await xbrlToken.balanceOf(alice)
      const bob_XBRL_Balance_Before = await xbrlToken.balanceOf(bob)

      const alice_Deposit_Before = await stabilityPool.getCompoundedXBRLDeposit(alice)
      const bob_Deposit_Before = await stabilityPool.getCompoundedXBRLDeposit(bob)

      const XBRLinSP_Before = await stabilityPool.getTotalXBRLDeposits()
      ...
      // Check Bob's XBRL balance has risen by only the value of his compounded deposit
      const bob_expectedXBRLBalance = (bob_XBRL_Balance_Before.add(bob_Deposit_Before)).toString()
      const bob_XBRL_Balance_After = (await xbrlToken.balanceOf(bob)).toString()
      assert.equal(bob_XBRL_Balance_After, bob_expectedXBRLBalance)

      // Alice attempts to withdraws 2309842309.000000000000000000 XBRL from the Stability Pool 
      await stabilityPool.withdrawFromSP('2309842309000000000000000000', { from: alice })

      // Check Alice's XBRL balance has risen by only the value of her compounded deposit
      const alice_expectedXBRLBalance = (alice_XBRL_Balance_Before.add(alice_Deposit_Before)).toString()
      const alice_XBRL_Balance_After = (await xbrlToken.balanceOf(alice)).toString()
      assert.equal(alice_XBRL_Balance_After, alice_expectedXBRLBalance)

      // Check XBRL in Stability Pool has been reduced by only Alice's compounded deposit and Bob's compounded deposit
      const expectedXBRLinSP = (XBRLinSP_Before.sub(alice_Deposit_Before).sub(bob_Deposit_Before)).toString()
      const XBRLinSP_After = (await stabilityPool.getTotalXBRLDeposits()).toString()
      assert.equal(XBRLinSP_After, expectedXBRLinSP)
    })

    it("withdrawFromSP(): Request to withdraw 2^256-1 XBRL only withdraws the caller's compounded deposit", async () => {
      // --- SETUP ---
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // Bob attempts to withdraws maxBytes32 XBRL from the Stability Pool
      ...
      // Check XBRL in Stability Pool has been reduced by only  Bob's compounded deposit
      const expectedXBRLinSP = (XBRLinSP_Before.sub(bob_Deposit_Before)).toString()
      const XBRLinSP_After = (await stabilityPool.getTotalXBRLDeposits()).toString()
      assert.equal(XBRLinSP_After, expectedXBRLinSP)
    })
    ...
    it("withdrawFromSP(): caller can withdraw full deposit and ETH gain during Recovery Mode", async () => {
      ...
      await openTrove({ extraXBRLAmount: toBN(dec(1000000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
      // Price halves
      await priceFeed.setPrice(dec(200, 18))

      // A, B, C open troves and make Stability Pool deposits
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(4, 18)), extraParams: { from: alice } })
      await openTrove({ extraXBRLAmount: toBN(dec(20000, 18)), ICR: toBN(dec(4, 18)), extraParams: { from: bob } })
      await openTrove({ extraXBRLAmount: toBN(dec(30000, 18)), ICR: toBN(dec(4, 18)), extraParams: { from: carol } })

      await borrowerOperations.openTrove(th._100pct, await getOpenTroveXBRLAmount(dec(10000, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(100, 'ether') })

      // A, B, C provides 10000, 5000, 3000 XBRL to SP
      ...
      const alice_XBRL_Balance_Before = await xbrlToken.balanceOf(alice)
      const bob_XBRL_Balance_Before = await xbrlToken.balanceOf(bob)
      const carol_XBRL_Balance_Before = await xbrlToken.balanceOf(carol)
      ...
      const alice_Deposit_Before = await stabilityPool.getCompoundedXBRLDeposit(alice)
      const bob_Deposit_Before = await stabilityPool.getCompoundedXBRLDeposit(bob)
      const carol_Deposit_Before = await stabilityPool.getCompoundedXBRLDeposit(carol)
      ...
      // Check XBRL balances of A, B, C have risen by the value of their compounded deposits, respectively
      const alice_expectedXBRLBalance = (alice_XBRL_Balance_Before.add(alice_Deposit_Before)).toString()

      const bob_expectedXBRLBalance = (bob_XBRL_Balance_Before.add(bob_Deposit_Before)).toString()
      const carol_expectedXBRLBalance = (carol_XBRL_Balance_Before.add(carol_Deposit_Before)).toString()

      const alice_XBRL_Balance_After = (await xbrlToken.balanceOf(alice)).toString()
 
      const bob_XBRL_Balance_After = (await xbrlToken.balanceOf(bob)).toString()
      const carol_XBRL_Balance_After = (await xbrlToken.balanceOf(carol)).toString()



      assert.equal(alice_XBRL_Balance_After, alice_expectedXBRLBalance)
      assert.equal(bob_XBRL_Balance_After, bob_expectedXBRLBalance)
      assert.equal(carol_XBRL_Balance_After, carol_expectedXBRLBalance)
      ...
      // Check XBRL in Stability Pool has been reduced by A, B and C's compounded deposit
      const expectedXBRLinSP = (XBRLinSP_Before
    ...
    it("getDepositorETHGain(): depositor does not earn further ETH gains from liquidations while their compounded deposit == 0: ", async () => {
      ...
      await openTrove({ extraXBRLAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1 } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_2 } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_3 } })

      // A, B, provide 10000, 5000 XBRL to SP
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_1, { from: alice })
      ...
      assert.equal(XBRLinSP, '0')

      // Check Stability deposits have been fully cancelled with debt, and are now all zero
      const alice_Deposit = (await stabilityPool.getCompoundedXBRLDeposit(alice)).toString()
      const bob_Deposit = (await stabilityPool.getCompoundedXBRLDeposit(bob)).toString()
      ...
      // Whale deposits 10000 XBRL to Stability Pool
    ...
    // --- STBL functionality ---
    it("withdrawFromSP(): triggers STBL reward event - increases the sum G", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(1, 24)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })

      // A, B, C open troves
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraXBRLAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("withdrawFromSP(), partial withdrawal: depositor receives STBL rewards", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    ...
    it("withdrawFromSP(), partial withdrawal: tagged front end receives STBL rewards", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    ...
    it("withdrawFromSP(), partial withdrawal: tagged front end's stake decreases", async () => {
      ...
      await openTrove({ extraXBRLAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraXBRLAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraXBRLAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraXBRLAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: F } })
    ...
    it("withdrawFromSP(), partial withdrawal: tagged front end's snapshots update", async () => {
      ...
      await openTrove({ extraXBRLAmount: toBN(dec(60000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })

      // D opens trove
      await openTrove({ extraXBRLAmount: toBN(dec(100000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it("withdrawFromSP(), full withdrawal: removes deposit's front end tag", async () => {
      ...
      await xbrlToken.transfer(B, dec(20000, 18), { from: whale })
    ...
    it("withdrawFromSP(), full withdrawal: zero's depositor's snapshots", async () => {
      ...
      await openTrove({ extraXBRLAmount: toBN(dec(20000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: E } })
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_3, { from: E })

      // Fast-forward time and make a second deposit, to trigger STBL reward and make G > 0
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_3, { from: E })
      ...
      // C, D open troves
      await openTrove({ extraXBRLAmount: toBN(dec(30000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: C } })
      await openTrove({ extraXBRLAmount: toBN(dec(40000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: D } })
    ...
    it("withdrawFromSP(), full withdrawal that reduces front end stake to 0: zero’s the front end’s snapshots", async () => {
      ...
      // E opens trove and makes a deposit
      await openTrove({ extraXBRLAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_3, { from: E })
      ...
      // A, B open troves
      await openTrove({ extraXBRLAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraXBRLAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
    ...
    it("withdrawFromSP(), reverts when initial deposit value is 0", async () => {
      ...
      await openTrove({ extraXBRLAmount: toBN(dec(10100, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_1, { from: A })

      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1 } })

      //  SETUP: Execute a series of operations to trigger STBL and ETH rewards for depositor A
    ...
    it("withdrawETHGainToTrove(): Applies XBRLLoss to user's deposit, and redirects ETH reward to user's Trove", async () => {
      // --- SETUP ---
      // Whale deposits 185000 XBRL in StabilityPool
      await openTrove({ extraXBRLAmount: toBN(dec(1000000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await stabilityPool.provideToSP(dec(185000, 18), frontEnd_1, { from: whale })
      ...
      const compoundedDeposit_A = await stabilityPool.getCompoundedXBRLDeposit(alice)
      ...
      const expectedXBRLLoss_A = liquidatedDebt.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18)))
      const expectedCompoundedDeposit_A = toBN(dec(15000, 18)).sub(expectedXBRLLoss_A)
      ...
      // check Alice's XBRLLoss has been applied to her deposit expectedCompoundedDeposit_A
    ...
    it("withdrawETHGainToTrove(): reverts if user has no trove", async () => {
     ...
     await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1 } })
      ...
      // A transfers XBRL to D
      await xbrlToken.transfer(dennis, dec(10000, 18), { from: alice })
    ...
    it("withdrawETHGainToTrove(): triggers STBL reward event - increases the sum G", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    ...
    it("withdrawETHGainToTrove(), eligible deposit: depositor receives STBL rewards", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    ...
    it("withdrawETHGainToTrove(), eligible deposit: tagged front end receives STBL rewards", async () => {
      await openTrove({ extraXBRLAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
     ...
     await priceFeed.setPrice(dec(105, 18))
      ...
      // Get front ends' STBL balance after
      const F1_STBLBalance_After = await stblToken.balanceOf(frontEnd_1)
      const F2_STBLBalance_After = await stblToken.balanceOf(frontEnd_2)
      const F3_STBLBalance_After = await stblToken.balanceOf(frontEnd_3)
    ...
    it("withdrawETHGainToTrove(): reverts when depositor has no ETH gain", async () => {
      ...
      // fastforward time, and E makes a deposit, creating STBL rewards for all
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)
      await openTrove({ extraXBRLAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
    ...
    it("registerFrontEnd(): reverts if address has a non-zero deposit already", async () => {
      // C, D, E open troves 
      await openTrove({ extraXBRLAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraXBRLAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraXBRLAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
