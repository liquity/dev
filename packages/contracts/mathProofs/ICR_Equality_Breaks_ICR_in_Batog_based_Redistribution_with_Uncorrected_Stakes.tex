...
\begin{itemize}
\item We are building a system that needs to keep CDPs in order to allow efficient redemptions in ascending order of collateral ratio.
\item Given that we also do redistributions, we need to ensure that redistributions do not break ordering.
\item It turns out that redistributing in proportion to collateral size maintains ordering.
\item It is obvious that redistributing in proportion to collateral size in a push-based manner does not scale, i.e. is of O(n).
\item We know from earlier research that there are scalable O(1) ways to assign proportional rewards as long as the basis for the rewards ("stakes") does not change over time. In other words, the method presented in the paper does not compound the reward.
\item We thus need to extend original Batog approach to make it so that subsequent collateral changes to CDPs are accounted for by correcting their stakes, to make sure they do not receive disproportionate rewards due to the fact that the existing CDPs stake does not take pending rewards from past liquidations into account.
\item We show that the corrected stake approach preserves ordering.
\end{itemize}
