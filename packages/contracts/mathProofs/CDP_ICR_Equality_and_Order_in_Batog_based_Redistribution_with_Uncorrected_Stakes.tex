...
\begin{definition}[System Order]
  A system of troves has an order, N. When the first trove is created, the system order equals 1.
...
\begin{itemize}
  ...
  \item Lemma \ref{result:3}. We consider a system transition $\Gamma_1 \rightarrow \Gamma_2$ triggered by the creation of a trove $j$, followed by a set of creations $N$, and then by a set of liquidations $P$, such that $|N|, |P| > 0$ and $N \cap P = \emptyset$. For any trove $i$ in $\Gamma_1$ with $\gamma_i = \gamma_j$, the corrected stake approach maintains $\gamma_i = \gamma_j$ in $\Gamma_2$.

    \includegraphics[width=\linewidth]{System_order_evolution_3.png}

  \item Theorem \ref{result:4}. In a second order system with $M$ previous liquidations, and $N$ second-order troves added after the last liquidation, ICR equality between a first-order trove and second-order trove holds across $P$ subsequent liquidation events
...
\begin{lemma}\label{result:1}
Corrected Stake Preserves ICR Equality Across a Reward Event in a Second Order System with $m$ Past Liquidations
\end{lemma}
...
\bigskip
Now, a fresh trove is added, $i$, with collateral $c_i$.  Let the ICR of trove $i$ equal the ICR of an active first-order trove $j$.\\

Now, trove $k$ liquidates. Upon liquidation, the second-order trove $i$ and the first-order trove $j$ earn the following collateral rewards:
...
\begin{equation} 
    \gamma_i=\frac{c_\mathsmaller{i}}{d_\mathsmaller{i}}
\end{equation}

\begin{equation} 
    \gamma_j=\frac{c_\mathsmaller{j}+x_\mathsmaller{j}}{d_\mathsmaller{j}+y_\mathsmaller{j}}
\end{equation}

\bigskip
Where $c_i$, $d_i$ and $c_j$, $d_j$ are the collateral and debt values of trove $i$ and trove $j$ respectively.\\

$x_j$, $y_j$ are the respective accumulated collateral and debt rewards for trove $j$ earned by its stake over its lifetime.\\

The ICR equality identity (\ref{eq:10}) yields the following relation:

\begin{equation} 
        c_i=\frac{d_\mathsmaller{i}}{d_\mathsmaller{j}+y_\mathsmaller{j}}(c_\mathsmaller{j}+x_\mathsmaller{j})
\end{equation}
...
\begin{equation} \label{eq:14}
    c_i=k(c_\mathsmaller{j}+x_\mathsmaller{j})
\end{equation}
...
\begin{equation} \label{eq:15}
    k=\frac{d_\mathsmaller{i}}{d_\mathsmaller{j}+y_\mathsmaller{j}}
\end{equation}


\bigskip
Trove $i$’s stake $s_i$ is given by the corrected stake rule (\ref{eq:2}), that is:

\begin{equation} 
    s_i=\frac{c_\mathsmaller{i} \cdot S_\emptyset}{C_\emptyset}
\end{equation}
...
\begin{equation} \label{eq:17}
    s_i=\frac{c_i \cdot C_n}{C_n+X_n}
\end{equation}

\bigskip
Now, a new liquidation occurs: trove $k$ liquidates. The system becomes second-order.\\

The event causes trove $k$’s collateral and debt ($c_k$ and $d_k$) to be redistributed between all active troves, proportional to their stake.\\
...
\bigskip
We define the collateral and debt rewards earned by trove $i$ and trove $j$ in the reward event:
...
\bigskip
And since $s_\mathsmaller{j}$ is a first-order stake:

\begin{equation} \label{eq:21}
    s_\mathsmaller{j}=c_\mathsmaller{j}
\end{equation}

\bigskip
To show ICR equivalence after the reward event, we must first obtain $s_i$ as a linear function of $c_j$. Recall our definition of trove $i$’s stake from (\ref{eq:17}):

\begin{equation} 
    s_\mathsmaller{i}=\frac{c_\mathsmaller{i} \cdot C_n}{(C_n+X_n)}
\end{equation}
...
\begin{equation} 
    s_\mathsmaller{i}=\frac{k(c_\mathsmaller{j}+x_\mathsmaller{j})C_n}{C_n+X_n}
\end{equation}
...
\begin{equation} 
    s_\mathsmaller{i}=\frac{k(c_j+Hc_j)C_n}{C_n+HC_n}
\end{equation}
...
\begin{equation} 
    s_i=\frac{kc_j(C_n+HC_n)}{(C_n+HC_n)}
\end{equation}
...
\bigskip
We now compare ICRs of trove $i$ and trove $j$, after liquidation of trove $k$, respectively $\gamma_{i}'$ and $\gamma_{j}'$.

\begin{equation} 
    \gamma_{i}'=\frac{c_\mathsmaller{i}+r_{c\mathsmaller{i}}}{d_\mathsmaller{i}+r_{d\mathsmaller{i}}}
\end{equation}

\begin{equation} 
    \gamma_{j}'=\frac{c_\mathsmaller{j}+x_\mathsmaller{j}+r_{c\mathsmaller{j}}}{d_\mathsmaller{j}+y_\mathsmaller{j}+r_{d\mathsmaller{j}}}
\end{equation}
...
\begin{equation} 
    \gamma_{i}'=\frac{c_i+as_i}{d_i+bs_i}
\end{equation}

\begin{equation} 
    \gamma_{j}'=\frac{c_\mathsmaller{j}+x_\mathsmaller{j}+as_\mathsmaller{j}}{d_\mathsmaller{j}+y_\mathsmaller{j}+bs_\mathsmaller{j}}
\end{equation}

\bigskip
Now, substituting our definitions for $s_\mathsmaller{j}$ (\ref{eq:21}) and $s_i$ (\ref{eq:29}):

\begin{equation} 
    \gamma_{i}'=\frac{c_\mathsmaller{i}+akc_\mathsmaller{j}}{d_\mathsmaller{i}+bkc_\mathsmaller{j}}
\end{equation}

\begin{equation} 
    \gamma_{j}'=\frac{c_\mathsmaller{j}+x_\mathsmaller{j}+ac_\mathsmaller{j}}{d_\mathsmaller{j}+y_\mathsmaller{j}+bc_\mathsmaller{j}}
\end{equation}

\bigskip
Using identities (\ref{eq:14}) for $c_i$, and (\ref{eq:15}) for $d_i$:

\begin{equation} 
    \gamma_{i}'=\frac{k(c_\mathsmaller{j} + x_\mathsmaller{j}+ac_\mathsmaller{j})}{k(d_\mathsmaller{j}+y_\mathsmaller{j}+bc_\mathsmaller{j})}
\end{equation}
...
\begin{equation} 
        \gamma_{i}'=\frac{c_\mathsmaller{j} + x_\mathsmaller{j}+ac_\mathsmaller{j}}{d_\mathsmaller{j}+y_\mathsmaller{j}+bc_\mathsmaller{j}}
\end{equation}
...
\begin{lemma} \label{result:2}
Arbitrary Number of Liquidation Events At Current System Order
\end{lemma}
...
Arbitrary Number of troves Added Between Liquidation Events
\end{lemma}

\begin{proof}

With $N$ second-order troves added between consecutive liquidation events, the stake $s_i$ of any given second-order trove is given by \ref{eq:2}:

\begin{equation} 
    s_i=\frac{c_i \cdot S_\emptyset}{C_\emptyset}
\end{equation}

\bigskip
The snapshots of the system state after the last liquidation event ($S_\emptyset$, $C_\emptyset$) remain constant until the next liquidation. It is clear that all $N$ second-order stakes $s_i$ have been corrected by the same constant factor.\\

Thus, $s_i$ in the N second-order troves case is equal to $s_i$ in the single second-order trove case.\\
...
\begin{proof}
Combining the previous 3 lemmas, this follows immediately.
\end{proof}
...
\bigskip
Clearly, the ratio  $k = \frac{d_j}{d_i+y_i} = 1$.\\

We substitute $k=1$ into the second-order system expression for $s_i$, from equation \ref{eq:29}, to yield:
...
\begin{proof}
We prove it by induction. We have already proved for $n=1$ that $\Gamma_i=\Gamma_j$.
