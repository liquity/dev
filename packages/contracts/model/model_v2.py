...
def get_new_token_price(data, params, redeemed_amount, momentum):
    ...
    price = (((data.token_demand  - redeemed_amount) / (data.loan_issuance[-1])) - (F * momentum)) * factor 
...
def get_new_loan_issuance(data, params, token_price, momentum ):
    loan_issuance = data.loan_issuance[-1] * (params.T*(token_price) + params.F*(momentum))
...
# compute the excess loan issuance needed to maintain the price at 1.1, according to QTM.
...
for i in range(1, 250):
    ...
    loan_issuance = loan_issuance + excess_issuance
    token_supply = get_new_token_supply(loan_issuance, 0)
