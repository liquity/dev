...
contract('Echidna debugger', async accounts => {
  ...
  before(async () => {
    ...
    oneusdToken = await ONEUSDToken.at(await echidnaTester.oneusdToken())
  ...
  it.only('1USD balance', async () => {
    ...
    const gasPoolBalance = await oneusdToken.balanceOf(GAS_POOL_ADDRESS);
    const activePoolBalance = await activePool.get1USDDebt();
    const defaultPoolBalance = await defaultPool.get1USDDebt();
    const stabilityPoolBalance = await stabilityPool.getTotal1USDDeposits();
    const currentTrove = await echidnaTester.echidnaProxies(0);
    const troveBalance = oneusdToken.balanceOf(currentTrove);
