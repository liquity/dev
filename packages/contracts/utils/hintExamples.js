...
async function main() {
  ...
  const ONEUSDAmount = toBN(toWei('2500')) // borrower wants to withdraw 2500 1USD
  ...
  const liquidationReserve = await troveManager.ONEUSD_GAS_COMPENSATION()
  const expectedFee = await troveManager.getBorrowingFeeWithDecay(ONEUSDAmount)

  // Total debt of the new trove = 1USD amount drawn, plus fee, plus the liquidation reserve
  const expectedDebt = ONEUSDAmount.add(expectedFee).add(liquidationReserve)
  ...
  await borrowerOperations.openTrove(maxFee, ONEUSDAmount, upperHint, lowerHint, { value: ETHColl })

  // --- adjust trove --- 

  const collIncrease = toBN(toWei('1'))  // borrower wants to add 1 ETH
  const ONEUSDRepayment = toBN(toWei('230')) // borrower wants to repay 230 1USD

  // Get trove's current debt and coll
  const { 0: debt, 1: coll } = await troveManager.getEntireDebtAndColl(borrower)

  const newDebt = debt.sub(ONEUSDRepayment)
  ...
  await borrowerOperations.adjustTrove(maxFee, 0, ONEUSDRepayment, false, upperHint, lowerHint, { value: collIncrease })


  // --- RedeemCollateral ---

  // Get the redemptions hints from the deployed HintHelpers contract
  const redemptionhint = await hintHelpers.getRedemptionHints(ONEUSDAmount, price, 50)
