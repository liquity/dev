...
contract EchidnaTester {
    ...
    function getAdjustedETH(uint actorBalance, uint _ETH, uint ratio) internal view returns (uint) {
        ...
        require(price > 0);
        uint minETH = ratio.mul(ONEUSD_GAS_COMPENSATION).div(price);
        require(actorBalance > minETH);
        uint ETH = minETH + _ETH % (actorBalance - minETH);
        return ETH;
    }

    function getAdjusted1USD(uint ETH, uint _1USDAmount, uint ratio) internal view returns (uint) {
        uint price = priceFeedTestnet.getPrice();
    ...
    function openTroveExt(uint _i, uint _ETH, uint _1USDAmount) public payable {
        uint actor = _i % NUMBER_OF_ACTORS;
        ...
        uint ONEUSDAmount = getAdjusted1USD(ETH, _1USDAmount, CCR);

        //console.log('ETH', ETH);
        //console.log('ONEUSDAmount', ONEUSDAmount);

        echidnaProxy.openTrovePrx(ETH, ONEUSDAmount, address(0), address(0), 0);
    ...
    function openTroveRawExt(uint _i, uint _ETH, uint _1USDAmount, address _upperHint, address _lowerHint, uint _maxFee) public payable {
        uint actor = _i % NUMBER_OF_ACTORS;
        echidnaProxies[actor].openTrovePrx(_ETH, _1USDAmount, _upperHint, _lowerHint, _maxFee);
    }
    ...
    function adjustTroveExt(uint _i, uint _ETH, uint _collWithdrawal, uint _debtChange, bool _isDebtIncrease) external payable {
        ...
        if (_isDebtIncrease) {
            ...
            debtChange = getAdjusted1USD(ETH, uint(_debtChange), MCR);
        }
