...
/*
 * The Stability Pool holds 1USD tokens deposited by Stability Pool depositors.
 *
 * When a trove is liquidated, then depending on system conditions, some of its 1USD debt gets offset with
 ...
 * of the total 1USD in the Stability Pool, depletes 40% of each deposit.
 *
...
contract StabilityPool is LiquityBase, Ownable, CheckContract, IStabilityPool {
    ...
    * after a series of liquidations have occurred, each of which cancel some 1USD debt with the deposit.
    *
    ...
    */
    function provideToSP(uint _amount, address _frontEndTag) external override {
        ...
        _send1USDtoStabilityPool(msg.sender, _amount);

        uint newDeposit = compounded1USDDeposit.add(_amount);
    ...
    */
    function withdrawFromSP(uint _amount) external override {
        ...
        // Update front end stake
        uint compoundedFrontEndStake = getCompoundedFrontEndStake(frontEnd);
        uint newFrontEndStake = compoundedFrontEndStake.sub(ONEUSDtoWithdraw);
        _updateFrontEndStakeAndSnapshots(frontEnd, newFrontEndStake);
        emit FrontEndStakeChanged(frontEnd, newFrontEndStake, msg.sender);

        _send1USDToDepositor(msg.sender, ONEUSDtoWithdraw);

        // Update deposit
        uint newDeposit = compounded1USDDeposit.sub(ONEUSDtoWithdraw);
        _updateDepositAndSnapshots(msg.sender, newDeposit);
        emit UserDepositChanged(msg.sender, newDeposit);
    ...
    * - Updates snapshots for deposit and tagged front end stake */
    ...
    function _updateG(uint _LQTYIssuance) internal {
        uint total1USD = total1USDDeposits; // cached to save an SLOAD
        /*
        * When total deposits is 0, G is not updated. In this case, the LQTY issued can not be obtained by later
        * depositors - it is missed out on, and remains in the balanceof the CommunityIssuance contract.
        *
        */
        if (total1USD == 0 || _LQTYIssuance == 0) {return;}

        uint LQTYPerUnitStaked;
        LQTYPerUnitStaked =_computeLQTYPerUnitStaked(_LQTYIssuance, total1USD);
    ...
    function _computeLQTYPerUnitStaked(uint _LQTYIssuance, uint _total1USDDeposits) internal returns (uint) {
        /*  
        ...
        */
        ...
        uint LQTYPerUnitStaked = LQTYNumerator.div(_total1USDDeposits);
        lastLQTYError = LQTYNumerator.sub(LQTYPerUnitStaked.mul(_total1USDDeposits));
    ...
    */
    function offset(uint _debtToOffset, uint _collToAdd) external override {
        ...
        uint total1USD = total1USDDeposits; // cached to save an SLOAD
        if (total1USD == 0 || _debtToOffset == 0) { return; }
        ...
        returns (uint ETHGainPerUnitStaked, uint ONEUSDLossPerUnitStaked)
    {
        ...
        */
        ...
        if (_debtToOffset == _total1USDDeposits) {
            ONEUSDLossPerUnitStaked = DECIMAL_PRECISION;  // When the Pool depletes to 0, so does each deposit 
            last1USDLossError_Offset = 0;
        } else {
            uint ONEUSDLossNumerator = _debtToOffset.mul(DECIMAL_PRECISION).sub(last1USDLossError_Offset);
            /*
            * Add 1 to make error in quotient positive. We want "slightly too much" 1USD loss,
            * which ensures the error in any given compounded1USDDeposit favors the Stability Pool.
            */
            ONEUSDLossPerUnitStaked = (ONEUSDLossNumerator.div(_total1USDDeposits)).add(1);
    ...
    function _updateRewardSumAndProduct(uint _ETHGainPerUnitStaked, uint _1USDLossPerUnitStaked) internal {
        ...
        assert(_1USDLossPerUnitStaked <= DECIMAL_PRECISION);
        /*
        * The newProductFactor is the factor by which to change all deposits, due to the depletion of Stability Pool 1USD in the liquidation.
        * We make the product factor 0 if there was a pool-emptying. Otherwise, it is (1 - ONEUSDLossPerUnitStaked)
        */
        uint newProductFactor = uint(DECIMAL_PRECISION).sub(_1USDLossPerUnitStaked);
    ...
    function _moveOffsetCollAndDebt(uint _collToAdd, uint _debtToOffset) internal {
        ...
        // Cancel the liquidated 1USD debt with the 1USD in the stability pool
        activePoolCached.decrease1USDDebt(_debtToOffset);
        _decrease1USD(_debtToOffset);
    ...
    function _decrease1USD(uint _amount) internal {
        uint newTotal1USDDeposits = total1USDDeposits.sub(_amount);
        total1USDDeposits = newTotal1USDDeposits;
    ...
    */
    function getCompounded1USDDeposit(address _depositor) public view override returns (uint) {
        uint initialDeposit = deposits[_depositor].initialValue;
        if (initialDeposit == 0) { return 0; }
    ...
    */
    ...
    function _send1USDtoStabilityPool(address _address, uint _amount) internal {
        oneusdToken.sendToPool(_address, address(this), _amount);
        uint newTotal1USDDeposits = total1USDDeposits.add(_amount);
        total1USDDeposits = newTotal1USDDeposits;
        emit StabilityPool1USDBalanceUpdated(newTotal1USDDeposits);
    }
    ...
    function _send1USDToDepositor(address _depositor, uint ONEUSDWithdrawal) internal {
        if (ONEUSDWithdrawal == 0) {return;}

        oneusdToken.returnFromPool(address(this), _depositor, ONEUSDWithdrawal);
        _decrease1USD(ONEUSDWithdrawal);
    }
