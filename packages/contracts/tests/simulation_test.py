...
def contracts():
    ...
    contracts.oneusdToken = ONEUSDToken.deploy(
        contracts.troveManager.address,
        contracts.stabilityPool.address,
        contracts.borrowerOperations.address,
        { 'from': accounts[0] }
...
* issuance fee
* trove pool formed
* 1USD supply determined
* 1USD stability pool demand determined
* 1USD liquidity pool demand determined
* 1USD price determined
...
def test_run_simulation(add_accounts, contracts, print_expectations):
    ...
    with open('tests/simulation.csv', 'w', newline='') as csvfile:
        ...
        datawriter.writerow(['iteration', 'ETH_price', 'price_1USD', 'price_LQTY', 'num_troves', 'total_coll', 'total_debt', 'TCR', 'recovery_mode', 'last_ICR', 'SP_1USD', 'SP_ETH', 'total_coll_added', 'total_coll_liquidated', 'total_1usd_redempted'])
        ...
        for index in range(1, n_sim):
            ...
            #close troves
            result_close = close_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_1USD, index)

            #adjust troves
            [coll_added_adjust, issuance_1USD_adjust] = adjust_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, index)

            #open troves
            [coll_added_open, issuance_1USD_open] = open_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_1USD, index)
            ...
            [price_1USD, redemption_pool, redemption_fee, issuance_1USD_stabilizer] = price_stabilizer(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_1USD, index)
            total_1usd_redempted = total_1usd_redempted + redemption_pool
            print('1USD price', price_1USD)
            print('LQTY price', price_LQTY_current)

            issuance_fee = price_1USD * (issuance_1USD_adjust + issuance_1USD_open + issuance_1USD_stabilizer)
            ...
            [ETH_price, num_troves, total_coll, total_debt, TCR, recovery_mode, last_ICR, SP_1USD, SP_ETH] = logGlobalState(contracts)
            print('Total redempted ', total_1usd_redempted)
            ...
            datawriter.writerow([index, ETH_price, price_1USD, price_LQTY_current, num_troves, total_coll, total_debt, TCR, recovery_mode, last_ICR, SP_1USD, SP_ETH, total_coll_added, total_coll_liquidated, total_1usd_redempted])
