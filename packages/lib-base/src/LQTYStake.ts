...
export class LQTYStake {
  ...
  /** @internal */
  constructor(stakedLQTY = Decimal.ZERO, collateralGain = Decimal.ZERO, oneusdGain = Decimal.ZERO) {
    this.stakedLQTY = stakedLQTY;
    this.collateralGain = collateralGain;
    this.oneusdGain = oneusdGain;
  }

  get isEmpty(): boolean {
    return this.stakedLQTY.isZero && this.collateralGain.isZero && this.oneusdGain.isZero;
  }
  ...
  equals(that: LQTYStake): boolean {
    return (
      ...
      this.oneusdGain.eq(that.oneusdGain)
    );
  }
