...
const liquidationReserve = Number(XBRL_LIQUIDATION_RESERVE);
const maximumBorrowingRate = Number(MAXIMUM_BORROWING_RATE);

const maxDebt = 10 * Number(XBRL_MINIMUM_DEBT);
        ...
        fc.property(validNonEmptyTrove(), validNonEmptyTrove(), (a, b) => {
          ...
          return (
            ...
            (change.type === "adjustment" &&
              ...
              !change.params.borrowXBRL?.isZero &&
              !change.params.repayXBRL?.isZero)
          );
        })
      );
    });
      ...
      fc.assert(
        fc.property(fc.float({ max: maximumBorrowingRate }), borrowingRate => {
          const withMinimumDebt = Trove.recreate(
            new Trove(Decimal.ONE, XBRL_MINIMUM_DEBT),
            borrowingRate
          );

          const ret = Trove.create(withMinimumDebt, borrowingRate).debt.gte(XBRL_MINIMUM_DEBT);
