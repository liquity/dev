...
import {
  ...
  TroveCreationParams,
  Fees,
  ONEUSD_LIQUIDATION_RESERVE,
  MAXIMUM_BORROWING_RATE,
  MINIMUM_BORROWING_RATE,
  ONEUSD_MINIMUM_DEBT,
  ONEUSD_MINIMUM_NET_DEBT
...
// Extra ONE sent to users to be spent on gas
const GAS_BUDGET = Decimal.from(0.1); // ONE
...
describe("EthersLiquity", () => {
  ...
  describe("findHintForCollateralRatio", () => {
    it("should pick the closest approx hint", async () => {
      ...
      const params = Trove.recreate(new Trove(Decimal.from(1), ONEUSD_MINIMUM_DEBT));
  ...
  describe("Trove", () => {
    ...
    it("should fail to create an undercollateralized Trove", async () => {
      const price = await liquity.getPrice();
      const undercollateralized = new Trove(ONEUSD_MINIMUM_DEBT.div(price), ONEUSD_MINIMUM_DEBT);
    ...
    it("should fail to create a Trove with too little debt", async () => {
      const withTooLittleDebt = new Trove(Decimal.from(50), ONEUSD_MINIMUM_DEBT.sub(1));

      await expect(liquity.openTrove(Trove.recreate(withTooLittleDebt))).to.eventually.be.rejected;
    });

    const withSomeBorrowing = { depositCollateral: 50, borrow1USD: ONEUSD_MINIMUM_NET_DEBT.add(100) };
    ...
    it("should create a Trove with some borrowing", async () => {
      ...
      expect(`${fee}`).to.equal(`${MINIMUM_BORROWING_RATE.mul(withSomeBorrowing.borrow1USD)}`);
    });
    ...
    const repaySomeDebt = { repay1USD: 10 };

    it("should repay some debt", async () => {
      const { newTrove, fee } = await liquity.repay1USD(repaySomeDebt.repay1USD);
      expect(newTrove).to.deep.equal(Trove.create(withSomeBorrowing).adjust(repaySomeDebt));
      expect(`${fee}`).to.equal("0");
    });

    const borrowSomeMore = { borrow1USD: 20 };

    it("should borrow some more", async () => {
      const { newTrove, fee } = await liquity.borrow1USD(borrowSomeMore.borrow1USD);
      expect(newTrove).to.deep.equal(
        Trove.create(withSomeBorrowing).adjust(repaySomeDebt).adjust(borrowSomeMore)
      );
      expect(`${fee}`).to.equal(`${MINIMUM_BORROWING_RATE.mul(borrowSomeMore.borrow1USD)}`);
    });
    ...
    const repayAndWithdraw = { repay1USD: 60, withdrawCollateral: 0.5 };
    ...
    const borrowAndDeposit = { borrow1USD: 60, depositCollateral: 0.5 };
    ...
    it("should borrow more and deposit some collateral at the same time", async () => {
      ...
      expect(`${fee}`).to.equal(`${MINIMUM_BORROWING_RATE.mul(borrowAndDeposit.borrow1USD)}`);
    ...
    it("should close the Trove with some 1USD from another user", async () => {
      const price = await liquity.getPrice();
      const initialTrove = await liquity.getTrove();
      const oneusdBalance = await liquity.getLQTYBalance();
      const oneusdShortage = initialTrove.netDebt.sub(oneusdBalance);

      let funderTrove = Trove.create({ depositCollateral: 1, borrow1USD: oneusdShortage });
      funderTrove = funderTrove.setDebt(Decimal.max(funderTrove.debt, ONEUSD_MINIMUM_DEBT));
      ...
      await funderLiquity.send1USD(await user.getAddress(), oneusdShortage);

      const { params } = await liquity.closeTrove();

      expect(params).to.deep.equal({
        withdrawCollateral: initialTrove.collateral,
        repay1USD: initialTrove.netDebt
      });
  ...
  describe("SendableEthersLiquity", () => {
    it("should parse failed transactions without throwing", async () => {
      ...
      const tx = await liquity.send.openTrove(
        { depositCollateral: 0.01, borrow1USD: 0.01 },
        undefined,
        { gasLimit: 1e6 }
      );
  ...
  describe("Frontend", () => {
    ...
    it("other user's deposit should be tagged with the frontend's address", async () => {
      ...
      await otherLiquity.openTrove({ depositCollateral: 20, borrow1USD: ONEUSD_MINIMUM_DEBT });

      await otherLiquity.deposit1USDInStabilityPool(ONEUSD_MINIMUM_DEBT);
  ...
  describe("StabilityPool", () => {
    before(async () => {
      ...
      await funder.sendTransaction({
        to: otherUsers[0].getAddress(),
        value: ONEUSD_MINIMUM_DEBT.div(170).hex
      });
    });

    const initialTroveOfDepositor = Trove.create({
      depositCollateral: ONEUSD_MINIMUM_DEBT.div(100),
      borrow1USD: ONEUSD_MINIMUM_NET_DEBT
    });
    ...
    it("should make a small stability deposit", async () => {
      ...
      const details = await liquity.deposit1USDInStabilityPool(smallStabilityDeposit);

      expect(details).to.deep.equal({
        oneusdLoss: Decimal.from(0),
        new1USDDeposit: smallStabilityDeposit,
        collateralGain: Decimal.from(0),
        lqtyReward: Decimal.from(0),

        change: {
          deposit1USD: smallStabilityDeposit
        }
      });
    });

    const troveWithVeryLowICR = Trove.create({
      depositCollateral: ONEUSD_MINIMUM_DEBT.div(180),
      borrow1USD: ONEUSD_MINIMUM_NET_DEBT
    });
    ...
    it("should liquidate other user's Trove", async () => {
      ...
      expect(details).to.deep.equal({
        ...
        oneusdGasCompensation: ONEUSD_LIQUIDATION_RESERVE,
    ...
    describe("when people overstay", () => {
      before(async () => {
        ...
        // Use this account to print ONEUSD
        await liquity.openTrove({ depositCollateral: 50, borrow1USD: 5000 });

        // otherLiquities[0-2] will be independent stability depositors
        await liquity.send1USD(await otherUsers[0].getAddress(), 3000);
        await liquity.send1USD(await otherUsers[1].getAddress(), 1000);
        await liquity.send1USD(await otherUsers[2].getAddress(), 1000);

        // otherLiquities[3-4] will be Trove owners whose Troves get liquidated
        await otherLiquities[3].openTrove({ depositCollateral: 21, borrow1USD: 2900 });
        await otherLiquities[4].openTrove({ depositCollateral: 21, borrow1USD: 2900 });

        await otherLiquities[0].deposit1USDInStabilityPool(3000);
        await otherLiquities[1].deposit1USDInStabilityPool(1000);
        ...
        await otherLiquities[2].deposit1USDInStabilityPool(1000);
        ...
        expect(`${await liquity.get1USDInStabilityPool()}`).to.equal("0");
      });
      ...
      it("should still be able to withdraw remaining deposit", async () => {
        for (const l of [otherLiquities[0], otherLiquities[1], otherLiquities[2]]) {
          ...
          await l.withdraw1USDFromStabilityPool(stabilityDeposit.current1USD);
        }
      });
    });
  });
  ...
  describe("Redemption", () => {
    const troveCreations = [
      { depositCollateral: 99, borrow1USD: 4600 },
      { depositCollateral: 20, borrow1USD: 2000 }, // net debt: 2010
      { depositCollateral: 20, borrow1USD: 2100 }, // net debt: 2110.5
      { depositCollateral: 20, borrow1USD: 2200 } //  net debt: 2211
    ];
    ...
    it("should fail to redeem during the bootstrap phase", async () => {
      ...
      await expect(liquity.redeem1USD(4326.5)).to.eventually.be.rejected;
    });

    const some1USD = Decimal.from(4326.5);

    it("should redeem some 1USD after the bootstrap phase", async () => {
      ...
      const expectedDetails = {
        attempted1USDAmount: some1USD,
        actual1USDAmount: some1USD,
        collateralTaken: some1USD.div(200),
        fee: new Fees(0, 0.99, 2, new Date(), new Date(), false)
          .redemptionRate(some1USD.div(total.debt))
          .mul(some1USD.div(200))
      };

      const { rawReceipt, details } = await waitForSuccess(liquity.send.redeem1USD(some1USD));
      ...
      expect(`${await liquity.get1USDBalance()}`).to.equal("273.5");
    ...
    it("borrowing rate should be maxed out now", async () => {
      const borrow1USD = Decimal.from(10);

      const { fee, newTrove } = await liquity.borrow1USD(borrow1USD);
      expect(`${fee}`).to.equal(`${borrow1USD.mul(MAXIMUM_BORROWING_RATE)}`);

      expect(newTrove).to.deep.equal(
        Trove.create(troveCreations[0]).adjust({ borrow1USD }, MAXIMUM_BORROWING_RATE)
      );
    });
  });

  describe("Redemption (truncation)", () => {
    const troveCreationParams = { depositCollateral: 20, borrow1USD: 2000 };
    const netDebtPerTrove = Trove.create(troveCreationParams).netDebt;
    const amountToAttempt = Decimal.from(3000);
    const expectedRedeemable = netDebtPerTrove.mul(2).sub(ONEUSD_MINIMUM_NET_DEBT);
    ...
    beforeEach(async () => {
      ...
      await liquity.openTrove({ depositCollateral: 99, borrow1USD: 5000 });
    ...
    it("should truncate the amount if it would put the last Trove below the min debt", async () => {
      const redemption = await liquity.populate.redeem1USD(amountToAttempt);
      expect(`${redemption.attempted1USDAmount}`).to.equal(`${amountToAttempt}`);
      expect(`${redemption.redeemable1USDAmount}`).to.equal(`${expectedRedeemable}`);
      expect(redemption.isTruncated).to.be.true;

      const { details } = await waitForSuccess(redemption.send());
      expect(`${details.attempted1USDAmount}`).to.equal(`${expectedRedeemable}`);
      expect(`${details.actual1USDAmount}`).to.equal(`${expectedRedeemable}`);
    });

    it("should increase the amount to the next lowest redeemable value", async () => {
      const increasedRedeemable = expectedRedeemable.add(ONEUSD_MINIMUM_NET_DEBT);

      const initialRedemption = await liquity.populate.redeem1USD(amountToAttempt);
      const increasedRedemption = await initialRedemption.increaseAmountByMinimumNetDebt();
      expect(`${increasedRedemption.attempted1USDAmount}`).to.equal(`${increasedRedeemable}`);
      expect(`${increasedRedemption.redeemable1USDAmount}`).to.equal(`${increasedRedeemable}`);
      expect(increasedRedemption.isTruncated).to.be.false;

      const { details } = await waitForSuccess(increasedRedemption.send());
      expect(`${details.attempted1USDAmount}`).to.equal(`${increasedRedeemable}`);
      expect(`${details.actual1USDAmount}`).to.equal(`${increasedRedeemable}`);
    });

    it("should fail to increase the amount if it's not truncated", async () => {
      const redemption = await liquity.populate.redeem1USD(netDebtPerTrove);
  ...
  describe("Redemption (gas checks)", function () {
    ...
    const collateralPerTrove = netDebtPerTrove
      .add(ONEUSD_LIQUIDATION_RESERVE)
    ...
    before(async function () {
      ...
      for (const otherLiquity of otherLiquities) {
        await otherLiquity.openTrove({
          ...
          borrow1USD: amountToBorrowPerTrove
        });
      }
    ...
    it("should redeem using the maximum iterations and almost all gas", async () => {
      await liquity.openTrove({
        ...
        borrow1USD: amountToRedeem
      });

      const { rawReceipt } = await waitForSuccess(liquity.send.redeem1USD(amountToRedeem));
  ...
  describe("Hints (adjustTrove)", () => {
    ...
    before(async () => {
      ...
      await openTroves(eightOtherUsers, [
        { depositCollateral: 30, borrow1USD: 2000 }, // 0
        { depositCollateral: 30, borrow1USD: 2100 }, // 1
        { depositCollateral: 30, borrow1USD: 2200 }, // 2
        { depositCollateral: 30, borrow1USD: 2300 }, // 3
        // Test 1:           30,             2400
        { depositCollateral: 30, borrow1USD: 2500 }, // 4
        { depositCollateral: 30, borrow1USD: 2600 }, // 5
        { depositCollateral: 30, borrow1USD: 2700 }, // 6
        { depositCollateral: 30, borrow1USD: 2800 } //  7
        // Test 2:           30,             2900
    ...
    it("should not traverse the whole list when bottom Trove moves", async () => {
      ...
      const targetTrove = Trove.create({ depositCollateral: 30, borrow1USD: 2900 });
      const interferingTrove = Trove.create({ depositCollateral: 30, borrow1USD: 3000 });
    ...
    it("should not traverse the whole list when lowering ICR of bottom Trove", async () => {
      ...
      const targetTrove = [
        Trove.create({ depositCollateral: 30, borrow1USD: 3100 }),
        Trove.create({ depositCollateral: 30, borrow1USD: 3200 })
      ];
  ...
  describe("Gas estimation", () => {
    ...
    before(async function () {
      ...
      await openTroves(fiveOtherUsers, [
        { depositCollateral: 20, borrow1USD: 2040 },
        { depositCollateral: 20, borrow1USD: 2050 },
        { depositCollateral: 20, borrow1USD: 2060 },
        { depositCollateral: 20, borrow1USD: 2070 },
        { depositCollateral: 20, borrow1USD: 2080 }
      ]);
    ...
    it("should include enough gas for updating lastFeeOperationTime", async () => {
      await liquity.openTrove({ depositCollateral: 20, borrow1USD: 2090 });

      // We just updated lastFeeOperationTime, so this won't anticipate having to update that
      // during estimateGas
      const tx = await liquity.populate.redeem1USD(1);
    ...
    it("should include enough gas for one extra traversal", async () => {
      ...
      expect(adjustment.borrow1USD).to.be.undefined;
      ...
      assertDefined(rudeCreation.borrow1USD);
      const oneusdShortage = rudeTrove.debt.sub(rudeCreation.borrow1USD);

      await liquity.send1USD(await rudeUser.getAddress(), oneusdShortage);
    ...
    it("should include enough gas for both when borrowing", async () => {
      ...
      const adjustment = trove.adjustTo(newTrove);
      expect(adjustment.borrow1USD).to.not.be.undefined;
  ...
  describe("Gas estimation (LQTY issuance)", () => {
    ...
    it("should include enough gas for issuing LQTY", async function () {
      ...
      await liquity.openTrove({ depositCollateral: 40, borrow1USD: 4000 });
      await liquity.deposit1USDInStabilityPool(19);
      ...
      const deposit = await liquity.populate.deposit1USDInStabilityPool(1);
      const withdraw = await liquity.populate.withdraw1USDFromStabilityPool(1);
      ...
      await deployerLiquity.deposit1USDInStabilityPool(creation.borrow1USD);
    ...
    before(async function () {
      ...
      await openTroves(
        ...
        someMoreUsers.map((_, i) => ({
          ...
          borrow1USD: ONEUSD_MINIMUM_NET_DEBT.add(i / 10)
        }))
      );
      ...
      await Promise.all(
        otherLiquities.map(async otherLiquity =>
          otherLiquity.send1USD(await user.getAddress(), await otherLiquity.get1USDBalance())
        )
      );
      ...
      const redeemedTroveDebt = await liquity
        .get1USDBalance()
        .then(x => x.div(10).add(ONEUSD_LIQUIDATION_RESERVE));
      ...
      const { actual1USDAmount } = await liquity.redeem1USD(redeemedTrove.netDebt);

      expect(`${actual1USDAmount}`).to.equal(`${redeemedTrove.netDebt}`);
