...
describe("EthersLiquity", () => {
  ...
  describe("Trove", () => {
    ...
    it("should repay some debt", async () => {
      const { newTrove, fee } = await liquity.repayXBRL(repaySomeDebt.repayXBRL);
    ...
    it("should borrow some more", async () => {
      const { newTrove, fee } = await liquity.borrowXBRL(borrowSomeMore.borrowXBRL);
    ...
    it("should close the Trove with some XBRL from another user", async () => {
      ...
      const xbrlBalance = await liquity.getSTBLBalance();
      ...
      await funderLiquity.sendXBRL(await user.getAddress(), xbrlShortage);
  ...
  describe("Frontend", () => {
    ...
    it("other user's deposit should be tagged with the frontend's address", async () => {
      ...
      await otherLiquity.openTrove({ depositCollateral: 20, borrowXBRL: XBRL_MINIMUM_DEBT });

      await otherLiquity.depositXBRLInStabilityPool(XBRL_MINIMUM_DEBT);
  ...
  describe("StabilityPool", () => {
    ...
    it("should make a small stability deposit", async () => {
      ...
      const details = await liquity.depositXBRLInStabilityPool(smallStabilityDeposit);
    ...
    describe("when people overstay", () => {
      before(async () => {
        ...
        await liquity.openTrove({ depositCollateral: 50, borrowXBRL: 5000 });

        // otherLiquities[0-2] will be independent stability depositors
        await liquity.sendXBRL(await otherUsers[0].getAddress(), 3000);
        await liquity.sendXBRL(await otherUsers[1].getAddress(), 1000);
        await liquity.sendXBRL(await otherUsers[2].getAddress(), 1000);
        ...
        expect(`${await liquity.getXBRLInStabilityPool()}`).to.equal("0");
      });
  ...
  describe("Redemption", () => {
    ...
    it("should fail to redeem during the bootstrap phase", async () => {
      ...
      await expect(liquity.redeemXBRL(4326.5)).to.eventually.be.rejected;
    });
    ...
    it("should redeem some XBRL after the bootstrap phase", async () => {
      ...
      const { rawReceipt, details } = await waitForSuccess(liquity.send.redeemXBRL(someXBRL));
      ...
      expect(`${await liquity.getXBRLBalance()}`).to.equal("273.5");
    ...
    it("borrowing rate should be maxed out now", async () => {
      ...
      const { fee, newTrove } = await liquity.borrowXBRL(borrowXBRL);
  ...
  describe("Redemption (truncation)", () => {
    ...
    beforeEach(async () => {
      ...
      await liquity.openTrove({ depositCollateral: 99, borrowXBRL: 5000 });
    ...
    it("should truncate the amount if it would put the last Trove below the min debt", async () => {
      const redemption = await liquity.populate.redeemXBRL(amountToAttempt);
    ...
    it("should increase the amount to the next lowest redeemable value", async () => {
      ...
      const initialRedemption = await liquity.populate.redeemXBRL(amountToAttempt);
    ...
    it("should fail to increase the amount if it's not truncated", async () => {
      const redemption = await liquity.populate.redeemXBRL(netDebtPerTrove);
  ...
  describe("Redemption (gas checks)", function () {
    ...
    it("should redeem using the maximum iterations and almost all gas", async () => {
      ...
      const { rawReceipt } = await waitForSuccess(liquity.send.redeemXBRL(amountToRedeem));
  ...
  describe("Gas estimation", () => {
    ...
    it("should include enough gas for updating lastFeeOperationTime", async () => {
      await liquity.openTrove({ depositCollateral: 20, borrowXBRL: 2090 });

      // We just updated lastFeeOperationTime, so this won't anticipate having to update that
      // during estimateGas
      const tx = await liquity.populate.redeemXBRL(1);
    ...
    it("should include enough gas for one extra traversal", async () => {
      ...
      await liquity.sendXBRL(await rudeUser.getAddress(), xbrlShortage);
  ...
  describe("Gas estimation (STBL issuance)", () => {
    ...
    it("should include enough gas for issuing STBL", async function () {
      ...
      await liquity.openTrove({ depositCollateral: 40, borrowXBRL: 4000 });
      await liquity.depositXBRLInStabilityPool(19);
      ...
      const deposit = await liquity.populate.depositXBRLInStabilityPool(1);
      const withdraw = await liquity.populate.withdrawXBRLFromStabilityPool(1);
      ...
      await deployerLiquity.depositXBRLInStabilityPool(creation.borrowXBRL);
    ...
    before(async function () {
      ...
      await Promise.all(
        otherLiquities.map(async otherLiquity =>
          otherLiquity.sendXBRL(await user.getAddress(), await otherLiquity.getXBRLBalance())
        )
      );
      ...
      const { actualXBRLAmount } = await liquity.redeemXBRL(redeemedTrove.netDebt);
