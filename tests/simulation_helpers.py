...
VST_GAS_COMPENSATION = 200.0
MIN_NET_DEBT = 1800.0
MAX_FEE = Wei(1e18)

"""# Ether price (exogenous)

Ether is the collateral for VST. The ether price $P_t^e$ follows
...
"""# VSTA price
In the first month, the price of VSTA follows

> $P_t^q = P_{t-1}^q (1+\zeta_t^q)(1+\sigma_t^q)$. 

Note that $\zeta_t^q \sim N(0,$ sd_VSTA) represents VSTA price shock and $\sigma_t^q$ the drift. Here, $\sigma_t^q =$ drift_VSTA, so that the expected VSTA price increases from price_VSTA_initial to the following at the end of the first month:
> $E(P_{720}^q) = $price_VSTA_initial$ \cdot (1+$ drift_VSTA$)^{720}$

The VSTA price from the second month on is endogenously determined.
"""

#VSTA price & airdrop
price_VSTA_initial = 0.4
...
"""**VSTA Endogenous Price**

The staked VSTA pool earning consists of the issuance fee revenue and redemption fee revenue
> $R_t^q = R_t^i + R_t^r.$
...
> $$r_t=\frac{VSTA Market Cap}{Annualized Earning}=\frac{MC_t}{E_t}$$

> $MC_t=P_t^q \cdot$ VSTA_total_supply

Therefore, the VSTA price dynamics is determined
> $$P_t^q=discount \cdot \frac{r^{PE}}{VSTA\_total\_supply}E_t$$

Interpretation: The denominator implies that with more VSTA tokens issued, VSTA price decreases. However, the depreciation effect can be counteracted by the growth of the earning.
...
$R_{t-1}^s$ is the return in the stability pool, which consists of liquidation gain and airdrop VSTA gain.
...
> (collateral in ether, debt in VST, collateral ratio target, rational inattention, collateral ratio)
...
$P_{t}^{l}$ is the price of VST.

$N_t^o$ is rounded to an integer.

---

The amount of VST tokens generated by a new trove is
...
- $R_t^a=P_{t}^q\hat{Q}_t^q$ is the airdrop gain, $\hat{Q}_t^q=1000$ denotes the amount of VSTA token airdropped to the stability pool providers
- $D_{t}^{s}$ is the total amount of VST deposited in the Stability Pool (see below)
...
def pending_liquidations(contracts, price_ether_current):
    ...
    stability_pool_balance = contracts.stabilityPool.getTotalVSTDeposits()
    trove = last_trove
...
# F = 0.5 ** (1/8760)
# F = 0.99992087674
def quantity_VSTA_airdrop(index):
    F = 0.99992087674
    if index <= 0:
        return 0
    return 32e6 * (F ** (index-1) - F ** index)

def liquidate_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_USDV, price_VSTA_current, data, index):
    if len(active_accounts) == 0:
        return [0, 0]

    stability_pool_previous = contracts.stabilityPool.getTotalVSTDeposits() / 1e18
    ...
    while pending_liquidations(contracts, price_ether_current):
        try:
            ...
            for i in range(NUM_LIQUIDATIONS):
                ...
                ICR = contracts.troveManager.getCurrentICR(trove, Wei(price_ether_current * 1e18))
                print(f"ICR: {ICR}")
    stability_pool_current = contracts.stabilityPool.getTotalVSTDeposits() / 1e18
    ...
    airdrop_gain = price_VSTA_current * quantity_VSTA_airdrop(index)
...
def transfer_from_to(contracts, from_account, to_account, amount):
    balance = contracts.vstToken.balanceOf(from_account)
    transfer_amount = min(balance, amount)
    if transfer_amount == 0:
        return amount
    if from_account == to_account:
        return amount
    contracts.vstToken.transfer(to_account, transfer_amount, { 'from': from_account })
    pending = amount - transfer_amount

    return pending

def get_USDV_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt):
    USDVBalance = contracts.vstToken.balanceOf(account)
    if debt > USDVBalance:
        ...
        # try with active accounts, which are more likely to hold VST
        ...
        if pending > 0:
            print(f"\n ***Error: not enough VST to repay! {debt / 1e18} VST for {account}")
    ...
    for i, working_trove in enumerate(active_accounts):
        ...
        if p >= ratio:
            ...
            if check < -1:
                ...
                if pending == 0:
                    contracts.borrowerOperations.repayVST(repay_amount, hints[0], hints[1], { 'from': account })
                ...
                if isNewTCRAboveCCR(contracts, 0, False, withdraw_amount_wei, True, floatToWei(price_ether_current)):
                    contracts.borrowerOperations.withdrawVST(MAX_FEE, withdraw_amount_wei, hints[0], hints[1], { 'from': account })
...
def open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_ether, CR_ratio, rational_inattention, price_ether_current):
    ...
    debtChange = floatToWei(supply_trove) + VST_GAS_COMPENSATION
    VST = get_USDV_amount_from_net_debt(contracts, floatToWei(supply_trove))
    if isNewTCRAboveCCR(contracts, coll, True, debtChange, True, floatToWei(price_ether_current)):
        contracts.borrowerOperations.openTrove(MAX_FEE, VST, hints[0], hints[1],
...
def stability_update(accounts, contracts, active_accounts, return_stability, index):
    supply = contracts.vstToken.totalSupply() / 1e18
    stability_pool_previous = contracts.stabilityPool.getTotalVSTDeposits() / 1e18
    ...
    if stability_pool > stability_pool_previous:
        ...
        while remaining > 0 and i < len(active_accounts):
          ...
          balance = contracts.vstToken.balanceOf(account) / 1e18
        ...
        current_deposit = contracts.stabilityPool.getCompoundedVSTDeposit(accounts[0])
...
---
With the supply and demand of VST tokens defined above, the price of VST at the current period is given by the following equilibrium condition:
> $$S_t = D_t^s + D_t^l = D_t^s + D_{t-1}^l (1+\zeta_t^l)(1+\sigma_t^l) (\frac{P_t^l}{P_{t-1}^l})^\delta$$

where $S$ is the total supply of VST.
...
There are two stabilizers to attenuate VST price deviation from its target range.
...
The amount of $Q_t^d(c)$ is expected to bring the VST price back to $1.1+f_t^i$. This means that
...
If $P_t^l < 1-f_t^r$, a fraction $\chi_t$ of VST in the liquidity pool is used for redemption
> $$D_t^r = \chi_t D_t^l,$$
...
Note that unlike stabilizer 1, stabilizer 2 has impact of VST price in
...
def redeem_trove(accounts, contracts, i, price_ether_current):
    USDV_balance = contracts.vstToken.balanceOf(accounts[i])
    [firstRedemptionHint, partialRedemptionHintNICR, truncatedVSTamount] = contracts.hintHelpers.getRedemptionHints(USDV_balance, price_ether_current, 70)
    if truncatedVSTamount == Wei(0):
        return None
    ...
    try:
        ...
        print(f"VST Token:    {contracts.vstToken.address}")
        print(f"i: {i}")
        print(f"account: {accounts[i]}")
        print(f"VST bal: {USDV_balance / 1e18}")
        print(f"truncated: {truncatedVSTamount / 1e18}")
        ...
        print(f"amount: {truncatedVSTamount}")
...
def price_stabilizer(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_USDV, index):
    ...
    stability_pool = contracts.stabilityPool.getTotalVSTDeposits() / 1e18
    redemption_pool = 0
    redemption_fee = 0
...
def VSTA_market(index, data):
    #quantity_VSTA = (VSTA_total_supply/3)*(1-0.5**(index/period))
    ...
    return [price_VSTA_current, annualized_earning]
