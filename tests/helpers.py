...
def logGlobalState(contracts):
    ...
    activePoolDebt = contracts.activePool.getVSTDebt()
    defaultPoolColl = contracts.defaultPool.getETH()
    defaultPoolDebt = contracts.defaultPool.getVSTDebt()
    ...
    SP_USDV = contracts.stabilityPool.getTotalVSTDeposits().to("ether")
    SP_ETH = contracts.stabilityPool.getETH().to("ether")
    print('SP VST         ', SP_USDV)
    print('SP ETH          ', SP_ETH)
